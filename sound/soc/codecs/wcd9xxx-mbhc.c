...
/* N-FAUDIO-151056005 start */
//  int idxmclk = wcd9xxx_acdb_mclk_index(mbhc->mbhc_cfg->mclk_rate);
    int idxmclk = 0;
    int count_waitloop = 250;//Max 10ms*250 = 2.5sec
    while(count_waitloop--){
        if(mbhc->mbhc_cfg != NULL){
            idxmclk = wcd9xxx_acdb_mclk_index(mbhc->mbhc_cfg->mclk_rate);
            break;
        }else{
            usleep_range(10000, 10000);  // 10ms sleep
        }
    }

    if(count_waitloop <= 0){
        pr_debug("%s give up! waiting over 2.5sec!!\n", __func__);
        return;
    }
/* N-FAUDIO-151056005 end */
