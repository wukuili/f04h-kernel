...
/* N-FAUDIO-151061000  start */
#include <linux/of_gpio.h>
/* N-FAUDIO-151061000  end */
#include "wcd9330.h"
...
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
#include <linux/kobject.h>
#include <linux/string.h>
#include <linux/sysfs.h>
#include <linux/module.h>
#include <linux/init.h>
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 end */
...
/* H-FAUDIO-000046000 start */
#define RX_B6_CTL_MUTE_EN_BIT 0x01
/* H-FAUDIO-000046000 end */
...
/* H-FAUDIO-000056000 start */
static int tomtom_suspend_flag = 0;
/* H-FAUDIO-000056000 end */
/* H-FAUDIO-000056003 start */
static bool earphone_set_flag = false;
/* H-FAUDIO-000056003 end */

/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
static struct kobject *volstepup_kobj = NULL;
static int volStepupMode = 0;

static ssize_t volstepup_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	ssize_t ret = sprintf(buf, "%s:%d\n", attr->attr.name, volStepupMode);
	return ret;
}
static ssize_t volstepup_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	sscanf(buf, "%du", &volStepupMode);
	return count;
}

static struct kobj_attribute volstepup_attribute = __ATTR(volstepupmode, 0660, volstepup_show, volstepup_store);

static struct attribute *volstepup_attrs[] = {
	&volstepup_attribute.attr,
	NULL,   /* need to NULL terminate the list of attributes */
};

static struct attribute_group volstepup_attr_group = {
	.attrs = volstepup_attrs,
};

static int volstepup_create_sysfs(void)
{
	int retval = 0;

	if (volstepup_kobj == NULL) {
		volstepup_kobj = kobject_create_and_add("volstepup", kernel_kobj);

		if (!volstepup_kobj) {
			pr_err("kobject_create_and_add for volstepup failed");
			return -ENOMEM;
		}

		retval = sysfs_create_group(volstepup_kobj, &volstepup_attr_group);
		if (retval) {
			pr_err("sysfs group creation failed, retval=%d\n", retval);
			kobject_put(volstepup_kobj);
		}
	}
...
/* H-FAUDIO-000056000 start */
#define SUSPEND_PENDING_CHK 255
/* H-FAUDIO-000056000 end */
...
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
extern bool lastcall_pl_flg;
/* T-FAUDIO-161026011 start */
static u32 disable_recv_ms = 0;
/* T-FAUDIO-161026011 end */
static bool vol_mute_thread_flag = false;
static bool vol_mute_thread_flag_recv = false;
static bool vol_stepup_thread_flag = false;
static struct task_struct *tomtom_vol_mute_task = NULL;
static struct task_struct *tomtom_vol_mute_task_recv = NULL;
static struct task_struct *tomtom_vol_stepup_task = NULL;
static bool mute_rx1_flg = false;
static bool mute_rx2_flg = false;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
/* N-FAUDIO-151016028 start */
static bool mute_rx7_flg = false;
/* N-FAUDIO-151016028 end */
...
struct tomtom_priv {
	...
	/* N-FAUDIO-151061000  start */
    /* N-FAUDIO-161061034 start */
	//int gpio_aud_asw;
    /* N-FAUDIO-161061034 end */
	/* N-FAUDIO-151061000  end */
};
...
/* H-FAUDIO-000046000 start */
static const char *rx_force_mute_param_text[] = {
       "MUTE_ALL_OFF", "MUTE_ALL_ON", "MUTE_RX1_OFF", "MUTE_RX1_ON",
       "MUTE_RX2_OFF", "MUTE_RX2_ON", "MUTE_RX3_OFF", "MUTE_RX3_ON",
       "MUTE_RX4_OFF", "MUTE_RX4_ON", "MUTE_RX5_OFF", "MUTE_RX5_ON",
       "MUTE_RX6_OFF", "MUTE_RX6_ON", "MUTE_RX7_OFF", "MUTE_RX7_ON"
};

static const struct soc_enum rx_force_mute_param_enum =
       SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(rx_force_mute_param_text), rx_force_mute_param_text);

static int rx_force_mute_get(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
       pr_debug("%s: called\n", __func__);
       return 0;
...
static int rx_force_mute_set(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	pr_debug("%s: control value = %d \n", __func__, (int)ucontrol->value.integer.value[0]);
	switch(ucontrol->value.integer.value[0]) {
	case 0:         // "MUTE_ALL_OFF"
		pr_debug("%s: Rx Force MUTE_ALL_OFF\n", __func__);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx1_flg = false;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX1_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx2_flg = false;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX2_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX3_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX4_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX5_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX6_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
/* N-FAUDIO-151016028 start */
		mute_rx7_flg = false;
/* N-FAUDIO-151016028 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 1:         // "MUTE_ALL_ON"
		pr_debug("%s: Rx Force MUTE_ALL_ON\n", __func__);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx1_flg = true;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX1_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx2_flg = true;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX2_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX3_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX4_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX5_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX6_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
/* N-FAUDIO-151016028 start */
		mute_rx7_flg = true;
/* N-FAUDIO-151016028 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 2:         // "MUTE_RX1_OFF"
		pr_debug("%s: Rx Force MUTE_RX1_OFF\n", __func__);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx1_flg = false;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX1_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 3:         // "MUTE_RX1_ON"
		pr_debug("%s: Rx Force MUTE_RX1_ON\n", __func__);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx1_flg = true;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX1_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 4:         // "MUTE_RX2_OFF"
		pr_debug("%s: Rx Force MUTE_RX2_OFF\n", __func__);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx2_flg = false;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX2_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 5:         // "MUTE_RX2_ON"
		pr_debug("%s: Rx Force MUTE_RX2_ON\n", __func__);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		mute_rx2_flg = true;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX2_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 6:         // "MUTE_RX3_OFF"
		pr_debug("%s: Rx Force MUTE_RX3_OFF\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX3_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 7:         // "MUTE_RX3_ON"
		pr_debug("%s: Rx Force MUTE_RX3_ON\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX3_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 8:         // "MUTE_RX4_OFF"
		pr_debug("%s: Rx Force MUTE_RX4_OFF\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX4_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 9:         // "MUTE_RX4_ON"
		pr_debug("%s: Rx Force MUTE_RX4_ON\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX4_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 10:         // "MUTE_RX5_OFF"
		pr_debug("%s: Rx Force MUTE_RX5_OFF\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX5_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 11:         // "MUTE_RX5_ON"
		pr_debug("%s: Rx Force MUTE_RX5_ON\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX5_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 12:         // "MUTE_RX6_OFF"
		pr_debug("%s: Rx Force MUTE_RX6_OFF\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX6_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 13:         // "MUTE_RX6_ON"
		pr_debug("%s: Rx Force MUTE_RX6_ON\n", __func__);
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX6_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	case 14:         // "MUTE_RX7_OFF"
		pr_debug("%s: Rx Force MUTE_RX7_OFF\n", __func__);
/* N-FAUDIO-151016028 start */
		mute_rx7_flg = false;
/* N-FAUDIO-151016028 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x00);
		break;
	case 15:         // "MUTE_RX7_ON"
		pr_debug("%s: Rx Force MUTE_RX7_ON\n", __func__);
/* N-FAUDIO-151016028 start */
		mute_rx7_flg = true;
/* N-FAUDIO-151016028 end */
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_B6_CTL, RX_B6_CTL_MUTE_EN_BIT, 0x01);
		break;
	default:
		pr_err("%s: Rx Force MUTE illegal input value (%d)\n", __func__, (int) ucontrol->value.integer.value[0]);
		break;
	}
	return 0;
}
...
/* N-FAUDIO-151046000 start */
static void rx_force_allmute_set(struct snd_soc_codec *codec, bool setmute)
{
	pr_debug("%s: setmute value = %d ", __func__, setmute);
...
/* N-FAUDIO-151046000 end */
/* N-FAUDIO-151047000 start */
/* N-FAUDIO-151047000 start */
static const char	*tomtom_gain_log_param_text[] = {
	"0", "1",
};

static const struct soc_enum tomtom_gain_log_param_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tomtom_gain_log_param_text), tomtom_gain_log_param_text);
/* N-FAUDIO-151047000 end */

static int tomtom_gain_rx_log_output_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	return 0;
}

static int tomtom_gain_rx_log_output_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	printk(KERN_INFO "%s: EAR PA Gain        = 0x%x\n", __func__, snd_soc_read(codec, TOMTOM_A_RX_EAR_GAIN));
	printk(KERN_INFO "%s: RX1 Digital Volume = 0x%x\n", __func__, snd_soc_read(codec, TOMTOM_A_CDC_RX1_VOL_CTL_B2_CTL));

	return 0;
}

static int tomtom_gain_tx_log_output_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	return 0;
}

static int tomtom_gain_tx_log_output_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	printk(KERN_INFO "%s: ADC1 Volume        = 0x%x\n", __func__, snd_soc_read(codec, TOMTOM_A_TX_1_GAIN));
	printk(KERN_INFO "%s: ADC4 Volume        = 0x%x\n", __func__, snd_soc_read(codec, TOMTOM_A_TX_4_GAIN));
	printk(KERN_INFO "%s: DEC6 Volume        = 0x%x\n", __func__, snd_soc_read(codec, TOMTOM_A_CDC_TX6_VOL_CTL_GAIN));
	printk(KERN_INFO "%s: DEC3 Volume        = 0x%x\n", __func__, snd_soc_read(codec, TOMTOM_A_CDC_TX3_VOL_CTL_GAIN));
...
/* H-FAUDIO-000046000 start */
	SOC_ENUM_EXT("Rx Force Mute", rx_force_mute_param_enum,
		       rx_force_mute_get, rx_force_mute_set),
/* H-FAUDIO-000046000 end */
/* N-FAUDIO-151047000 start */
	SOC_ENUM_EXT("GAIN Rx Log Output", tomtom_gain_log_param_enum,
		tomtom_gain_rx_log_output_get, tomtom_gain_rx_log_output_put),
	SOC_ENUM_EXT("GAIN Tx Log Output", tomtom_gain_log_param_enum,
		tomtom_gain_tx_log_output_get, tomtom_gain_tx_log_output_put),
/* N-FAUDIO-151047000 end */
};
...
//		ucontrol->value.integer.value[0]);
	printk(KERN_INFO "%s: wname %s cname %s value %u shift %d item %ld\n", __func__,
		widget->name, ucontrol->id.name, widget->value, widget->shift,
		ucontrol->value.integer.value[0]);
/* N-FAUDIO-151047000 end */
...
/* H-FAUDIO-000026000 start */
static int tomtom_vol_stepup_thread(void *data)
{
	struct snd_soc_codec *codec = (struct snd_soc_codec *)data;
/* N-FAUDIO-151026000 start */
	// 0xD3:-45db,0xD8:-40db,0xDD:-35db
	// 0xE2:-30db,0xEC:-20db,0x08:+8db
	// +8db from mixer_paths.xml definition speaker-lite,
	// if change the gain setting, reflect it in here.
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
	u8 value[15] = {0xDF, 0xE1, 0xE3, 0xE5, 0xE7, 0xE9, 0xEB, 0xED, 0xEF, 0xF1, 0xF3, 0xF6, 0xFC, 0x02, 0x08};
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 end */
/* N-FAUDIO-152026007 start */
	u8 value_mode1[6] = {0xD8, 0xE2, 0xEC, 0xF6, 0x00, 0x08};
	int mode = volStepupMode;
/* N-FAUDIO-152026007 end */
	int i = 0;
/* N-FAUDIO-151026000 end */

	/* vol_stepup_thread start */
	vol_stepup_thread_flag = true;

/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
	/* min volume set */
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
/* N-FAUDIO-152026007 start */
//	snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, 0xDD);
	if (mode == 1) {
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, 0xCE);
		pr_err("[audio_test]%s volStepupMode is enabled\n", __func__);
	} else {
	snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, 0xDD);
	}
/* N-FAUDIO-152026007 end */
...
//			pr_info("%s thread stop ", __func__);
///* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
//			break;
//		}
//		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, value[i]);
//	}
	if (mode == 1) {
		for(i=0;i<6; i++) {
			msleep(10);
			...
			snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, value_mode1[i]);
		}
	} else {
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
	for(i=0;i<15; i++) {
...
/* N-FAUDIO-152026007 end */
/* N-FAUDIO-151026000 end */
	/* vol_stepup_thread exit */
	vol_stepup_thread_flag = false;
/* N-FAUDIO-152026007 start */
	volStepupMode = 0;
/* N-FAUDIO-152026007 end */
...
/* N-FAUDIO-151046000 start */
static int tomtom_vol_mute_thread(void *data)
{
	struct snd_soc_codec *codec = (struct snd_soc_codec *)data;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
/* FUJITSU LIMITED:2014-09-02 N-FAUDIO-151026000 start */
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
	u8 value_mode1[6] = {0xD8, 0xE2, 0xEC, 0xF6, 0x00, 0x03};
	u8 value_default[15] = {0xDF, 0xE1, 0xE3, 0xE5, 0xE7, 0xE9, 0xEB, 0xED, 0xEF, 0xF1, 0xF3, 0xF6, 0xFC, 0x02, 0x03};
	int mode = volStepupMode;
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 end */
	int i = 0;
/* FUJITSU LIMITED:2014-09-02 N-FAUDIO-151026000 end */
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */

	/* vol_mute_thread start */
	vol_mute_thread_flag = true;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
	/* min volume set */
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
	if (mode == 1) {
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, 0xCE);
		pr_info("%s volStepupMode is enabled\n", __func__);
	} else {
		snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, 0xDD);
	}
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 end */
...
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
	// mute off
	rx_force_allmute_set(codec, false);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
/* FUJITSU LIMITED:2014-09-02 N-FAUDIO-151026000 start */
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 start */
	if (mode == 1) {
		...
		for(i=0;i<15; i++) {
			...
			snd_soc_update_bits(codec, TOMTOM_A_CDC_RX7_VOL_CTL_B2_CTL, 0xFF, value_default[i]);
		}
	}
/* FUJITSU LIMITED:2014-09-17 N-FAUDIO-151026000 end */
/* FUJITSU LIMITED:2014-09-02 N-FAUDIO-151026000 end */
...
/* N-FAUDIO-151046000 start */
		// mute on
		rx_force_allmute_set(codec, true);
/* N-FAUDIO-151046000 end */
/* N-FAUDIO-151061000  start */
/* N-FAUDIO-161061034 start */
//		pr_info("%s: switched to speaker\n",  __func__);
//		gpio_set_value(tomtom->gpio_aud_asw, 0); // L:SPK
/* N-FAUDIO-161061034 end */
/* N-FAUDIO-151061000  end */
		tomtom->spkr_pa_widget_on = true;
    /* N-FAUDIO-161061000 start */
//		snd_soc_update_bits(codec, spk_drv_reg, 0x80, 0x80);
		snd_soc_update_bits(codec, spk_drv_reg, 0xEF, 0xEF);
    /* N-FAUDIO-161061000 end */
/* H-FAUDIO-000026000 start */
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		if(!lastcall_pl_flg){
			// mute off
			rx_force_allmute_set(codec, false);
			do_gettimeofday(&nowtime_tv);
			nowtime_ms = (nowtime_tv.tv_sec * 1000) + (nowtime_tv.tv_usec / 1000);
			/* within 3 sec. receiver -> speaker, and call finished */
			if(nowtime_ms < disable_recv_ms + 3 * 1000) {
				if(vol_stepup_thread_flag) {
					pr_err("%s vol_stepup_thread working, stop thread.", __func__);
					if(NULL!=tomtom_vol_stepup_task) {
						kthread_stop(tomtom_vol_stepup_task);
						tomtom_vol_stepup_task = NULL;
					}
				}
				pr_info("%s kthread run tomtom_vol_stepup_thread\n", __func__);
				tomtom_vol_stepup_task = kthread_run(tomtom_vol_stepup_thread, codec, "tomtom_volstepup");
				if (IS_ERR(tomtom_vol_stepup_task)) {
					ret = PTR_ERR(tomtom_vol_stepup_task);
					pr_err("%s kthread run failed ret = %d\n", __func__, ret);
/* N-FAUDIO-152026007 start */
					volStepupMode = 0;
				}
			} else {
...
/* FUJITSU LIMITED:2014-09-19 N-FAUDIO-151026000 start */
//			volStepupMode = 0;
/* FUJITSU LIMITED:2014-09-19 N-FAUDIO-151026000 end */
/* N-FAUDIO-152026007 end */
			disable_recv_ms = 0;
		}
/* H-FAUDIO-000026000 end */
...
/* FUJITSU LIMITED:2014-08-25 N-FAUDIO-151026000 start */
			do_gettimeofday(&nowtime_tv);
			nowtime_ms = (nowtime_tv.tv_sec * 1000) + (nowtime_tv.tv_usec / 1000);
			/* within 3 sec. receiver -> speaker, and call finished */
			if(nowtime_ms < disable_recv_ms + 3 * 1000) {
/* FUJITSU LIMITED:2014-08-25 N-FAUDIO-151026000 end */
				if(vol_mute_thread_flag) {
					pr_err("%s vol_mute_thread working, stop thread.", __func__);
					if(NULL!=tomtom_vol_mute_task) {
						kthread_stop(tomtom_vol_mute_task);
						tomtom_vol_mute_task = NULL;
					}
				}
				pr_info("%s kthread run tomtom_vol_mute_thread\n", __func__);
				tomtom_vol_mute_task = kthread_run(tomtom_vol_mute_thread, codec, "tomtom_volmute");
				if (IS_ERR(tomtom_vol_mute_task)) {
					ret = PTR_ERR(tomtom_vol_mute_task);
					pr_err("%s mute kthread run failed ret = %d\n", __func__, ret);
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
/* FUJITSU LIMITED:2014-09-22 N-FAUDIO-151026000 start */
					volStepupMode = 0;
/* FUJITSU LIMITED:2014-09-22 N-FAUDIO-151026000 end */
					// mute off
					rx_force_allmute_set(codec, false);
				}
/* FUJITSU LIMITED:2014-08-25 N-FAUDIO-151026000 start */
...
/* FUJITSU LIMITED:2014-09-19 N-FAUDIO-151026000 start */
				/* T-FAUDIO-161026011 start */
				/*
				* disable_recv_ms > 0 means the sequence is ear SND_SOC_DAPM_POST_PMD -> spk SND_SOC_DAPM_PRE_PMU.
				* Any in this case, we reset the value of volStepupMode.
				*/
				if(disable_recv_ms > 0) {
					volStepupMode = 0;
				}
				/* T-FAUDIO-161026011 end */
/* FUJITSU LIMITED:2014-09-19 N-FAUDIO-151026000 end */
...
/* FUJITSU LIMITED:2014-09-16 N-FAUDIO-151026000 start */
			disable_recv_ms = 0;
/* FUJITSU LIMITED:2014-09-16 N-FAUDIO-151026000 end */
/* FUJITSU LIMITED:2014-08-25 N-FAUDIO-151026000 end */
		}
/* N-FAUDIO-151046000 end */
		break;
...
//		snd_soc_update_bits(codec, spk_drv_reg, 0x80, 0x00);
		snd_soc_update_bits(codec, spk_drv_reg, 0xEF, 0x00);
    /* N-FAUDIO-161061000 end */
		break;
	}
	return 0;
...
/* H-FAUDIO-000056000 start */
static struct snd_soc_codec *codec_micbias2external = NULL;
#define MB2_VOTE_CLIENT_ALSA    1
#define MB2_VOTE_CLIENT_EDC     2

/* called under codec_resource_lock acquisition */
static void tomtom_codec_vote_micbias2external(int client_mask,int on) {
	static int vote = 0;
	int vote_curr = vote;
	struct tomtom_priv *priv = snd_soc_codec_get_drvdata(codec_micbias2external);
	u8 cfilt_sel_val = priv->resmgr.pdata->micbias.bias2_cfilt_sel;
	
	if (on) {
		vote |= client_mask;
		if (!vote_curr && vote) {
			/* Get cfilt */
			wcd9xxx_resmgr_cfilt_get(&priv->resmgr, cfilt_sel_val);
			/************************************************/
			/* ported from tomtom_codec_enable_micbias below */
			snd_soc_update_bits(codec_micbias2external,TOMTOM_A_MICB_2_CTL, 1 << 7, 1 << 7);
			/* ported from tomtom_codec_enable_micbias above */
			/************************************************/
		}
	} else {
		vote &= ~client_mask;
		if (vote_curr && !vote) {
			/************************************************/
			/* ported from tomtom_codec_enable_micbias below */
			snd_soc_update_bits(codec_micbias2external,TOMTOM_A_MICB_2_CTL, 1 << 7, 0);
			/* ported from tomtom_codec_enable_micbias above */
			/************************************************/
			/* Put cfilt */
			wcd9xxx_resmgr_cfilt_put(&priv->resmgr, cfilt_sel_val);
		}
	}
}

/* called under codec_resource_lock acquisition */
static void tomtom_codec_vote_ldo_h(int client_mask,int on) {
	static int vote = 0;
	int vote_curr = vote;
	struct tomtom_priv *priv = snd_soc_codec_get_drvdata(codec_micbias2external);
	if (on) {
		...
		if (!vote_curr && vote) {
			/************************************************/
			/* ported from __tomtom_codec_enable_ldo_h below */
			WCD9XXX_BG_CLK_LOCK(&priv->resmgr);
			wcd9xxx_resmgr_get_bandgap(&priv->resmgr,
						   WCD9XXX_BANDGAP_AUDIO_MODE);
			wcd9xxx_resmgr_get_clk_block(&priv->resmgr,
						     WCD9XXX_CLK_RCO);
/* N-FAUDIO-151056006 start */
//			snd_soc_update_bits(codec_micbias2external, TOMTOM_A_LDO_H_MODE_1, 1 << 7,
//					    1 << 7);
			snd_soc_update_bits(codec_micbias2external, TOMTOM_A_LDO_H_MODE_1, 0xFF, 0xC9);
/* N-FAUDIO-151056006 end */
			wcd9xxx_resmgr_put_clk_block(&priv->resmgr,
						     WCD9XXX_CLK_RCO);
			WCD9XXX_BG_CLK_UNLOCK(&priv->resmgr);
			/* LDO enable requires 1ms to settle down */
			usleep_range(1000, 1000);
			/* ported from __tomtom_codec_enable_ldo_h above */
			/************************************************/
		}
	} else {
		vote &= ~client_mask;
...
/* externalized function for EDC */
int headset_micbias_ctrl(int on) {
    static int micbias_pending_operation = 0;

	if (codec_micbias2external == NULL) {
		/* status error */
		printk("headset_micbias_ctrl err: no init \n");
		return -EINVAL;
	}
	pr_info("%s: called, on = %d\n", __func__, on);

    if(on == SUSPEND_PENDING_CHK){                            //call from tomtom_resume
        if(micbias_pending_operation == 1){ //pending
            on = 0;
        }else{                              // not pending
            return 0;
       }
	...
	mutex_lock(&codec_micbias2external->mutex);
	if (on) {
		/* H-FAUDIO-000056003 start */
		earphone_set_flag = true;
		/* H-FAUDIO-000056003 end */
		tomtom_codec_vote_ldo_h(MB2_VOTE_CLIENT_EDC,true);
		tomtom_codec_vote_micbias2external(MB2_VOTE_CLIENT_EDC,true);
/* N-FAUDIO-151056006 start */
		snd_soc_update_bits(codec_micbias2external, TOMTOM_A_MICB_CFILT_2_VAL, 0xFC, 0x9C);
/* N-FAUDIO-151056006 end */
/* N-FAUDIO-161061000 start */
        snd_soc_update_bits(codec_micbias2external, TOMTOM_A_MICB_1_MBHC, 0xFF, 0x02);
        snd_soc_update_bits(codec_micbias2external, TOMTOM_A_MICB_2_CTL, 0xFF, 0xB6);
/* N-FAUDIO-161061000 end */
/* N-FAUDIO-161061027 start */
		snd_soc_update_bits(codec_micbias2external, TOMTOM_A_MICB_2_INT_RBIAS, 0xFF, 0x24);
/* N-FAUDIO-161061027 end */
        micbias_pending_operation = 0; // last operation : headset micbias == on
	} else {
        if(tomtom_suspend_flag == 0) {
			/* H-FAUDIO-000056003 start */
			earphone_set_flag = false;
			/* H-FAUDIO-000056003 end */
			tomtom_codec_vote_micbias2external(MB2_VOTE_CLIENT_EDC,false);
			tomtom_codec_vote_ldo_h(MB2_VOTE_CLIENT_EDC,false);
            micbias_pending_operation = 0; // last operation : headset micbias == off
/* N-FAUDIO-161061000 start */
            snd_soc_update_bits(codec_micbias2external, TOMTOM_A_MICB_1_MBHC, 0xFF, 0x01);
/* N-FAUDIO-161061000 end */
		}
        else{
            pr_info("headset_micbias_ctrl tomtom_suspend_flag != 0 on=0 micbias not set!!");
            micbias_pending_operation = 1; // pending operation
        }
	}
	mutex_unlock(&codec_micbias2external->mutex);
	return 0;
}
EXPORT_SYMBOL_GPL(headset_micbias_ctrl);
/* H-FAUDIO-000056000 end */
...
/* N-FAUDIO-161056008 start */
	if (micb_ctl_reg == TOMTOM_A_MICB_2_CTL) {
		pr_info("%s: MICB_2_CTL NOP return \n",__func__);
		return 0;
	}
/* N-FAUDIO-161056008 end */
...
/* N-FAUDIO-161061000 start */
    snd_soc_update_bits(codec, TOMTOM_A_MICB_CFILT_1_VAL, 0xFF, 0x9C);
    snd_soc_update_bits(codec, TOMTOM_A_MICB_1_CTL, 0xFF, 0x86);
/* N-FAUDIO-161061000 end */
/* N-FAUDIO-161061027 start */
    snd_soc_update_bits(codec, TOMTOM_A_MICB_1_INT_RBIAS, 0xFF, 0x24);
//    snd_soc_update_bits(codec, TOMTOM_A_MICB_2_INT_RBIAS, 0xFF, 0x24);
/* N-FAUDIO-161061027 end */
...
//			w->name, dec_name, decimator);
	printk(KERN_INFO "%s(): widget = %s dec_name = %s decimator = %u event = %d\n", __func__,
			w->name, dec_name, decimator, event);
/* N-FAUDIO-151047000 end */
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		/* T-FAUDIO-161054QQ1 start */
		/*
		* Before PA up, check whether OCP has been detected.
		* If yes, to set OCP_FSM_EN and wait for 30ms to delay PA openning.
		*/
		if ((snd_soc_read(codec, TOMTOM_A_RX_HPH_L_STATUS) & 0x04) ||
			(snd_soc_read(codec, TOMTOM_A_RX_HPH_R_STATUS) & 0x04)) {

			pr_debug("%s: SND_SOC_DAPM_PRE_PMU OCP detected, ENABLE_OCP_FSM\n", __func__);

			snd_soc_update_bits(codec, WCD9XXX_A_RX_HPH_OCP_CTL, 0x10, 0x00);
			snd_soc_update_bits(codec, WCD9XXX_A_RX_HPH_OCP_CTL, 0x10, 0x10);
			msleep(30);
		}
		/* T-FAUDIO-161054QQ1 end */
		if (w->shift == 5)
	...
	case SND_SOC_DAPM_POST_PMD:
		/* T-FAUDIO-161054QQ1 start */
		/*
		* After PA down, check whether OCP has been detected.
		* If yes, to set OCP_FSM_EN again. If not, to reset OCP_FSM_EN.
		*/
		if ((snd_soc_read(codec, TOMTOM_A_RX_HPH_L_STATUS) & 0x04) ||
			(snd_soc_read(codec, TOMTOM_A_RX_HPH_R_STATUS) & 0x04)) {

			pr_debug("%s: SND_SOC_DAPM_POST_PMD OCP detected, ENABLE_OCP_FSM\n", __func__);

			snd_soc_update_bits(codec, WCD9XXX_A_RX_HPH_OCP_CTL, 0x10, 0x00);
			snd_soc_update_bits(codec, WCD9XXX_A_RX_HPH_OCP_CTL, 0x10, 0x10);
		} else {
			pr_debug("%s: SND_SOC_DAPM_POST_PMD OCP detected, DISABLE_OCP_FSM\n", __func__);
...
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
static int tomtom_vol_mute_recv_thread(void *data)
{
	struct snd_soc_codec *codec = (struct snd_soc_codec *)data;
	u8 value = 0xCE;

	vol_mute_thread_flag_recv = true;

	/* min volume set */
	snd_soc_update_bits(codec, TOMTOM_A_CDC_RX1_VOL_CTL_B2_CTL, 0xFF, value);

	// sleep 300ms in mute
	msleep(300);
	if (kthread_should_stop()) {
		pr_info("%s thread stop ", __func__);
		vol_mute_thread_flag_recv = false;
		return 0;
	}
	// mute off
	rx_force_allmute_set(codec, false);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	struct tomtom_priv *tomtom_p = snd_soc_codec_get_drvdata(codec);
/* H-FAUDIO-000026000 start */
	struct timeval disable_recv_tv;
/* H-FAUDIO-000026000 end */
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
	int ret;
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 end */
...
/* N-FAUDIO-151061000  start */
/* N-FAUDIO-161061034 start */
//		pr_info("%s: switched to receiver\n",  __func__);
//		gpio_set_value(tomtom_p->gpio_aud_asw, 1); // H:RCV
/* N-FAUDIO-161061034 end */
/* N-FAUDIO-151061000  end */
		wcd9xxx_clsh_fsm(codec, &tomtom_p->clsh_d,
...
/* FUJITSU LIMITED:2014-08-14 N-FAUDIO-151026000 start */
		if(lastcall_pl_flg) {
			if(vol_mute_thread_flag_recv) {
				if (tomtom_vol_mute_task_recv!=NULL) {
					pr_err("%s tomtom_vol_mute_task_recv working, stop thread.", __func__);
					kthread_stop(tomtom_vol_mute_task_recv);
					tomtom_vol_mute_task_recv = NULL;
				}
			}
			pr_info("%s kthread run tomtom_vol_mute_recv_thread\n", __func__);
			tomtom_vol_mute_task_recv = kthread_run(tomtom_vol_mute_recv_thread, codec, "tomtom_vol_stepup_recv");
			if (IS_ERR(tomtom_vol_mute_task_recv)) {
				ret = PTR_ERR(tomtom_vol_mute_task_recv);
				pr_err("%s tomtom_vol_mute_recv_thread run failed ret = %d\n", __func__, ret);
				// mute off
				rx_force_allmute_set(codec, false);
			}
		}
...
/* H-FAUDIO-000026000 start */
	case SND_SOC_DAPM_POST_PMD:
		do_gettimeofday(&disable_recv_tv);
		disable_recv_ms = (disable_recv_tv.tv_sec * 1000) + (disable_recv_tv.tv_usec / 1000);
		break;
/* H-FAUDIO-000026000 end */
	}
	return 0;
...
/* N-FAUDIO-151026006 start */
//			tomtom_codec_enable_ear_pa, SND_SOC_DAPM_POST_PMU),
			tomtom_codec_enable_ear_pa, SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
/* N-FAUDIO-151026006 end */
...
/* H-FAUDIO-000059000 start */
//	snd_soc_add_codec_controls(codec, impedance_detect_controls,
//				   ARRAY_SIZE(impedance_detect_controls));
/* H-FAUDIO-000059000 end */
...
/* N-FAUDIO-151061000  start */
/* N-FAUDIO-161061034 start */
//	tomtom->gpio_aud_asw = of_get_named_gpio(codec->dev->parent->of_node, "qcom,gpio-aud-asw", 0);
//	if (tomtom->gpio_aud_asw < 0) {
//		pr_err("%s: no gpio-aud-asw entry%d\n", __func__, tomtom->gpio_aud_asw);
//		goto err_pdata;
//	}
/* N-FAUDIO-161061034 end */
/* N-FAUDIO-151061000  end */
...
/* H-FAUDIO-000056000 start */
	codec_micbias2external = codec;
/* H-FAUDIO-000056000 end */
/* N-FAUDIO-151056006 start */
	snd_soc_update_bits(codec, TOMTOM_A_MBHC_INSERT_DETECT2, 0xFF, 0x00);
/* N-FAUDIO-151056006 end */
/* N-FAUDIO-151061014 start */
    snd_soc_update_bits(codec, TOMTOM_A_CDC_BOOST_MODE_CTL, 0x03, 0x01);
/* N-FAUDIO-151061014 end */
	return ret;
