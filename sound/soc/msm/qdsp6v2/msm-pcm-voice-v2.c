...
static int hce_auditory_info(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_info *uinfo) {
...
static int hce_auditory_control_voice_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
#ifdef GOOGLE_CHANGE_80779_MERGED
	return voc_get_hce_auditory_control(ucontrol->value.bytes.data);
#else
	return voc_get_hce_auditory_control(ucontrol->value.integer.value);
#endif
}
static int hce_auditory_control_voice_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
#ifdef GOOGLE_CHANGE_80779_MERGED
	return voc_set_hce_auditory_control(ucontrol->value.bytes.data,voc_get_session_id(VOICE_SESSION_NAME));
#else
	return voc_set_hce_auditory_control(ucontrol->value.integer.value,voc_get_session_id(VOICE_SESSION_NAME));
#endif
}
/* FUJITSU LIMITED:2014-09-30 N-FAUDIO-151038000 start */
static int hce_auditory_control_volte_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
#ifdef GOOGLE_CHANGE_80779_MERGED
	return voc_get_hce_auditory_control(ucontrol->value.bytes.data);
#else
...
static int hce_auditory_control_volte_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
#ifdef GOOGLE_CHANGE_80779_MERGED
	return voc_set_hce_auditory_control(ucontrol->value.bytes.data,voc_get_session_id(VOLTE_SESSION_NAME));
#else
	return voc_set_hce_auditory_control(ucontrol->value.integer.value,voc_get_session_id(VOLTE_SESSION_NAME));
#endif
}
/* FUJITSU LIMITED:2014-09-30 N-FAUDIO-151038000 end */
static int hce_auditory_control_voip_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
#ifdef GOOGLE_CHANGE_80779_MERGED
	return voc_get_hce_auditory_control(ucontrol->value.bytes.data);
#else
...
static int hce_auditory_control_voip_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
#ifdef GOOGLE_CHANGE_80779_MERGED
	return voc_set_hce_auditory_control(ucontrol->value.bytes.data,voc_get_session_id(VOIP_SESSION_NAME));
#else
	return voc_set_hce_auditory_control(ucontrol->value.integer.value,voc_get_session_id(VOIP_SESSION_NAME));
#endif
}
static int hce_auditory_conf_info(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_info *uinfo) {
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
	return 0;
}
static int hce_auditory_conf_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
	return voc_get_hce_auditory_conf(ucontrol->value.integer.value);
}
static int hce_auditory_conf_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol) {
	return voc_set_hce_auditory_conf(ucontrol->value.integer.value);
}
/* FUJITSU LIMITED:2014-09-30 N-FAUDIO-151038000 start */
#define HCE_AUDITORY_CONTROLS \
	{\
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE, \
		.name = "HCE Auditory Control Voice", \
		.info = hce_auditory_info, \
		.get = hce_auditory_control_voice_get, \
		.put = hce_auditory_control_voice_put, \
	},\
	{\
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE, \
		.name = "HCE Auditory Control VoLTE", \
		.info = hce_auditory_info, \
		.get = hce_auditory_control_volte_get, \
		.put = hce_auditory_control_volte_put, \
	},\
	{\
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE, \
		.name = "HCE Auditory Control VoIP", \
		.info = hce_auditory_info, \
		.get = hce_auditory_control_voip_get, \
		.put = hce_auditory_control_voip_put, \
	},\
	{\
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE, \
		.name = "HCE Auditory Conf", \
		.info = hce_auditory_conf_info, \
		.get = hce_auditory_conf_get, \
		.put = hce_auditory_conf_put, \
	}
/* FUJITSU LIMITED:2014-09-30 N-FAUDIO-151038000 end */
