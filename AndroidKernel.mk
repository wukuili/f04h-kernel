...
$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT)
	...
	$(hide) if [ "$(TARGET_BUILD_VARIANT)" != "user" ] || [ "$(TARGET_BUILD_TYPE)" != "release" ]; then \
			echo "Build kernel for FREE_PAGES_RDONLY support..."; \
			echo "CONFIG_FREE_PAGES_RDONLY=y" >> $(KERNEL_OUT)/.config; \
			$(MAKE) -C kernel O=../$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) oldconfig; fi

	$(hide) if [ "$(TARGET_BUILD_VARIANT)" != "user" ]; then \
			echo "Build kernel for non-LSM..."; \
			echo "# CONFIG_LSM_ON is not set" >> $(KERNEL_CONFIG); \
			$(MAKE) -C kernel O=../$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) oldconfig; fi
