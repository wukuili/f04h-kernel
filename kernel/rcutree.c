				...
				bool user)
{
	...
	smp_mb__before_clear_bit();  /* See above. */
	atomic_inc(&rdtp->dynticks);
	smp_mb__after_clear_bit();  /* Force ordering with next sojourn. */
...
static void rcu_eqs_exit_common(struct rcu_dynticks *rdtp, long long oldval,
			       int user)
{
	smp_mb__before_clear_bit();  /* Force ordering w/previous sojourn. */
	atomic_inc(&rdtp->dynticks);
	/* CPUs seeing atomic_inc() must see later RCU read-side crit sects */
	smp_mb__after_clear_bit();  /* See above. */
	WARN_ON_ONCE(!(atomic_read(&rdtp->dynticks) & 0x1));
...
void rcu_nmi_enter(void)
{
	...
	smp_mb__before_clear_bit();  /* Force delay from prior write. */
	atomic_inc(&rdtp->dynticks);
	/* CPUs seeing atomic_inc() must see later RCU read-side crit sects */
	smp_mb__after_clear_bit();  /* See above. */
	WARN_ON_ONCE(!(atomic_read(&rdtp->dynticks) & 0x1));
...
void rcu_nmi_exit(void)
{
	...
	smp_mb__before_clear_bit();  /* See above. */
	atomic_inc(&rdtp->dynticks);
	smp_mb__after_clear_bit();  /* Force delay to next write. */
	WARN_ON_ONCE(atomic_read(&rdtp->dynticks) & 0x1);
}
