...
#define FTA_ADDITIONAL_INFOLEN 12
//#define LOG_LINE_MAX		(1024 - PREFIX_MAX)
#define LOG_LINE_MAX		(1024 - PREFIX_MAX + FTA_ADDITIONAL_INFOLEN)
...
/* FUJITSU LIMITED:2014-10-14 H1510004 add start */
#ifdef FJ_PRODUCT_RELEASE
/* FUJITSU LIMITED:2014-10-14 H1510004 add end */
...
//		return snprintf(NULL, 0, "%5d %5d [%5lu.000000] ",
//			        current ? current->tgid : -1, current ? current->pid : -1, (unsigned long)ts);
		return snprintf(NULL, 0, "[%5lu.000000] %5d %5d ",
			        (unsigned long)ts, current ? current->tgid : -1, current ? current->pid : -1);
	}

//	return sprintf(buf, "[%5lu.%06lu] ",
//		       (unsigned long)ts, rem_nsec / 1000);
//	return sprintf(buf, "%5d %5d [%5lu.%06lu] ",
//		       current ? current->tgid : -1, current ? current->pid : -1, (unsigned long)ts, rem_nsec / 1000);
	return sprintf(buf, "[%5lu.%06lu] %5d %5d ",
		       (unsigned long)ts, rem_nsec / 1000, current ? current->tgid : -1, current ? current->pid : -1);
}
...
int syslog_print(char __user *buf, int size)
{
	...
	/* FUJITSU LIMITED:2015-03-19 H1510202 mod start */
	if (oops_buf_len < 0) {
		kfree(text);
		return oops_buf_len;
	}
	/* FUJITSU LIMITED:2015-03-19 H1510202 mod end */
...
static void call_console_drivers(int level, const char *text, size_t len)
{
	...
	for_each_console(con) {
		...
		if (!(con->flags & CON_ENABLED))
			continue;
		// if (!con->write)
		if (!con->write && !con->writex)
			continue;
		if (!cpu_online(smp_processor_id()) &&
		    !(con->flags & CON_ANYTIME))
			continue;
        if (con->writex) {
                con->writex(con, level, text, len);
        } else {
