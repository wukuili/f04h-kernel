...
static DEFINE_MUTEX(sched_hmp_param_mutex);

#define FLAG_INIT_TASK_LOAD	(1 << 0)
#define FLAG_SMALL_TASK		(1 << 1)
#define FLAG_UPMIGRATE		(1 << 2)
#define FLAG_DOWNMIGRATE	(1 << 3)
#define FLAG_UPMIGRATE_MIN_NICE	(1 << 4)
int sched_set_hmp_params(
			 ...
			 unsigned int flag)
{
	...
	pr_debug("%s(init_task_load:%u, small_task:%u, upmigrate:%u, downmigrate:%u, upmigrate_min_nice:%d, flag:%u)\n",
		__FUNCTION__, init_task_load, small_task, upmigrate, downmigrate, upmigrate_min_nice, flag);

	mutex_lock(&sched_hmp_param_mutex);

	pr_debug("  now param init_task_load:%u, small_task:%u, upmigrate:%u, downmigrate:%u, upmigrate_min_nice:%d\n",
		sysctl_sched_init_task_load_pct, sysctl_sched_small_task_pct, sysctl_sched_upmigrate_pct, sysctl_sched_downmigrate_pct, sysctl_sched_upmigrate_min_nice);

	/* valid check (see the write process of sched_hmp_proc_update_handler) */
	if (!(flag & FLAG_UPMIGRATE))          upmigrate          = sysctl_sched_upmigrate_pct;
	if (!(flag & FLAG_DOWNMIGRATE))        downmigrate        = sysctl_sched_downmigrate_pct;
	if ((downmigrate > upmigrate) ||
	    ((flag & FLAG_INIT_TASK_LOAD)     && init_task_load     > 100) ||
	    ((flag & FLAG_SMALL_TASK)         && small_task         > 100) ||
	    ((flag & FLAG_UPMIGRATE)          && upmigrate          > 100) ||
	    ((flag & FLAG_DOWNMIGRATE)        && downmigrate        > 100)) {
		/* invalid value */
		mutex_unlock(&sched_hmp_param_mutex);
		pr_err("%s() failed %d\n", __FUNCTION__, -EINVAL);
		return -EINVAL;
	}

	/* update check (see the write process of sched_hmp_proc_update_handler) */
	reclassify = (((flag & FLAG_UPMIGRATE) && sysctl_sched_upmigrate_pct != upmigrate) ||
		((flag & FLAG_SMALL_TASK) && sysctl_sched_small_task_pct != small_task));

	if (flag & FLAG_INIT_TASK_LOAD)     sysctl_sched_init_task_load_pct   = init_task_load    ;
	if (flag & FLAG_SMALL_TASK)         sysctl_sched_small_task_pct       = small_task        ;
	if (flag & FLAG_UPMIGRATE)          sysctl_sched_upmigrate_pct        = upmigrate         ;
	if (flag & FLAG_DOWNMIGRATE)        sysctl_sched_downmigrate_pct      = downmigrate       ;
	if (flag & FLAG_UPMIGRATE_MIN_NICE) sysctl_sched_upmigrate_min_nice   = upmigrate_min_nice;
	pr_debug("%s() update init_task_load:%u, small_task:%u, upmigrate:%u, downmigrate:%u, upmigrate_min_nice:%d\n",
		__FUNCTION__, sysctl_sched_init_task_load_pct, sysctl_sched_small_task_pct, sysctl_sched_upmigrate_pct, sysctl_sched_downmigrate_pct, sysctl_sched_upmigrate_min_nice);
...
#define FLAG_MOSTLY_IDLE_LOAD	(1 << 0)
#define FLAG_MOSTLY_IDLE_FREQ	(1 << 1)
#define FLAG_MOSTLY_IDLE_NR_RUN	(1 << 2)
#define FLAG_PREFER_IDLE	(1 << 3)
int sched_set_cpu_params(
                        cpumask_t cpu_mask,
                        ...
                        unsigned int flag)
{
	...
	pr_debug("%s(mostly_idle_load:%u, mostly_idle_freq:%u, mostly_idle_nr_run:%u, prefer_idle:%u flag:%u)\n",
		__FUNCTION__, mostly_idle_load, mostly_idle_freq, mostly_idle_nr_run, prefer_idle, flag);

	mutex_lock(&sched_hmp_param_mutex);
        for_each_cpu(cpu, &cpu_mask) {
	        if (err == 0 && flag & FLAG_MOSTLY_IDLE_LOAD)
			err = sched_set_cpu_mostly_idle_load(cpu, mostly_idle_load);
        	if (err == 0 && flag & FLAG_MOSTLY_IDLE_FREQ)
			err = sched_set_cpu_mostly_idle_freq(cpu, mostly_idle_freq);
        	if (err == 0 && flag & FLAG_MOSTLY_IDLE_NR_RUN)
			err = sched_set_cpu_mostly_idle_nr_run(cpu, mostly_idle_nr_run);
        	if (err == 0 && flag & FLAG_PREFER_IDLE)
			err = sched_set_cpu_prefer_idle(cpu, prefer_idle);
        }
	mutex_unlock(&sched_hmp_param_mutex);
		...
		loff_t *ppos)
{
	...
	{ /* FUJITSU:2014-10-24 POWERSAVE Add */
		mutex_unlock(&sched_hmp_param_mutex); /* FUJITSU:2014-10-24 POWERSAVE Add */
		goto done;
	} /* FUJITSU:2014-10-24 POWERSAVE Add */
