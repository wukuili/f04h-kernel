...
/*
 * FCNT LIMITED adds below source code in this that Linus Torvalds
 * holds original Copyright since 1991.
 */
/* FCNT LIMITED:2016-03-23 H16100761 add start */
#ifdef	DONT_USE_THREAD_INFO_MEMPOOL
/* FCNT LIMITED:2016-03-23 H16100761 add end */
...
static void *mempool_alloc_pages_noswap(gfp_t gfp_mask, void *pool_data)
{
	...
	ret = alloc_pages(gfp_mask, order);
	if (!ret && thread_info_pool)
		pr_debug("thread_info mempool(%d/%d)\n", thread_info_pool->curr_nr, thread_info_pool->min_nr);
	return ret;
}
...
static int copy_signal(unsigned long clone_flags, struct task_struct *tsk)
{
	...
	if (strstr(tsk->comm, "main") && (sig->oom_score_adj != -1000))
		printk(KERN_ERR "%s: oom_score_adj fail '%s' (%d), oom_score_adj to %hd\n",
			__func__, tsk->comm, tsk->pid, sig->oom_score_adj);
