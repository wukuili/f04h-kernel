...
struct mmc_host_ops {
	...
	int	(*get_host_type)(struct mmc_host *host); /* FUJITSU LIMITED:2015-01-23 H1510175 add */
...
#define MMC_BUSRESUME_NEEDS_RESUME	(1 << 1)
	...
	struct scatterlist	*cur_sg_pool;
	struct scatterlist	*pre_sg_pool;
...
void mmc_detect_work_sync(struct mmc_host *);	/* FUJITSU LIMITED:2016-01-19 H16100337 add */

extern void mmc_down_speed_mode(struct mmc_host *host);	/* FUJITSU LIMITED:2015-12-03 H1610017 add */
...
#define HOST_TYPE_MMC		(1 << 0)		/* MMC  */
#define HOST_TYPE_SD		(1 << 1)		/* SD   */
#define HOST_TYPE_SDIO		(1 << 2)		/* SDIO */
static inline int mmc_host_get_type(struct mmc_host *host)
{
	int ret = 0;

	if (!host->ops->get_host_type)
		pr_err(" %pF -> %s() [%s] null access\n", __builtin_return_address(0), __func__, mmc_hostname(host));
	else
...
int pre_mmc_alloc_sg(struct mmc_host *mmc); /* FUJITSU LIMITED:2014-11-26 H1510114 add */
