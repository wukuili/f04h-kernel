...
#define __FJSEC_TZ_CIPHER__

#ifdef __FJSEC_TZ_CIPHER__
#define N_GROUP_MAX 33 /* 1 + NGROUPS_SMALL @cred.h */
...
#ifdef __FJSEC_TZ_CIPHER__
/*
 * qseecom_client_authentication_req
 * @ clnt_id : client ID
 * @ res     : authentication result
 */
struct qseecom_client_authentication_req {
	int clnt_id;
	int *res;
};
/*
 * qseecom_client_info
 * @ u_id   : user ID
 * @ ng_ids : number of groups
 * @ g_ids  : array of all group ids
 * @ hash   : hash value
 */
struct qseecom_client_info{
	int u_id;
	int ng_ids;
	int g_ids[N_GROUP_MAX];
	int hash;
};
#endif /*__FJSEC_TZ_CIPHER__*/
...
#ifdef __FJSEC_TZ_CIPHER__
#define QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ	\
    _IOWR(QSEECOM_IOC_MAGIC, 50, struct qseecom_client_authentication_req)
#endif /* __FJSEC_TZ_CIPHER__ */
