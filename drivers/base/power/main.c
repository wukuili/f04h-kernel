...
/*		dev_name(dev), pm_verb(state.event), info, error); */
	printk(KERN_WARNING "PM: Device %s failed to %s%s: error %d\n",
		dev_name(dev), pm_verb(state.event), info, error);
...
#ifdef CALLBACK_TIME_WARNING_REPORT
	unsigned long stamp;
#endif /* CALLBACK_TIME_WARNING_REPORT */
...
/* FUJITSU LIMITED:2015-11-17 H1610003 add start */
#ifdef CALLBACK_TIME_WARNING_REPORT
	stamp = jiffies + WARNING_REPORT_TIME;
#endif /* CALLBACK_TIME_WARNING_REPORT */
/* FUJITSU LIMITED:2015-11-17 H1610003 add end */
...
/* FUJITSU LIMITED:2015-11-17 H1610003 add start */
	...
	if (time_after(jiffies, stamp)) {
		printk(KERN_WARNING "%s %s%s returned %d after %dms\n",
			   dev_name(dev), info, pm_verb(state.event), error,
			   jiffies_to_msecs(jiffies - stamp + WARNING_REPORT_TIME));
	}
#endif /* CALLBACK_TIME_WARNING_REPORT */
/* FUJITSU LIMITED:2015-11-17 H1610003 add end */
...
#ifdef CALLBACK_TIME_WARNING_REPORT
	if (time_after(jiffies, stamp)) {
		printk(KERN_WARNING "%s %s returned %d after %dms\n",
...
int dpm_suspend(pm_message_t state)
{
	...
	mutex_unlock(&dpm_list_mtx);
	async_synchronize_full();
/* FUJITSU LIMITED:2015-11-17 H1610003 mod start */
#if 0
	if (!error)
		error = async_error;
#endif
	if (!error) {
		error = async_error;
		if (error) {
			printk(KERN_ERR "PM: Device failed to suspend async:%d\n", error);
		}
	}
/* FUJITSU LIMITED:2015-11-17 H1610003 mod end */
	if (error) {
...
/*		printk(KERN_ERR "%s(): %pF returns %d\n", function, fn, ret); */
		printk(KERN_WARNING "%s(): %pF returns %d\n", function, fn, ret);
/* FUJITSU LIMITED:2014-10-30 H1510014 mod end */
}
