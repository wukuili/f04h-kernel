...
#ifdef CONFIG_SECURITY_FJSEC
#if 0
fjsec_page_acl_pre_update(cma_areas[i].name, __phys_to_pfn(cma_areas[i].base), cma_areas[i].size);
#else
fjsec_page_acl_pre_update(cma_areas[i].name, __phys_to_pfn((cma_areas[i].base - PHYS_OFFSET)), cma_areas[i].size);
#endif
#endif
/* FUJITSU:2015-03-07 GEN-141-SCRU-ALT ADD E */
	}
...
#if 0
fjsec_page_acl_pre_update("default region", __phys_to_pfn(base), sel_size);
#else
fjsec_page_acl_pre_update("default region", __phys_to_pfn((base - PHYS_OFFSET)), sel_size);
#endif
