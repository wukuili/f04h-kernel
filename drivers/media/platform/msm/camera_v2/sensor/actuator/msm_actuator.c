...
#define CAMERA_LOG_TAG "msm_actuator: "
#define CAMERA_NV_INDEX 2
...
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add start */
	a_ctrl->before_dac_correct_value = 0;
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add end */
	/*
...
// FUJITSU:Internal-ISP add start
static int32_t msm_actuator_i2c_write(struct msm_actuator_ctrl_t *a_ctrl,
	...
	struct reg_settings_t *settings)
{
	...
	int32_t i = 0;
	CDBG("Enter\n");
    LOGD("%s:%d size[%d] type[%d]\n", __func__, __LINE__, size, type);
	for (i = 0; i < size; i++) {
		switch (type) {
		case MSM_ACTUATOR_BYTE_DATA:
            LOGD("%s:%d reg_addr[%d]=[0x%04x] reg_data[%d]=[0x%04x]\n", __func__, __LINE__, i, settings[i].reg_addr, i, settings[i].reg_data);
			rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_write(
				&a_ctrl->i2c_client,
				settings[i].reg_addr,
...
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add end */
// FUJITSU:Internal-ISP add start
    int16_t dac_correct_value = move_params->dac_correct_value;
    LOGD("%s:%d dac_correct_value[%d]\n", __func__, __LINE__, dac_correct_value);
// FUJITSU:Internal-ISP add end
...
/* FCNT LIMITED:2016-03-10 [AF]:step0_move mod start */
#if 0
#if 1  //FUJITSU:Internal-ISP ADD-S
	if (dest_step_pos == a_ctrl->curr_step_pos)
  	  LOGD("curr_step_pos %d is stable, num_steps %d\n", dest_step_pos, num_steps);
#else
	if (dest_step_pos == a_ctrl->curr_step_pos)
		return rc;
#endif //FUJITSU:Internal-ISP ADD-E
#endif
	if ((dest_step_pos == a_ctrl->curr_step_pos) ||
	    (a_ctrl->step_position_table[a_ctrl->curr_step_pos] == a_ctrl->step_position_table[dest_step_pos])) {
		if (a_ctrl->before_dac_correct_value != dac_correct_value) {
			CDBG("forcibly set on : dest=%d(correct=%d) curr=%d(correct=%d)\n",
					dest_step_pos, a_ctrl->before_dac_correct_value,
					a_ctrl->curr_step_pos, dac_correct_value);
			forcibly = true;
		} else {
			target_lens_pos = a_ctrl->step_position_table[dest_step_pos] - dac_correct_value;
			if (target_lens_pos < a_ctrl->limitDac) {
				target_lens_pos = a_ctrl->limitDac;
			}
			if(move_params->curr_lens_pos_ptr){
				rc = copy_to_user(move_params->curr_lens_pos_ptr, &(target_lens_pos), sizeof(uint16_t) * 1);
			}
...
/* FCNT LIMITED:2016-03-10 [AF]:step0_move mod end */
...
// FUJITSU:Internal-ISP add start
    LOGD("%s:%d curr_step_pos[%d] dest_step_pos[%d] curr_lens_pos[%d]\n", __func__, __LINE__, a_ctrl->curr_step_pos, dest_step_pos, curr_lens_pos);
// FUJITSU:Internal-ISP add end
/* FCNT LIMITED:2016-03-10 [AF]:step0_move mod start */
#if 0
	while (a_ctrl->curr_step_pos != dest_step_pos) {
#endif
	while (a_ctrl->curr_step_pos != dest_step_pos || forcibly) {
/* FCNT LIMITED:2016-03-10 [AF]:step0_move mod end */
		step_boundary =
...
// FUJITSU:Internal-ISP add start
            LOGD("%s:%d target_lens_pos[%d]\n", __func__, __LINE__, target_lens_pos);
// FUJITSU:Internal-ISP add end
// FUJITSU:Internal-ISP chg start
            // correct target lens pos (dac) for Af Posture
            target_lens_pos -= dac_correct_value;
// FUJITSU:Internal-ISP add start
            LOGD("%s:%d target_lens_pos-dac_correct_value[%d]\n", __func__, __LINE__, target_lens_pos);
            LOGD("%s:%d limitDac[%d]\n", __func__, __LINE__, a_ctrl->limitDac);
            if(target_lens_pos < a_ctrl->limitDac){
              target_lens_pos = a_ctrl->limitDac;
            }
// FUJITSU:Internal-ISP add end
            LOGD("%s:%d target_lens_pos[%d]\n", __func__, __LINE__, target_lens_pos);
// FUJITSU:Internal-ISP chg end
...
// FUJITSU:Internal-ISP chg start
#if 1
            // take no thought step_boundary process.
            target_step_pos = dest_step_pos;
#else
...
// FUJITSU:Internal-ISP add start
    LOGD("%s:%d curr_lens_pos[%d]\n", __func__, __LINE__, curr_lens_pos);
// FUJITSU:Internal-ISP add end
// FUJITSU:Internal-ISP chg start
#if 1
    if(move_params->curr_lens_pos_ptr){
        rc = copy_to_user(move_params->curr_lens_pos_ptr, &(curr_lens_pos), sizeof(uint16_t) * 1);
// FUJITSU:Internal-ISP add start
        LOGD("%s:%d curr_lens_pos_ptr[%d]\n", __func__, __LINE__, *(move_params->curr_lens_pos_ptr));
// FUJITSU:Internal-ISP add end
    }else{
        pr_err("%s:%d curr_lens_pos_ptr is Null\n", __func__, __LINE__);
    }
#else
	rc = copy_to_user(move_params->curr_lens_pos_ptr, &(curr_lens_pos), sizeof(uint16_t) * 1);
#endif
...
// FUJITSU:Internal-ISP add start
    LOGD("%s:%d curr_lens_pos[%d]\n", __func__, __LINE__, move_params->curr_lens_pos);
// FUJITSU:Internal-ISP add end
	reg_setting.reg_setting = a_ctrl->i2c_reg_tbl;
	reg_setting.data_type = a_ctrl->i2c_data_type;
	reg_setting.size = a_ctrl->i2c_tbl_index;
...
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add start */
	a_ctrl->before_dac_correct_value = dac_correct_value;
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add end */
	a_ctrl->i2c_tbl_index = 0;
	CDBG("Exit\n");
	...
	struct msm_actuator_set_info_t *set_info)
{
	...
	int16_t cur_code = 0;
/* FUJITSU:Internal-ISP add start */
    int16_t code_per_step_dec = 0;
    int32_t cur_code_dec = 0;
/* FUJITSU:Internal-ISP add end */
	uint16_t step_index = 0, region_index = 0;
	uint16_t step_boundary = 0;
	uint32_t max_code_size = 1;
	...
	CDBG("Enter\n");
/* FUJITSU:Internal-ISP add start */
    a_ctrl->limitDac = set_info->af_tuning_params.limitDac;
	LOGD("%s:%d a_ctrl->limitDac = %d:\n", __func__, __LINE__, a_ctrl->limitDac);
/* FUJITSU:Internal-ISP add end */
...
/* FUJITSU:Internal-ISP add start */
        code_per_step_dec =
            a_ctrl->region_params[region_index].code_per_step_dec;
/* FUJITSU:Internal-ISP add end */
		qvalue =
		...
		for (; step_index <= step_boundary;
			...
			cur_code += set_info->af_tuning_params.initial_code;
/* FUJITSU:Internal-ISP add start */
                        cur_code_dec += code_per_step_dec;
/* FUJITSU:Internal-ISP add end */
			if (cur_code < max_code_size)
/* FUJITSU:Internal-ISP chg start */
//                               a_ctrl->step_position_table[step_index] =
...
// FUJITSU:Internal-ISP chg start
	LOGD("%s:%d Generated DAC Table:\n", __func__, __LINE__);
	for (step_index=0; step_index < set_info->af_tuning_params.total_steps + 1; step_index++){
		LOGD("step[%d]:%03d ", step_index, a_ctrl->step_position_table[step_index]);
	}
// FUJITSU:Internal-ISP chg end
	CDBG("Exit\n");
	return 0;
...
// FUJITSU:Internal-ISP add start
static int32_t msm_actuator_set_af_default_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_move_params_t *move_params)
{
	int32_t rc = 0;
	LOGD("%s:%d Enter\n", __func__, __LINE__);
    LOGD("%s:%d ->actuator_move_focus\n", __func__, __LINE__);
	rc = a_ctrl->func_tbl->actuator_move_focus(a_ctrl, move_params);
    LOGD("%s:%d <-actuator_move_focus rc[%d]\n", __func__, __LINE__, rc);
    pr_err("%s:%d <-actuator_move_focus rc[%d]\n", __func__, __LINE__, rc);
	LOGD("%s:%d Exit rc[%d]\n", __func__, __LINE__, rc);
	return rc;
}

static int32_t msm_actuator_get_dac_table(
	struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_get_info_t *get_info)
{
	...
	LOGD("%s:%d Enter\n", __func__, __LINE__);
	if(!a_ctrl || !get_info){
        pr_err("%s:%d Invalid param %p or %p \n", __func__, __LINE__, a_ctrl, get_info);
        return -EINVAL;
    }
    LOGD("%s:%d total_step[%d] a_ctrl->total_steps[%d]\n", __func__, __LINE__, get_info->total_step, a_ctrl->total_steps);
    pr_err("%s:%d total_step[%d] a_ctrl->total_steps[%d]\n", __func__, __LINE__, get_info->total_step, a_ctrl->total_steps);

    rc = copy_to_user(get_info->step_position_table, a_ctrl->step_position_table, sizeof(uint16_t)*(get_info->total_step));
    if(rc != 0){
        pr_err("%s:%d step_position_table copy_to_user Failed rc[%d]\n", __func__, __LINE__, rc);
        return rc;
    }
...
static int32_t msm_actuator_set_register(
	struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_set_info_t *set_info)
{
	int32_t rc = 0;
	struct reg_settings_t *reg_settings = NULL;
	
	LOGD("%s:%d Enter\n", __func__, __LINE__);
	if(!a_ctrl || !set_info){
        pr_err("%s:%d Invalid param %p or %p \n", __func__, __LINE__, a_ctrl, set_info);
        return -EINVAL;
    }
    LOGD("%s:%d reg_setting_size[%d]\n", __func__, __LINE__, set_info->reg_settings.reg_setting_size);
    pr_err("%s:%d reg_setting_size[%d]\n", __func__, __LINE__, set_info->reg_settings.reg_setting_size);
    if(set_info->reg_settings.reg_setting_size){
        reg_settings = kmalloc(sizeof(struct reg_settings_t)*(set_info->reg_settings.reg_setting_size),GFP_KERNEL);
        if (reg_settings == NULL) {
			pr_err("Error allocating memory for reg_settings\n");
			return -EFAULT;
		}
		if (copy_from_user(reg_settings,
			(void *)set_info->reg_settings.reg_setting,
			set_info->reg_settings.reg_setting_size *
			sizeof(struct reg_settings_t))) {
			kfree(reg_settings);
			pr_err("Error copying reg_settings\n");
			return -EFAULT;
		}
		LOGD("%s:%d reg_addr[0x%02x] reg_data[0x%02x]\n", __func__, __LINE__, reg_settings->reg_addr, reg_settings->reg_data);
		pr_err("%s:%d reg_addr[0x%02x] reg_data[0x%02x]\n", __func__, __LINE__, reg_settings->reg_addr, reg_settings->reg_data);
		...
		kfree(reg_settings);
		if (rc < 0) {
			pr_err("Error actuator_i2c_write\n");
			return -EFAULT;
		}
    }
...
// FUJITSU:Internal-ISP add start
            LOGI("%s:%d reg_name[%s] type[%d] min_voltage[%d] max_voltage[%d] op_mode[%d] delay[%d] config[%d]\n", __func__, __LINE__, 
                    vreg_cfg->cam_vreg[i].reg_name, vreg_cfg->cam_vreg[i].type,
                    vreg_cfg->cam_vreg[i].min_voltage, vreg_cfg->cam_vreg[i].max_voltage,
                    vreg_cfg->cam_vreg[i].op_mode, vreg_cfg->cam_vreg[i].delay, config);
// FUJITSU:Internal-ISP add end
...
// FUJITSU:Internal-ISP add start
    LOGD("%s:%d curr_step_pos[%d] curr_region_index[%d]\n", __func__, __LINE__, a_ctrl->curr_step_pos, a_ctrl->curr_region_index);
// FUJITSU:Internal-ISP add end
	CDBG("Exit\n");
...
// FUJITSU:Internal-ISP add start
    case CFG_SET_AF_DEFAULT_FOCUS:
        rc = a_ctrl->func_tbl->actuator_set_af_default_focus(a_ctrl,
			&cdata->cfg.move);
		if (rc < 0){
			pr_err("move macro focus failed %d\n", rc);
		}
        break;
    case CFG_GET_ACTUATOR_DAC_TABLE:
        rc = a_ctrl->func_tbl->actuator_get_dac_table(a_ctrl,
			&cdata->cfg.get_info);
		if (rc < 0){
			pr_err("%s:%d get dac table failed %d\n", __func__, __LINE__, rc);
		}
        break;
    case CFG_SET_ACTUATOR_REGISTER:
        rc = a_ctrl->func_tbl->actuator_set_register(a_ctrl,
			&cdata->cfg.set_info);
		if (rc < 0){
			pr_err("%s:%d set dac table failed %d\n", __func__, __LINE__, rc);
		}
        break;
// FUJITSU:Internal-ISP add end
	default:
...
// FUJITSU:Internal-ISP add start
			actuator_data.cfg.set_info.af_tuning_params
				.limitDac =
				u32->cfg.set_info.af_tuning_params.limitDac;
// FUJITSU:Internal-ISP add end
			...
			actuator_data.cfg.set_info.af_tuning_params
				...
				.region_params);
// FUJITSU:Internal-ISP add start
			actuator_data.cfg.set_info.reg_settings
				.reg_setting_size =
				u32->cfg.set_info.reg_settings.reg_setting_size;

			actuator_data.cfg.set_info.reg_settings
				.reg_setting = compat_ptr(
				u32->cfg.set_info.reg_settings
				.reg_setting);
// FUJITSU:Internal-ISP add end
...
// FUJITSU:Internal-ISP add start
			actuator_data.cfg.move.dac_correct_value =
				u32->cfg.move.dac_correct_value;
    
			actuator_data.cfg.move.curr_lens_pos_ptr =
				compat_ptr(u32->cfg.move.curr_lens_pos_ptr);
// FUJITSU:Internal-ISP add end
...
// FUJITSU:Internal-ISP add start
        case CFG_GET_ACTUATOR_DAC_TABLE:
            actuator_data.cfgtype = u32->cfgtype;
            actuator_data.cfg.get_info.total_step = u32->cfg.get_info.total_steps;
            actuator_data.cfg.get_info.step_position_table = compat_ptr(u32->cfg.get_info.step_position_table);
            parg = &actuator_data;
            break;
/* FCNT LIMITED:2016-03-02 [AF]:set cfg add start */
		case CFG_SET_ACTUATOR_REGISTER:
			/* To Do */            
            actuator_data.cfgtype = u32->cfgtype;
            actuator_data.cfg.set_info.reg_settings.reg_setting_size = u32->cfg.set_info.reg_settings.reg_setting_size;
		    actuator_data.cfg.set_info.reg_settings.reg_setting =  compat_ptr(u32->cfg.set_info.reg_settings.reg_setting);
			parg = &actuator_data;
			break;
/* FCNT LIMITED:2016-03-02 [AF]:set cfg add end */
// FUJITSU:Internal-ISP add end
		default:
			actuator_data.cfgtype = u32->cfgtype;
			parg = &actuator_data;
...
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add start */
	act_ctrl_t->before_dac_correct_value = 0;
/* FCNT LIMITED:2016-03-10 [AF]:step0_move add end */
...
#ifdef FJ_CAMERA_CUSTOM // start
	_camera_get_all_non_info(&cam_nv_info);
#endif // FJ_CAMERA_CUSTOM end
...
// FUJITSU:Internal-ISP add start
    LOGD("%s:%d qcom,cci-master %d, rc %d\n", __func__, __LINE__, msm_actuator_t->cci_master, rc);
// FUJITSU:Internal-ISP add end
...
// FUJITSU:Internal-ISP add start
    LOGD("%s:%d msm_sd.sd.name[%s]\n", __func__, __LINE__, msm_actuator_t->msm_sd.sd.name);
// FUJITSU:Internal-ISP add end
...
// FUJITSU:Internal-ISP chg end
// FUJITSU:Internal-ISP add start
        .actuator_set_af_default_focus = msm_actuator_set_af_default_focus,
        .actuator_get_dac_table = msm_actuator_get_dac_table,
        .actuator_set_register = msm_actuator_set_register,
