...
#define CAMERA_LOG_TAG "msm_sensor: "
#define CAMERA_NV_INDEX 0
...
#ifdef FJ_CAMERA_CUSTOM // start
    LOGI("%s:%d qcom,sensor-name %s, rc %d\n", __func__, __LINE__, sensordata->sensor_name, rc);
#endif // FJ_CAMERA_CUSTOM end
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
    LOGI("%s:%d qcom,cci-master %d, rc %d\n", __func__, __LINE__, s_ctrl->cci_i2c_master, rc);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
	if (rc < 0) {
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
    LOGI("%s:%d qcom,mount-angle %d\n", __func__, __LINE__, sensordata->sensor_info->sensor_mount_angle);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
    LOGI("%s:%d qcom,sensor-position %d\n", __func__, __LINE__, sensordata->sensor_info->position);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
    LOGI("%s:%d qcom,sensor-mode %d\n", __func__, __LINE__, sensordata->sensor_info->modes_supported);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
            LOGI("%s:%d gpio_array[%d] = %d\n", __func__, __LINE__, i, gpio_array[i]);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
		}
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
	LOGI("%s:%d slave addr 0x%x sensor reg 0x%x id 0x%x mask 0x%x\n",
		__func__, __LINE__,
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
    LOGI("%s:%d s_ctrl->init_sensor_state[%d] sensor_name[%s]\n", __func__, __LINE__, s_ctrl->init_sensor_state, s_ctrl->sensordata->sensor_name);
    if (s_ctrl->init_sensor_state != MSM_SENSOR_PROBE_END) {
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
static int msm_sensor_config32_fj(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
    ...
    case CFG_READ_I2C: {
        struct msm_camera_i2c_read32 conf_read_array32;
        struct msm_camera_i2c_read read_config;
        uint16_t local_data = 0;
        uint16_t orig_slave_addr = 0, read_slave_addr = 0;
    	memset(&conf_read_array32, 0, sizeof(struct msm_camera_i2c_read32));
    	memset(&read_config, 0, sizeof(struct msm_camera_i2c_read));
		if (copy_from_user(&conf_read_array32,
			(void *)compat_ptr(cdata->cfg.setting),
			sizeof(struct msm_camera_i2c_read32))) {
			LOGE("%s:%d failed\n", __func__, __LINE__);
			rc = -EFAULT;
			break;
		}
		read_slave_addr = conf_read_array32.slave_addr;
		read_config.data_type = conf_read_array32.data_type;
		read_config.reg_addr = conf_read_array32.reg_addr;
		read_config.data = compat_ptr(conf_read_array32.data);
		LOGI("%s:%d CFG_READ_I2C:", __func__, __LINE__);
		LOGI("%s:slave_addr=0x%x reg_addr=0x%x, data_type=%d\n",
		...
		LOGI("%s:orig_slave_addr=0x%x, new_slave_addr=0x%x",
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
		LOGI("%s:%d sensor name %s\n", __func__, __LINE__, cdata->cfg.sensor_info.sensor_name);
		LOGI("%s:%d session id %d\n", __func__, __LINE__, cdata->cfg.sensor_info.session_id);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
		for (i = 0; i < SUB_MODULE_MAX; i++) {
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
            LOGI("%s:%d subdev_id[%d] %d\n", __func__, __LINE__, i, cdata->cfg.sensor_info.subdev_id[i]);
			LOGI("%s:%d subdev_intf[%d] %d\n", __func__, __LINE__, i, cdata->cfg.sensor_info.subdev_intf[i]);
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
		}
...
#ifdef FJ_CAMERA_CUSTOM // start
        LOGI("%s:%d mount angle valid %d value %d\n", __func__, __LINE__, cdata->cfg.sensor_info.is_mount_angle_valid, cdata->cfg.sensor_info.sensor_mount_angle);
        cdata->cfg.sensor_info.camera_type = s_ctrl->sensordata->cam_slave_info->camera_id;
        LOGI("%s:%d camera_type %d\n", __func__, __LINE__, cdata->cfg.sensor_info.camera_type);
#endif // FJ_CAMERA_CUSTOM end
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
        LOGI("%s:%d init params mode %d pos %d mount %d\n", __func__, __LINE__,
            cdata->cfg.sensor_init_params.modes_supported,
            cdata->cfg.sensor_init_params.position,
            cdata->cfg.sensor_init_params.sensor_mount_angle);
#endif // FJ_CAMERA_CUSTOM end
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
        LOGI("%s:%d size[%d] addr_type[%d] delay[%d]\n", __func__, __LINE__, conf_array.size, conf_array.addr_type, conf_array.delay);
        for (i = 0; i < conf_array.size; i++) {
            LOGI("%s:%d reg_addr[%d]=[0x%02x] reg_data_size[%d]=[%d]\n", __func__, __LINE__, i, conf_array.reg_setting[i].reg_addr, i, conf_array.reg_setting[i].reg_data_size);
            for (j = 0; j < conf_array.reg_setting[i].reg_data_size; j++) {
                LOGI("%s:%d reg_data[%d]=[0x%02x]\n", __func__, __LINE__, j, conf_array.reg_setting[i].reg_data[j]);
            }
        }
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
...
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM start */
#ifdef FJ_CAMERA_CUSTOM // start
		rc = msm_sensor_config32_fj(s_ctrl, argp);
#else // FJ_CAMERA_CUSTOM
		rc = -EFAULT;
#endif // FJ_CAMERA_CUSTOM end
/* FUJITSU:2016-01-19 FJ_CAMERA_CUSTOM end */
