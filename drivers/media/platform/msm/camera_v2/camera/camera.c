...
#define CAMERA_NV_INDEX 5
#include "../sensor/fj_camera_log.h"
struct cam_nonvolatile_info cam_nv_info;
...
#ifdef FJ_CAMERA_CUSTOM // start
			if (rc == -ETIMEDOUT) {
				pr_debug("%s:%d MSM_CAMERA_NEW_SESSION timeout rc %d do retry post event\n", __func__, __LINE__, rc);
				rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
				if (rc < 0) {
