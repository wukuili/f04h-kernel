...
void clock_dump_info(void)
{
	...
	if (!mutex_trylock(&clk_list_lock)) {
		pr_err("clock_dump_info: Clocks are being registered. Cannot print clock state now.\n");
		return;
	}
	...
	list_for_each_entry(c, &clk_list, list) {
		if (c) {
			clock_debug_rate_get(c, &rate);
			clock_debug_enable_get(c, &enable);
			clock_debug_local_get(c, &is_local);
			clock_debug_hwcg_get(c, &has_hw_gating);
			printk(KERN_INFO "DCLK,%s,%lld,%lld,%lld,%lld,\n",
				c->dbg_name, enable, has_hw_gating, is_local, rate);
		} else {
