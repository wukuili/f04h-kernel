 ...
 ******************************************************************************/
/*----------------------------------------------------------------------------*/
// tuner_drv.c COPYRIGHT FUJITSU CONNECTED TECHNOLOGIES LIMITED 2015-2017
/*----------------------------------------------------------------------------*/
/*..+....1....+....2....+....3....+....4....+....5....+....6....+....7....+...*/
...
#ifdef CONFIG_COMPAT
static long tuner_module_entry_compat_ioctl( struct file *file,
		unsigned int uCommand,
		unsigned long uArgument );
#endif /* CONFIG_COMPAT */
...
//FUJITSU:2015-05-23 For Socionext tuner Add -S
#ifdef CONFIG_COMPAT
		.compat_ioctl = tuner_module_entry_compat_ioctl,
#endif /* CONFIG_COMPAT */
//FUJITSU:2015-05-23 For Socionext tuner Add -E
#endif /* LINUX_VERSION_CODE */
...
static int g_mmtuner_major = TUNER_CONFIG_DRV_MAJOR;
static struct cdev g_mmtunerDevs;
...
//FUJITSU:2015-05-18 For Socionext tuner Modify -S
	...
	if (register_chrdev(TUNER_CONFIG_DRV_MAJOR, TUNER_CONFIG_DRIVER_NAME,
			&TunerFileOperations)) {
		ERROR_PRINT("register_chrdev() Failed (Major:%d).",
...
static int __exit tuner_remove(struct platform_device *pdev)
{
	...
	/* un-register driver */
//FUJITSU:2015-05-18 For Socionext tuner Modify -S
	cdev_del(&g_mmtunerDevs);
	unregister_chrdev_region(g_mmtuner_major, 1);
//FUJITSU:2015-05-18 For Socionext tuner Modify -E
...
//FUJITSU:2015-05-18 For Socionext tuner Add -E
	...
	printk(KERN_INFO "mmtuner (for MN885%02x) ver. %d.%d.%d %s (%s)\n",
			mmtuner_version.device,
			mmtuner_version.major, mmtuner_version.minor, mmtuner_version.hotfix,
			mmtuner_version.rc, mmtuner_version.describe);

//FUJITSU:2015-05-18 For Socionext tuner Add -S
	g_mmtuner_major = TUNER_CONFIG_DRV_MAJOR;
	
	if(g_mmtuner_major){
		dev_num = MKDEV(g_mmtuner_major, TUNER_CONFIG_DRV_MINOR);
		ret = register_chrdev_region(dev_num, 1, "mmtuner");
	} else {
		ret = alloc_chrdev_region(&dev_num, 0, 1, "mmtuner");
		g_mmtuner_major = MAJOR(dev_num);
	}
	if(ret < 0){
		printk(KERN_ERR "g_mmtuner_major error fail to get major %d [%d]\n", g_mmtuner_major, ret);
		printk(KERN_INFO "g_mmtuner_major:%s -E\n", __func__);
		unregister_chrdev_region(g_mmtuner_major, 1);
		return ret;
	}
//FUJITSU:2015-05-18 For Socionext tuner Add -E
...
//FUJITSU:2015-05-18 For Socionext tuner Add -S
	cdev_init(&g_mmtunerDevs, &TunerFileOperations);
	g_mmtunerDevs.owner = THIS_MODULE;
	ret = cdev_add(&g_mmtunerDevs, MKDEV(g_mmtuner_major, TUNER_CONFIG_DRV_MINOR), 1);
	if(ret){
		printk(KERN_ERR "cdev_add Error %d adding rfs%d\n", ret, TUNER_CONFIG_DRV_MINOR);
		cdev_del(&g_mmtunerDevs);
		return ret;
	}
//FUJITSU:2015-05-18 For Socionext tuner Add -E
...
//FUJITSU:2015-05-18 For Socionext tuner Modify -S
	dev = device_create(device_class, NULL,
			MKDEV(g_mmtuner_major, TUNER_CONFIG_DRV_MINOR), NULL,
			"mmtuner");
//FUJITSU:2015-05-18 For Socionext tuner Modify -E
	if (IS_ERR(dev)) {
...
//FUJITSU:2015-05-18 For Socionext tuner Modify -S
	device_destroy(device_class,
			MKDEV(g_mmtuner_major, TUNER_CONFIG_DRV_MINOR));
//FUJITSU:2015-05-18 For Socionext tuner Modify -E
...
//			INFO_PRINT("%s: resume!", __FUNCTION__);
	...
	return size;
#else
	INFO_PRINT("NOT support the TS Slave I/F of the tuner device.")
	return -ENOSYS;
#endif
}
		...
		unsigned int uCommand, unsigned long uArgument)
{
	int ret = 0;
	TUNER_DATA_RW rw;
	COMPAT_TUNER_DATA_RW compat_rw;
	TUNER_DATA_EVENT ev;
	unsigned long copy_ret;
...
//	INFO_PRINT("%s START", __FUNCTION__);
	...
	uArgument = (unsigned long)compat_ptr(uArgument);

	switch (uCommand) {
	/* get a parameter of the register of Tuner */
	case COMPAT_TUNER_IOCTL_VALGET:
		copy_ret = copy_from_user(&compat_rw, &(*(COMPAT_TUNER_DATA_RW *) uArgument), sizeof(COMPAT_TUNER_DATA_RW));
		if (copy_ret != 0) {
			TRACE();
			mtxLock(); /* exclusive control (lock) */
			return -EINVAL;
		...
		rw.sngl.bank = compat_rw.sngl.bank;
		rw.sngl.adr = compat_rw.sngl.adr;
		rw.sngl.sbit = compat_rw.sngl.sbit;
		rw.sngl.ebit = compat_rw.sngl.ebit;
		rw.sngl.param = compat_rw.sngl.param;
		rw.sngl.enabit = compat_rw.sngl.enabit;

		ret = tuner_drv_read_regs(&rw, 1);
		if (ret == 0) {

			compat_rw.sngl.bank = rw.sngl.bank;
			compat_rw.sngl.adr = rw.sngl.adr;
			compat_rw.sngl.sbit = rw.sngl.sbit;
			compat_rw.sngl.ebit = rw.sngl.ebit;
			compat_rw.sngl.param = rw.sngl.param;
			compat_rw.sngl.enabit = rw.sngl.enabit;

			copy_ret = copy_to_user(&(*(COMPAT_TUNER_DATA_RW *) uArgument), &compat_rw, sizeof(COMPAT_TUNER_DATA_RW));
			if (copy_ret != 0) {
				TRACE();
				mtxUnlock(); /* exclusive control (unlock) */
				return -EINVAL;
		...
		break;
	/* write a parameters to the register of Tuner */
	case COMPAT_TUNER_IOCTL_VALSET:
		copy_ret = copy_from_user(&compat_rw, &(*(COMPAT_TUNER_DATA_RW *) uArgument), sizeof(COMPAT_TUNER_DATA_RW));
		if (copy_ret != 0) {
			TRACE();
			mtxUnlock(); /* exclusive control (unlock) */
		...
		break;
	/* write the continuous address registers */
	case COMPAT_TUNER_IOCTL_CNTSET:
	{
		int i;
		uint8_t *buf;
		uint8_t *buf_trans;
		...
		buf = (uint8_t *)vmalloc(compat_rw.cont.len);
		if (buf == NULL) {
			TRACE();
			mtxUnlock() /* exclusive control (unlock) */
			return -ENOMEM;
		}
		copy_ret = copy_from_user(buf, (uint8_t*)compat_ptr(compat_rw.cont.buf), compat_rw.cont.len);
		if (copy_ret != 0) {
			TRACE();
			mtxUnlock() /* exclusive control (unlock) */
			vfree(buf);
		...
		// Žb’è‘Îˆ:I2C‚Ü‚Æ‚ß‘—‚è‚ÅTIMEOUT”­¶‚Ì‚½‚ßA1byte‚¸‚Â“]‘—‚É•ÏX 
		DEBUG_PRINT( "len:%d", compat_rw.cont.len );
//		ret = tuner_drv_hw_write_reg(compat_rw.cont.bank, compat_rw.cont.adr, compat_rw.cont.len, buf);
		buf_trans = buf;
		for (i = 0; i < compat_rw.cont.len; i++) {
			DEBUG_PRINT( "buf_ptr:0x%p data:0x%x", buf_trans, *buf_trans );
			ret = tuner_drv_hw_write_reg(compat_rw.cont.bank, compat_rw.cont.adr, 1, buf_trans);
			buf_trans++;
		}
		...
		break;
		/* write the continuous address registers */
	case COMPAT_TUNER_IOCTL_CNTGET:
	{
		uint8_t *buf;
		...
		ret = tuner_drv_hw_read_reg(compat_rw.cont.bank, compat_rw.cont.adr, compat_rw.cont.len, buf);
		if (ret == 0) {
			copy_ret = copy_to_user((uint8_t*)compat_ptr(compat_rw.cont.buf), buf, compat_rw.cont.len);
			if (copy_ret != 0) {
				TRACE();
				mtxUnlock() /* exclusive control (unlock) */
				vfree(buf);
		...
		break;
	/* get the interrupt factor and status */
	case COMPAT_TUNER_IOCTL_EVENT_GET:
		copy_ret = copy_to_user(&(*(TUNER_DATA_EVENT *)uArgument),
				&g_cnt.ev, sizeof(TUNER_DATA_EVENT));
		if (copy_ret) {
			TRACE();
		...
		break;
	/* be available some interrupts */
	case COMPAT_TUNER_IOCTL_EVENT_SET:
		DEBUG_PRINT("*** VALSET_EVENT ***");
		copy_ret = copy_from_user(&ev, &(*(TUNER_DATA_EVENT *)uArgument),
				sizeof(TUNER_DATA_EVENT));
		if (copy_ret != 0) {
		...
		break;
	/* be disable some interrupts */
	case COMPAT_TUNER_IOCTL_EVENT_REL:
		copy_ret = copy_from_user(&ev, &(*(TUNER_DATA_EVENT *) uArgument),	sizeof(TUNER_DATA_EVENT));
		if (copy_ret != 0) {
			TRACE();
			mtxUnlock(); /* exclusive control (unlock) */
		...
		break;
	/* start to receive TS data from Tuner */
	case COMPAT_TUNER_IOCTL_TSIF_START:
#if defined(DPATH_SPI) || defined(DPATH_SDIO)
		if (!g_tscnt.tsifth_wait) {
			TRACE();
			mtxUnlock();
...
#endif
	/* stop to receive TS data from Tuner */
	case COMPAT_TUNER_IOCTL_TSIF_STOP:
#if defined(DPATH_SPI) || defined(DPATH_SDIO)
		ret = tuner_drv_tsif_stop();
		break;
#endif
	/* return byte num of a TS packet */
	case COMPAT_TUNER_IOCTL_TSIF_PKTSIZE:
#if defined(DPATH_SPI) || defined(DPATH_SDIO)
	{
		int len = 0;
		len = tuner_drv_tsif_pktsize();
	...
	{
		unsigned int ver = (mmtuner_version.device << 24) |
				(mmtuner_version.major << 16) |	(mmtuner_version.minor << 8) |
