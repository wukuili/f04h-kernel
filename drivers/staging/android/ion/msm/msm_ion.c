...
extern void fjsec_page_acl_preset(const char *id, long param1, long param2);
#endif
...
{
	return type == ((enum ion_heap_type) ION_HEAP_TYPE_DMA);
}
#endif
/* FUJITSU:2015-03-07 GEN-141-SCRU-ALT ADD E */
...
/* FUJITSU:2015-03-07 GEN-141-SCRU-ALT ADD S */
	...
	case ION_IOC_CLONE:
	{
		struct ion_clone_data data;
		struct ion_handle *handle;
		void* kvaddr;

		pr_debug("call ION_IOC_CLONE\n");
		// Copy ion clone data info from user to kernel 
		if (copy_from_user(&data, (void __user *)arg, sizeof(struct ion_clone_data))) {
			pr_err("Failed to get user data\n");
			return -EFAULT;
		}


		// Get handle to sec ion buf
		handle = ion_import_dma_buf(client, data.fd);
		if (IS_ERR(handle)) {
			pr_err("Failed to get handle by fd\n");
			return PTR_ERR(handle);
		}

		// map ion mem to kernel address space and get virtual address
		...
		// Copy ion clone data from user to kernel
		if(copy_from_user(kvaddr, (void __user *)data.vaddr, data.length)) {
			pr_err("Failed to copy to sec buffer\n");
			return -EFAULT;
		}

		// ummap ion mem from kernel space
...
/* FUJITSU:2015-03-07 GEN-141-SCRU-ALT MOD S */
			if (heap_data->size) {
...
/* FUJITSU:2015-03-07 GEN-141-SCRU-ALT MOD E */
/* FUJITSU:2014-12-08 GEN-141-SCRU-ALT ADD S */
#ifdef CONFIG_SECURITY_FJSEC
			else {
				pr_info("ION heap %s created\n",
							  heap_data->name);
				// Regist ion attribute to sec module
				fjsec_page_acl_preset(heap_data->name, heap_data->id, -1);
			}
#else
/* FUJITSU:2014-12-08 GEN-141-SCRU-ALT ADD E */
				...
				pr_info("ION heap %s created\n",
							  heap_data->name);
#endif /* FUJITSU:2014-12-08 GEN-141-SCRU-ALT ADD */
