...
void msm_tlmm_gp_show_irq_resume(void)
{
		...
		if (msm_tlmm_get_intr_status(ic, i)) {
			...
			if(virq){
				...
				if (desc) {
					...
					snprintf(buf, sizeof(buf), "irq=%d name=%s%s%s action=%s handler=%pf count=%u",
						virq,
					...
					printk( KERN_NOTICE "%s: triggered %s\n", __func__, buf);
				}
/* FUJITSU LIMITED:2015-05-07 H1520349 add start */
				{
					extern void filog_resume_irq(unsigned int irq);
					filog_resume_irq((unsigned int)virq);
				}
...
/* FUJITSU LIMITED:2015-11-17 H1610003 add start */
#define TLMM_GP_DUMP_BASE              (0xfd511000)
#define TLMM_GP_DUMP_REG_SIZE          (0x3000)
#define TLMM_GP_DUMP_PIN_NUM           (146)
#define TLMM_GP_DUMP_CFG(base,gpio)    (base + 0x0 + (0x10 * (gpio)))
#define TLMM_GP_DUMP_IN_OUT(base,gpio) (base + 0x4 + (0x10 * (gpio)))
#ifndef TLMM_PULL_KEEPER
#define TLMM_PULL_KEEPER               (2)
#endif
unsigned __msm_tlmm_gp_get_config(void __iomem *reg_base, unsigned gpio)
{
	return readl_relaxed(TLMM_GP_DUMP_CFG(reg_base,gpio));
}

unsigned __msm_tlmm_gp_get_oe_config(void __iomem *reg_base, unsigned gpio)
{
	return readl_relaxed(TLMM_GP_DUMP_CFG(reg_base,gpio)) & BIT(GPIO_OE_BIT);
}
unsigned __msm_tlmm_gp_get_io_value(void __iomem *reg_base, unsigned gpio)
{
	return readl_relaxed(TLMM_GP_DUMP_IN_OUT(reg_base,gpio));
}
static void __iomem *dump_reg_base = 0;
void msm_tlmm_gp_dbg_out(void)
{
	unsigned	i;
	unsigned	loop;
	unsigned	gpio = 0;
	...
	if (msm_gpio_ready) {
		ic = &msm_tlmm_gp_irq;
		pinfo = ic_to_pintype(ic);
		chip = pintype_get_gc(pinfo);
		dev = chip->dev;
		printk("GPIOs %d-%d, %s/%s, %s, %s:\n",
						chip->base, chip->base + chip->ngpio - 1,
						dev ? (dev->bus ? dev->bus->name : "no-bus") : "",
						dev ? dev_name(dev) : "",
						chip->label ? chip->label : "",
						chip->can_sleep ? ", can sleep" : "");
		loop = chip->ngpio;
	} else {
		dump_reg_base = ioremap(TLMM_GP_DUMP_BASE,TLMM_GP_DUMP_REG_SIZE);
		if(!dump_reg_base){
			printk("dump_reg_base ioremap err\n");
			return;
		}
		loop = TLMM_GP_DUMP_PIN_NUM;
	}

	printk("MSM8992 GPIO,gpio,alternate,direction,value(out),value(in),config,pull,strength\n");
	for (i = 0; i < loop; i++, gpio++) {
		if (msm_gpio_ready) {
			spin_lock_irqsave(&ic->irq_lock, irq_flags);
			//: get config value.
			config = __msm_tlmm_gp_get_config(pinfo->reg_base,gpio);
			//: check in/out bit.
			is_out = __msm_tlmm_gp_get_oe_config(pinfo->reg_base,gpio);
			//: get gpio value.
			state = __msm_tlmm_gp_get_io_value(pinfo->reg_base,gpio);
			spin_unlock_irqrestore(&ic->irq_lock, irq_flags);
		} else {
			//: get config value.
			config = __msm_tlmm_gp_get_config(dump_reg_base,gpio);
			//: check in/out bit.
			is_out = __msm_tlmm_gp_get_oe_config(dump_reg_base,gpio);
			//: get gpio value.
			state = __msm_tlmm_gp_get_io_value(dump_reg_base,gpio);
		}
		printk("MSM8992 GPIO,%d,%d,%s,%s,%s,\"%04x\",", 
					gpio, 
					(config>>TLMM_GP_FUNC_SHFT)&TLMM_GP_FUNC_MASK,
					is_out ? "O" : "I",
					(state & 2) ? "H" : "L", 
					(state & 1) ? "H" : "L", 
					config);

		switch (config & TLMM_GP_PULL_MASK) {
		case TLMM_NO_PULL:
			printk("NO_PULL,");
			break;
		case TLMM_PULL_DOWN:
		...
		switch ((config >> TLMM_GP_DRV_SHFT) & TLMM_GP_DRV_MASK) {
		case 0:
			printk("2mA");
			break;
		case 1:
	...
	if (!msm_gpio_ready) {
		if (dump_reg_base) {
			iounmap(dump_reg_base);
			dump_reg_base = 0;
		}
	}
}
EXPORT_SYMBOL(msm_tlmm_gp_dbg_out);
/* FUJITSU LIMITED:2015-11-17 H1610003 add end */
