...
static const char * const regulator_string[] = {
	"pm8994_s1",
	"pm8994_s2",
	"pm8994_s3",
	"pm8994_s4",
...
#define REGULATOR_STRING_SIZE (sizeof(regulator_string)/sizeof(*regulator_string))

static void local_rmp_vreg_dump_info(const char *name)
{
	...
	if ((IS_ERR(reg)) || (reg == NULL)) {
		printk(KERN_INFO "DRPM,%s,ERR regulator=NULL\n", name);
		return;
	}
	is_enabled = regulator_is_enabled(reg);
	voltage = regulator_get_voltage(reg);
	regulator_put(reg);

	printk(KERN_INFO "DRPM,%s,%d,%d\n", name, is_enabled, voltage);
}

void rmp_vreg_dump_info(void)
{
	int i;

	printk(KERN_INFO "DRPM, name, is_enabled, voltage\n");
	for (i = 0; i < REGULATOR_STRING_SIZE; i++){
		local_rmp_vreg_dump_info(regulator_string[i]);
	}
}
