...
/* FCNT LIMITED:2016-02-16 USB H16100644 add start */
#include <linux/qpnp/pin.h>
#include <linux/gpio.h>
/* FCNT LIMITED:2016-02-16 USB H16100644 add end */
...
#include "debug.h"
#include "xhci.h"
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
#include "linux/mfd/fj_charger.h"
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */

#define VBUS_REG_CHECK_DELAY	(msecs_to_jiffies(1000))
#define MAX_INVALID_CHRGR_RETRY 3
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
static enum dwc3_chg_type start_chg_type = DWC3_INVALID_CHARGER;
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
/* FCNT LIMITED:2016-02-16 USB H16100644 add start */
#define FJ_OTG_VBUS_GPIO    5
#define FJ_OTG_VBUS_ON      1
#define FJ_OTG_VBUS_OFF     0
#define FJ_OTG_VBUS_DELAY   100
static bool fj_vbus_request = false;
static struct delayed_work *fj_vbus_work = NULL;
static struct workqueue_struct *fj_vbus_wq = NULL;
/* FCNT LIMITED:2016-02-16 USB H16100644 add end */
...
static int dwc3_otg_set_power(struct usb_phy *phy, unsigned mA)
{
	...
	struct dwc3_otg *dotg = container_of(phy->otg, struct dwc3_otg, otg);
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
	char *chg_name[] = {"UNKNOWN", "SDP", "DCP", "CDP", "PROPRIETARY", "FLOATED"};
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
...
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
	dev_info(phy->dev, "****** %s ( %s ): [%s] Charger, [%u] mA ******\n", __func__,
			mA ? "Charge start" : "Charge stop",
			0 <= dotg->charger->chg_type && dotg->charger->chg_type <= DWC3_FLOATED_CHARGER ?
			chg_name[dotg->charger->chg_type] : chg_name[0], mA);

	if(mA != 0) {
		switch (start_chg_type){
		case DWC3_SDP_CHARGER:
			fj_chg_usb_vbus_draw(FJ_CHG_ENABLE);    /* 500mA */
			break;
		case DWC3_DCP_CHARGER:
		case DWC3_CDP_CHARGER:
			fj_chg_ac_vbus_draw(FJ_CHG_ENABLE);    /* 1500mA */
			break;
		default:
			fj_chg_other_vbus_draw(FJ_CHG_ENABLE);    /* 500mA */
			break;
		}
	} else {
		switch (start_chg_type){
		case DWC3_SDP_CHARGER:
			fj_chg_usb_vbus_draw(FJ_CHG_DISABLE);    /* 500mA */
			break;
		case DWC3_DCP_CHARGER:
		case DWC3_CDP_CHARGER:
			fj_chg_ac_vbus_draw(FJ_CHG_DISABLE);    /* 1500mA */
			break;
		default:
			fj_chg_other_vbus_draw(FJ_CHG_DISABLE);    /* 500mA */
			break;
		}
	}
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
	...
	case OTG_STATE_B_IDLE:
		if (!test_bit(ID, &dotg->inputs)) {
			...
			if (charger) {
				...
				case DWC3_DCP_CHARGER:
/*				case DWC3_PROPRIETARY_CHARGER:*//* FUJITSU LIMITED:2016-01-21 USB H16100640 del */
					dev_dbg(phy->dev, "lpm, DCP charger\n");
					start_chg_type = DWC3_DCP_CHARGER;    /* FUJITSU LIMITED:2016-01-21 USB H16100640 add */
...
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
				case DWC3_PROPRIETARY_CHARGER:
					...
					start_chg_type = DWC3_PROPRIETARY_CHARGER;
					dwc3_otg_set_power(phy, FJ_CHG_USB_CURRENT);
					dbg_event(0xFF, "PROPCHG put", 0);
					pm_runtime_put_sync(phy->dev);
					break;
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
				case DWC3_CDP_CHARGER:
					start_chg_type = DWC3_CDP_CHARGER;    /* FUJITSU LIMITED:2016-01-21 USB H16100640 add */
					dwc3_otg_set_power(phy,
							DWC3_IDEV_CHG_MAX);
				...
				case DWC3_SDP_CHARGER:
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
					start_chg_type = DWC3_SDP_CHARGER;
					dwc3_otg_set_power(phy, FJ_CHG_USB_CURRENT);
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
					dwc3_otg_start_peripheral(&dotg->otg,
									1);
				...
				case DWC3_FLOATED_CHARGER:
					...
					if (dotg->charger_retry_count ==
						max_chgr_retry_count) {
/*						dwc3_otg_set_power(phy, 0);*//* FUJITSU LIMITED:2016-01-21 USB H16100640 del */
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
						start_chg_type = DWC3_FLOATED_CHARGER;
						dwc3_otg_set_power(phy, FJ_CHG_USB_CURRENT);
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
...
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add start */
			start_chg_type = DWC3_INVALID_CHARGER;
/* FUJITSU LIMITED:2016-01-21 USB H16100640 add end */
			dev_dbg(phy->dev, "No device, trying to suspend\n");
...
/* FCNT LIMITED:2016-02-16 USB H16100644 add start */
/*
 * dwc3_otg_fj_vbus_work - workqueue function.
 * @w: Pointer to the dwc3 otg workqueue
 */
static void dwc3_otg_fj_vbus_work(struct work_struct *w)
{
	int value;

	if (fj_vbus_request == true) {
		value = gpio_get_value(qpnp_pin_map("pmi8994-gpio", FJ_OTG_VBUS_GPIO));
		if(value == FJ_OTG_VBUS_OFF){
			gpio_set_value_cansleep(qpnp_pin_map("pmi8994-gpio", FJ_OTG_VBUS_GPIO), FJ_OTG_VBUS_ON);
			pr_info("%s: FJ_OTG_VBUS_ON\n", __func__);
		}
	} else {
		value = gpio_get_value(qpnp_pin_map("pmi8994-gpio", FJ_OTG_VBUS_GPIO));
		if(value == FJ_OTG_VBUS_ON){
			gpio_set_value_cansleep(qpnp_pin_map("pmi8994-gpio", FJ_OTG_VBUS_GPIO), FJ_OTG_VBUS_OFF);
			pr_info("%s: FJ_OTG_VBUS_OFF\n", __func__);
		}
	}
}

/*
 * dwc3_otg_set_fj_vbus: USB3 OTG bus voltge enable/disable
 * @fj_vbus: USB3_OTG_VBUS_EN(GPTO_5) enable(true) / disable(false)
 */
void dwc3_otg_set_fj_vbus(bool fj_vbus)
{
	fj_vbus_request = fj_vbus;

	if (fj_vbus_wq) {
		pr_info("%s: fj_vbus_request = %d\n", __func__, fj_vbus_request);
		queue_delayed_work(fj_vbus_wq, fj_vbus_work, FJ_OTG_VBUS_DELAY);
	} else {
		pr_err("%s: no create fj_vbus_work_q\n", __func__);
	}
}
EXPORT_SYMBOL(dwc3_otg_set_fj_vbus);
/* FCNT LIMITED:2016-02-16 USB H16100644 add end */
...
int dwc3_otg_init(struct dwc3 *dwc)
{
	...
	int ret = 0;    /* FCNT LIMITED:2016-02-16 USB H16100644 add */
...
/*	dotg->otg.phy->set_power = dwc3_otg_set_power;*//* FUJITSU LIMITED:2016-01-21 USB H16100640 del */
...
/* FCNT LIMITED:2016-02-16 USB H16100644 add start */
	ret = gpio_request(qpnp_pin_map("pmi8994-gpio", FJ_OTG_VBUS_GPIO), "FJ_OTG_VBUS_GPIO");
	if (ret) {
		dev_err(dwc->dev, "failed to gpio_request ret = %d\n", ret);
	}

	fj_vbus_wq = create_singlethread_workqueue("fj_vbus_work_q");
	if (!fj_vbus_wq) {
		dev_err(dwc->dev, "%s: Unable to create workqueue fj_vbus_work_q\n", __func__);
	} else {
		INIT_DELAYED_WORK(&dotg->fj_vbus_work, dwc3_otg_fj_vbus_work);
		fj_vbus_work = &dotg->fj_vbus_work;
	}
/* FCNT LIMITED:2016-02-16 USB H16100644 add end */
...
void dwc3_otg_exit(struct dwc3 *dwc)
{
	...
	if (dotg) {
		...
		pm_runtime_put(dwc->dev);
/* FCNT LIMITED:2016-02-16 USB H16100644 add start */
		gpio_free(qpnp_pin_map("pmi8994-gpio", FJ_OTG_VBUS_GPIO));
		if (fj_vbus_work) {
			destroy_workqueue(fj_vbus_wq);
		}
/* FCNT LIMITED:2016-02-16 USB H16100644 add end */
		dwc->dotg = NULL;
	}
