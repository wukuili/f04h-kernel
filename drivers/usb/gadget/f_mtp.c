...
struct {
	struct mtp_ext_config_desc_header	header;
	struct mtp_ext_config_desc_function    function_mtp;
	struct mtp_ext_config_desc_function    function_adb;
...
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
	...
	uint8_t kitting_spmode_data[2];
	unsigned long data_size = sizeof(kitting_data);
	unsigned long spmode_data_size = sizeof(kitting_spmode_data);
	int usb_kitting = USB_KITTING_DISABLE;
	int ret = 0;

	if(!makercmd_mode){
		ret = get_nonvolatile( &kitting_data[0], APNV_KITTING_USB_FLAG, data_size);
		if(ret < 0) {
		...
		ret = get_nonvolatile( &kitting_spmode_data[0], APNV_KITTING_SPMODE_FLAG, spmode_data_size);
		if(ret < 0) {
		...
		if(usb_kitting == USB_KITTING_ENABLE){
				return -ENODEV;
		}
	}
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
		...
		if (ctrl->bRequest == 1
				...
				&& (w_index == 4 || w_index == 5)) {
/* FUJITSU LIMITED:2015-11-27 USB H1617002 del start */
#if 0
			value = (w_length < sizeof(mtp_ext_config_desc) ?
					w_length : sizeof(mtp_ext_config_desc));
			memcpy(cdev->req->buf, &mtp_ext_config_desc, value);
#endif
/* FUJITSU LIMITED:2015-11-27 USB H1617002 del end */
/* FUJITSU LIMITED:2015-11-27 USB H1617002 add start */
			if (dev->adb_enable) {
...
/* FUJITSU LIMITED:2015-11-27 USB H1617002 add start */
static void mtp_set_adb_enable(bool enable)
{
	struct mtp_dev *dev = _mtp_dev;
