...
/* FUJITSU LIMITED:2015-11-27 USB H1617003 add start */
#include "f_mmp.c"
/* FUJITSU LIMITED:2015-11-27 USB H1617003 add end */
...
#include "f_ncm.c"
#include "f_charger.c"
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
#include <linux/nonvolatile_common.h>
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
...
/* FUJITSU LIMITED:2015-11-27 USB H1617004 add start */
extern int debugusb_set_pid(unsigned long pid);
/* FUJITSU LIMITED:2015-11-27 USB H1617004 add end */
...
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
#define APNV_KITTING_USB_FLAG     51001
#define APNV_KITTING_SPMODE_FLAG  51004
#define APNV_KITTING_SPMODE_SIZE  2
#define USB_KITTING_DISABLE       0
#define USB_KITTING_ENABLE        1
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
...
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
	...
	int pm_qos_vote = -1;
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
	uint8_t kitting_data[2];
	unsigned long data_size = sizeof(kitting_data);
	int usb_kitting = USB_KITTING_DISABLE;
	unsigned short enableSPMODE;
	int res = 0;
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
	...
	if (uevent_envp) {
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
		res = get_nonvolatile( &kitting_data[0], APNV_KITTING_USB_FLAG, data_size);
		if (res == data_size) {
			if ((kitting_data[0] & KITTING_USB) != 0) {
				usb_kitting = USB_KITTING_ENABLE;
			}
		}

		res = get_nonvolatile(
			(void*)&enableSPMODE, APNV_KITTING_SPMODE_FLAG, APNV_KITTING_SPMODE_SIZE);

		if ((enableSPMODE & 0x02) != 0) {
			usb_kitting =  USB_KITTING_DISABLE;
		}
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
		/*
...
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
			if (usb_kitting == USB_KITTING_DISABLE) {
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
...
/* FUJITSU LIMITED:2015-11-27 USB H1617002 add start */
static int android_get_function_enable(struct android_usb_function *fc, char *name)
{
	struct android_dev *dev = fc->android_dev;
	struct android_configuration *conf;
	...
	list_for_each_entry(conf, &dev->configs, list_item)
		list_for_each_entry(f_holder, &conf->enabled_functions, enabled_list) {
			if (!strcmp(name, f_holder->f->name)) {
				enable = 1;
				break;
			}
		}
...
/* FUJITSU LIMITED:2015-11-27 USB H1617002 add end */
...
/*static bool qdss_debug_intf;*/    /* FUJITSU LIMITED:2015-11-27 USB H1617004 del */
static bool qdss_debug_intf = 1;    /* FUJITSU LIMITED:2015-11-27 USB H1617004 add */
...
{
/* FUJITSU LIMITED:2015-11-27 USB H1617002 add start */
	if (android_get_function_enable(f, "ffs"))
		mtp_set_adb_enable(true);
	else
		mtp_set_adb_enable(false);
/* FUJITSU LIMITED:2015-11-27 USB H1617002 add end */
...
static int mmp_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	gserial_alloc_line(&tty_line);
	pr_info("%s: gserial_alloc_line [%d]\n", __func__, tty_line);
	return mmp_bind_config(c, 0);
}

static struct android_usb_function mmp_function = {
	.name = "mmp",
	.bind_config = mmp_function_bind_config,
};
/* FUJITSU LIMITED:2015-11-27 USB H1617003 add end */
...
/* FUJITSU LIMITED:2015-11-27 USB H1617004 add end */
	if (enabled && !dev->enabled) {
		 ...
		 */
/* FUJITSU LIMITED:2015-11-27 USB H1617004 add start */
		debugusb_set_pid(device_desc.idProduct);
		pr_info("%s: nv_set debug pid =[%x]\n", __func__, device_desc.idProduct);
/* FUJITSU LIMITED:2015-11-27 USB H1617004 add end */
...
/* FUJITSU LIMITED:2015-11-27 USB H1617003 del start */
#if 0
...
#endif
/* FUJITSU LIMITED:2015-11-27 USB H1617003 del end */
/* FUJITSU LIMITED:2015-11-27 USB H1617003 add start */
	strlcpy(manufacturer_string, "Fujitsu", sizeof(manufacturer_string) - 1);
	strlcpy(product_string, "FJDEV", sizeof(product_string) - 1);
	strlcpy(serial_string, "F0XX000000", sizeof(serial_string) - 1);
/* FUJITSU LIMITED:2015-11-27 USB H1617003 add end */
