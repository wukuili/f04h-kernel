...
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add start */
#include <linux/fj_mode.h>
#define S_DUMP  1
#ifdef S_DUMP
  #define serial_printk(level, format, args...) \
    do { \
      if (fj_boot_mode == FJ_MODE_MAKER_MODE || fj_boot_mode == FJ_MODE_KERNEL_MODE ) \
		printk(level format "\n", ## args); \
    } while (0)

  #define serial_dump(_ttl, _p, _l) \
    do { \
      if (fj_boot_mode == FJ_MODE_MAKER_MODE || fj_boot_mode == FJ_MODE_KERNEL_MODE ) \
        if (_l > 0) \
          print_hex_dump(KERN_INFO, _ttl, DUMP_PREFIX_NONE, 16, 1, _p, _l, false); \
    } while (0)
#else
  #define serial_printk(level, format, args...)   do {} while (0)
  #define serial_dump(_ttl, _p, _l)               do {} while (0)
#endif
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add end */
...
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add start */
            serial_printk(KERN_INFO, "%s(): ttyGS%d (%p) reading %d bytes",__func__,port->port_num, tty, size);
            serial_dump(" @@@ RX Data Dump => ", packet, size);
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add end */
...
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add start */
    serial_printk(KERN_INFO, "%s()",__func__);
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add end */
...
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add start */
    serial_printk(KERN_INFO, "%s(): ttyGS%d (%p) writing %d bytes",__func__,port->port_num, tty, count);
    serial_dump(" @@@ TX Data Dump => ", buf, count);
/* FUJITSU LIMITED:2014-10-28 USB H1517007 add end */
