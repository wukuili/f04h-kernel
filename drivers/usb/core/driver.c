...
static int usb_kitting_device(const struct device *dev, const struct usb_device_id *id) {

	#define KITTING_USB				0x01
	#define KITTING_USB_ETHERNET	0x02
	#define KITTING_USB_HID_CLASS	0x04

	static const struct usb_device_descriptor	use_device[] = {
	...
	for (i = 0; i < (sizeof(use_device) / sizeof(use_device[0])) ; i++) {
		if ((le16_to_cpu(udev->descriptor.idVendor) == use_device[i].idVendor) &&
			(le16_to_cpu(udev->descriptor.idProduct) == use_device[i].idProduct)) {
			return USB_KITTING_DISABLE;
		}
	}
	...
	for (i = 0; i < intf->num_altsetting; i++) {
		...
		if (host_if->desc.bInterfaceClass == USB_CLASS_HUB) {
			return USB_KITTING_DISABLE;
		}
	}
	...
	if ((kitting_data[0] & KITTING_USB_ETHERNET) == 0) {
		...
		dinfo = (struct driver_info *)id->driver_info;

		if (dinfo != NULL) {
			if ((dinfo->flags & FLAG_ETHER) != 0) {
				return USB_KITTING_DISABLE;
			}
		}
	}
...
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add start */
	if (usb_kitting_device(dev, id)) {
		dev_dbg(dev, "kitting device : idVendor = %d, idProduct = %d\n", le16_to_cpu(udev->descriptor.idVendor), le16_to_cpu(udev->descriptor.idProduct));
		return -ENODEV;
	}
/* FUJITSU LIMITED:2015-06-30 USB H1527034 add end */
