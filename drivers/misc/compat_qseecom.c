...
static int compat_get_qseecom_client_authentication_req(
		struct compat_qseecom_client_authentication_req __user *data32,
		struct qseecom_client_authentication_req __user *data)
{
	compat_int_t clnt_id;
	compat_uptr_t res;
	int err;

	err = get_user(clnt_id, &data32->clnt_id);
	err |= put_user(clnt_id, &data->clnt_id);

	err |= get_user(res, &data32->res);
	data->res = NULL;
	err |= put_user(res, (compat_uptr_t *)&data->res);
	return err;
}
...
#ifdef __FJSEC_TZ_CIPHER__
	case COMPAT_QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ:
		return QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ;
#endif /* __FJSEC_TZ_CIPHER__ */
	default:
		return cmd;
	}
		...
		unsigned int cmd, unsigned long arg)
{
    ...
    case COMPAT_QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ: {
		...
		err = compat_get_qseecom_client_authentication_req(data32, data);
		if (err)
			return err;
