...
#ifdef __FJSEC_TZ_CIPHER__
#include "qseecom_auth.h"
#endif /* __FJSEC_TZ_CIPHER__ */
...
#ifdef __FJSEC_TZ_CIPHER__
static int qseecom_check_exist_process(pid_t pid)
{
	struct task_struct *r = NULL;

	rcu_read_lock();
	r = find_task_by_vpid(pid);
	rcu_read_unlock();
	if (r == NULL)
	{
...
static void qseecom_deregister_cliauthinfo(int pid, int appid)
{
	int loop = 0;
	int clnt_cnt = 0;

	for( clnt_cnt = 0; clnt_cnt < LOCAL_CLIENT_MAX; clnt_cnt++ ){
		/* Search for app_id in the tzc_valid_id_list */
		for(loop = 0; loop < TZAPP_CLIENT_MAX; loop++){

			/* app_id is registered */
			if(tzc_valid_id_list[clnt_cnt][loop][TZAPP_ID_LIST] == appid
		   	&& tzc_valid_id_list[clnt_cnt][loop][PROCESS_ID_LIST] == pid){
				/* Clear app_id */
				tzc_valid_id_list[clnt_cnt][loop][PROCESS_ID_LIST] = 0;
				tzc_valid_id_list[clnt_cnt][loop][TZAPP_ID_LIST] = 0;
				break;
			}
		}
	}
...
static int qseecom_load_app(struct qseecom_dev_handle *data, void __user *argp)
{
	...
	/*Access from fjsec_tz_cipher or not?*/
	load_img_req.img_name[MAX_APP_NAME_SIZE - 1] = '\0';
	for( clnt_cnt = 0; clnt_cnt < LOCAL_CLIENT_MAX; clnt_cnt++ ){
		if( strcmp( load_img_req.img_name, tzc_name[clnt_cnt] ) == 0 ){

			pr_debug("this is %s process \n", tzc_name[clnt_cnt]);

			/* Check if process_ID is in tzc_valid_id_list */
			for(loop = 0; loop < TZAPP_CLIENT_MAX; loop++){

				if(tzc_valid_id_list[clnt_cnt][loop][PROCESS_ID_LIST] == current->tgid){

					/* process_ID is in tzc_valid_id_list -> Continue the process */
					pr_debug("this is valid %s process \n", tzc_name[clnt_cnt]);
					break;
				}
			}

			if(loop == TZAPP_CLIENT_MAX){
				/* process_ID is NOT in tzc_valid_id_list -> Break the process */
				pr_err("no valid process ID\n");
				return -EFAULT;
			}
			break;
		}
	...
	if (app_id) {
		...
		pr_debug("qseecom_set_bandwidth(true) ret=%d\n", ret);
		if (ret) {
			if (!IS_ERR_OR_NULL(ihandle))
				ion_free(qseecom.ion_clnt, ihandle);
			ret = -EINVAL;
		...
		ret_buf = qseecom_set_bandwidth(&handle, false);
		pr_debug("qseecom_set_bandwidth(false) ret=%d\n", ret_buf);
...
#ifdef __FJSEC_TZ_CIPHER__
	tzc_valid_id_list[clnt_cnt][loop][TZAPP_ID_LIST] = app_id; /* Regist app_id */
#endif /* __FJSEC_TZ_CIPHER__ */
	if (copy_to_user(argp, &load_img_req, sizeof(load_img_req))) {
		pr_err("copy_to_user failed\n");
#ifdef __FJSEC_TZ_CIPHER__
		qseecom_deregister_cliauthinfo(current->tgid, app_id);
#endif /* __FJSEC_TZ_CIPHER__ */
		ret = -EFAULT;
		if (first_time == true) {
			spin_lock_irqsave(
...
#ifdef __FJSEC_TZ_CIPHER__
	qseecom_deregister_cliauthinfo(current->tgid, data->client.app_id);
#endif /* __FJSEC_TZ_CIPHER__ */
					...
					void __user *argp)
{
	...
	/* Search for app_id in the tzc_valid_id_list */
	for(clnt_cnt = 0; clnt_cnt < LOCAL_CLIENT_MAX; clnt_cnt++){
		for(i = 0; i < TZAPP_CLIENT_MAX; i++){

			/* app_id is registered */
			if(tzc_valid_id_list[clnt_cnt][i][TZAPP_ID_LIST] == data->client.app_id){

				load_flag = 1;
				/* Check if process_id is consistent or not */
				if( tzc_valid_id_list[clnt_cnt][i][PROCESS_ID_LIST] == current->tgid ){

					/* Continue the process */
					pr_debug("this is authenticated %s app\n",tzc_name[clnt_cnt]);
					auth_flag = 1;
					break;
				}
			}
	...
	if(load_flag == 1 && i == TZAPP_CLIENT_MAX){
		/* Break the process */
		pr_err("this is NOT authenticated %s app\n", tzc_name[clnt_cnt]);
		return -EINVAL;
	}
	...
	if (!strcmp(data->client.app_name, "fj_iris_verifier"))
...
int qseecom_set_bandwidth(struct qseecom_handle *handle, bool high)
{
	...
	if (high) {
		if (qseecom.support_bus_scaling) {
			mutex_lock(&qsee_bw_mutex);
			ret = __qseecom_register_bus_bandwidth_needs(handle->dev,
									HIGH);
			mutex_unlock(&qsee_bw_mutex);
			if (ret)
...
#ifdef __FJSEC_TZ_CIPHER__
		for( clnt_cnt = 0; clnt_cnt < LOCAL_CLIENT_MAX; clnt_cnt++ ){
			if( strcmp( req.app_name, tzc_name[clnt_cnt] ) == 0 ){

				pr_debug("%s process is already loaded\n", tzc_name[clnt_cnt]);
				...
				for(loop = 0; loop < TZAPP_CLIENT_MAX ; loop++){
					...
					if(tzc_valid_id_list[clnt_cnt][loop][PROCESS_ID_LIST] == current->tgid){
						...
						/* process_ID is in tzc_valid_id_list -> Continue the process */
						pr_debug("this is valid %s process \n",tzc_name[clnt_cnt]);
						tzc_valid_id_list[clnt_cnt][loop][TZAPP_ID_LIST] = ret;
						break;
					}	
				}
				if(loop == TZAPP_CLIENT_MAX){
...
#ifdef __FJSEC_TZ_CIPHER__
static int qseecom_get_client_info(struct qseecom_client_info *c_info)
{
	int count = 0;
	...
	if(cred == NULL){
		...
		pr_err("failed to read credientials info\n");
		return -1;
	}

	/* Set client information into c_info */
	c_info->u_id = (int) cred->uid;
	c_info->ng_ids = (int) cred->group_info->ngroups + 1;
	c_info->g_ids[0] = (int) cred->gid;

	for(count = 0; count < cred->group_info->ngroups; count++ ){

		c_info->g_ids[count+1] = (int) cred->group_info->small_block[count];
	}
...
static int qseecom_client_authentication(void __user *argp, 
										 struct qseecom_client_info c_info)
{
	struct qseecom_client_authentication_req req;
	int ret = -1;
	int count = 0;
	int auth_res =0;
	...
	if(argp == NULL){
		...
		pr_err("Invarid parameter\n");
		return -EINVAL;
	}

	/* Get qseecom_client_authentication_req from linux*/
	ret = copy_from_user(&req, argp, sizeof(req));
	if(ret){
	...
	if(authentication_array[req.clnt_id][AUTH_TYPE] & CHECK_USER_ID){

		pr_debug("authentication using user id start\n");
		if( c_info.u_id == authentication_array[req.clnt_id][USER_ID_ARRAY] ){

			auth_res |= CHECK_USER_ID;
		}else{

			pr_err("invalid user id\n");
		}
	}

	/* Authentication using group ID */
	if(authentication_array[req.clnt_id][AUTH_TYPE] & CHECK_GROUP_ID){

		pr_debug("authentication using group id start\n");

		for(count = 0; count < c_info.ng_ids; count++){

			pr_debug("GIDs : %d\n", c_info.g_ids[count]);

			if( c_info.g_ids[count] 
				== authentication_array[req.clnt_id][GROUP_ID_ARRAY]){

				auth_res |= CHECK_GROUP_ID;
				break;
			}
		}

		if(count == c_info.ng_ids){

			pr_err("no valid group id\n");
		}
	}

	/* Authentication using hash */
	if(authentication_array[req.clnt_id][AUTH_TYPE] & CHECK_HASH_VALUE){

		/* Will be implemented... */
		/* if hash value is valid, res |= CHECK_HASH_VALUE */
	}

	/* Final judgement */
	if(auth_res == authentication_array[req.clnt_id][AUTH_TYPE]){

		pr_debug("authentication accepted\n");
		for(count = 0; count < TZAPP_CLIENT_MAX; count++){

			if(tzc_valid_id_list[req.clnt_id][count][PROCESS_ID_LIST] == 0){

				tzc_valid_id_list[req.clnt_id][count][PROCESS_ID_LIST] = current->tgid;
				pr_debug("process ID %d was registered at %d\n", current->tgid, count);
				*(req.res) = 0;
				break;
			}
		}

		if(count == TZAPP_CLIENT_MAX){
			for(count = 0; count < TZAPP_CLIENT_MAX; count++){
				ret = qseecom_check_exist_process(tzc_valid_id_list[req.clnt_id][count][PROCESS_ID_LIST]);
				if( ret == 0 ){
					tzc_valid_id_list[req.clnt_id][count][PROCESS_ID_LIST] = current->tgid;
					pr_debug("process ID %d was registered at %d\n", current->tgid, count);
					*(req.res) = 0;
			...
			if(count == TZAPP_CLIENT_MAX){
				pr_err("could not regist process ID in the tzc_valid_id_list\n");
				*(req.res) = -1;
				return -1;
			}
		}
	}else{
		pr_err("authentication failed\n");
		*(req.res) = -1;
	}
...
long qseecom_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	case QSEECOM_IOCTL_PERF_ENABLE_REQ:{
		...
		if (qseecom.support_bus_scaling) {
			mutex_lock(&qsee_bw_mutex);
			ret = __qseecom_register_bus_bandwidth_needs(data, HIGH);
			mutex_unlock(&qsee_bw_mutex);
		} else {
			ret = qseecom_perf_enable(data);
			if (ret)
...
#ifdef __FJSEC_TZ_CIPHER__
	case QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ: {
		struct qseecom_client_info c_info;

		memset(&c_info, 0x00, sizeof(c_info));
		pr_debug("QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ : Start\n");

		if (data->type != QSEECOM_GENERIC) {
			pr_err("QSEECOM_IOCTL_CLIENT_AUTHENTICATION_REQ : invalid handle (%d)\n", data->type);
			ret = -EINVAL;
			break;
		}
		...
		/* Get client info */
		ret = qseecom_get_client_info(&c_info);
		if(ret != 0){
			pr_err("qseecom_get_client_info : Failed\n");
			goto TZC_END;
		}

		/* Client authentication */
		ret = qseecom_client_authentication(argp, c_info);
		if(ret != 0){
			pr_err("qseecom_client_authentication : Failed\n");
		}
