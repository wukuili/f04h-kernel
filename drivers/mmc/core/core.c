...
#include "sd_ops.h"
#include "sdio_ops.h"
#include "cmdlog.h"	/* FCNT LIMITED:2016-03-07 H16100340 add */
...
/* FUJITSU LIMITED:2015-05-14 H1520355 add start */
#ifdef CONFIG_MMC_PASSWORDS
extern struct mutex lockable_lock;
...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	ktime_t diff;
#endif
	MMC_PRINT_RESPONSE(host, mrq);	/* FCNT LIMITED:2016-03-07 H16100340 add */
	if (host->card)
		...
		if (err == MMC_BLK_URGENT || err == MMC_BLK_URGENT_DONE) {
			...
			if (areq) {
				if (!(areq->cmd_flags &
					...
					MMC_PRINT_CMDLOG(host, areq, areq->mrq);	/* FCNT LIMITED:2016-03-07 H16100340 add */
...
{
	MMC_PRINT_CMDLOG(host, NULL, mrq);	/* FCNT LIMITED:2016-03-07 H16100340 add */
...
/* FUJITSU LIMITED:2014-11-26 H1510110 mod start */
#if 0
	/*
...
#endif
	ext_csd = card->work_buff;
/* FUJITSU LIMITED:2014-11-26 H1510110 mod end */
...
out:
/*	kfree(ext_csd); */	/* FUJITSU LIMITED:2014-11-26 H1510110 del */
	return err;
}
...
/* FUJITSU LIMITED:2014-11-26 H1510109 mod start */
	if (card->csd.tacc_ns <= UINT_MAX / mult){
		data->timeout_ns = card->csd.tacc_ns * mult;
	}else{
		data->timeout_ns = UINT_MAX;
	}
/* FUJITSU LIMITED:2014-11-26 H1510109 mod end */
...
void mmc_set_ios(struct mmc_host *host)
{
	...
	if (mmc_host_get_type(host) != HOST_TYPE_SDIO) /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		trace_clock_set_rate(mmc_hostname(host), ios->clock, raw_smp_processor_id()); 	/* FUJITSU LIMITED:2014-11-26 H1510112 add */
}
...
void mmc_detect_work_sync(struct mmc_host *host)
{
	flush_delayed_work(&host->detect);
}

EXPORT_SYMBOL(mmc_detect_work_sync);
...
{
	/* FUJITSU LIMITED:2015-01-23 H1510178 add start */
	if (!host->clk_scaling.enable)
		return;
	/* FUJITSU LIMITED:2015-01-23 H1510178 add end */
...
/* FUJITSU LIMITED:2015-05-14 H1520355 add start */
#ifdef CONFIG_MMC_PASSWORDS
	if (mmc_host_get_type(host) == HOST_TYPE_SD) { /* FUJITSU LIMITED:2015-01-23 H1510175 mod */
...
/* FUJITSU LIMITED:2015-02-18 H1510189 mod start */
	else if (mmc_host_get_type(host) == HOST_TYPE_SD) { /* FUJITSU LIMITED:2015-01-23 H1510175 mod */
...
/* FUJITSU LIMITED:2015-02-18 H1510189 mod end */
...
/* FUJITSU LIMITED:2015-05-14 H1520355 add start */
#ifdef CONFIG_MMC_PASSWORDS
int mmc_rescan_lock(struct mmc_host *host)
{
	int  ret = SDLOCCK_ERR;
	bool extend_wakelock = false;
	...
	host->detect_change = 1;
	mmc_bus_get(host);
	host->rescan_disable = 0; /* FUJITSU LIMITED:2015-05-14 H1520355 add */
	/*
	 * if there is a _removable_ card registered, check whether it is
	 * still present
	 */
	...
	mmc_rpm_hold(host, &host->class_dev);
	mmc_claim_host(host);
	ret = mmc_rescan_try_freq(host, host->f_min);
	if (!ret)
		extend_wakelock = true;
	mmc_release_host(host);
	mmc_rpm_release(host, &host->class_dev);
...
void mmc_start_host(struct mmc_host *host)
{
	...
	if (host->caps2 & MMC_CAP2_NO_PRESCAN_POWERUP)
		...
		if (mmc_host_get_type(host) != HOST_TYPE_SD) /* FUJITSU LIMITED:2015-02-18 H1510189 add */
			mmc_power_up(host);
	mmc_detect_change(host, 0);
}
...
/* FUJITSU LIMITED:2015-12-03 H1610017 add start */
void mmc_down_speed_mode(struct mmc_host *host)
{
	if (host->bus_ops && host->bus_ops->down_speed_mode)
		host->bus_ops->down_speed_mode(host);

	return;
}
EXPORT_SYMBOL(mmc_down_speed_mode);
/* FUJITSU LIMITED:2015-12-03 H1610017 add end */
...
int mmc_suspend_host(struct mmc_host *host)
{
	...
	mmc_bus_get(host);
/* FUJITSU LIMITED:2015-07-13 H1520393 add start */
	if (!mmc_try_claim_host(host)) {
		printk("%s:%s EBUSY\n", __func__, mmc_hostname(host));
		mmc_bus_put(host);
		return -EBUSY;
	}
/* FUJITSU LIMITED:2015-07-13 H1520393 add end */
	...
	mmc_release_host(host); /* FUJITSU LIMITED:2015-07-13 H1520393 add */
...
out:
	...
	if (!(host->card && mmc_card_sdio(host->card)))
		mmc_release_host(host);
	mmc_bus_put(host); /* FUJITSU LIMITED:2015-07-13 H1520393 add */
	return err;
}
...
/* FUJITSU LIMITED:2015-03-16 H1510175-1 mod start */
		if ((mmc_host_get_type(host) == HOST_TYPE_SD) ||
		    (mmc_host_get_type(host) == HOST_TYPE_SDIO)) {
/* FUJITSU LIMITED:2015-03-16 H1510175-1 mod end */
