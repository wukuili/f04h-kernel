...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 7) {
		pr_info("eMMC info LifeTime typeA:%u typeB:%u\n", 
...
/* FUJITSU LIMITED:2015-05-14 H1520355 add start */
	...
	if (err){
		SDLOG_ERR("CMD13: mmc_send_status() error. [err=%d]", err);
		goto free_card;
	}
...
/* FUJITSU LIMITED:2015-05-14 H1520355 mod start */
#ifndef CONFIG_MMC_PASSWORDS
		host->card->state &= ~(MMC_STATE_HIGHSPEED | MMC_STATE_HIGHSPEED_200);
#else
		host->card->state &= ~(MMC_STATE_HIGHSPEED | MMC_STATE_HIGHSPEED_200 | MMC_STATE_LOCKED);
#endif
/* FUJITSU LIMITED:2015-05-14 H1520355 mod end */
