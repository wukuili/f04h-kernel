...
/* FUJITSU LIMITED:2014-11-26 H1510093 add start */
	if (pack_stats->pack_stop_reason[WRITE_ALIGN]) {
		snprintf(temp_buf, TEMP_BUF_SIZE,
			 "%s: %d times: write alignment request\n",
			mmc_hostname(card->host),
			pack_stats->pack_stop_reason[WRITE_ALIGN]);
		strlcat(ubuf, temp_buf, cnt);
	}
...
static ssize_t mmc_life_time_stats_read(struct file *filp, char __user *ubuf,
				size_t cnt, loff_t *ppos)
{
	char *buf = filp->private_data;
	char *temp_buf;
	...
	sprintf(temp_buf, "eMMC info LifeTime typeA:%c%c typeB:%c%c\n",
	      buf[EXT_CSD_LIFE_TIME_EST_TYP_A*2], buf[EXT_CSD_LIFE_TIME_EST_TYP_A*2+1],
	      buf[EXT_CSD_LIFE_TIME_EST_TYP_B*2], buf[EXT_CSD_LIFE_TIME_EST_TYP_B*2+1]);

	ret = simple_read_from_buffer(ubuf, cnt, ppos, temp_buf, strlen(temp_buf));
...
static const struct file_operations mmc_dbg_life_time_stats_fops = {
	.open		= mmc_ext_csd_open,
	.read		= mmc_life_time_stats_read,
	.release	= mmc_ext_csd_release,
};
...
void mmc_add_card_debugfs(struct mmc_card *card)
{
	...
	/* FUJITSU LIMITED:2015-05-14 H1520351 add start */
	if (mmc_card_mmc(card) && (card->ext_csd.rev >= 7))
		if (!debugfs_create_file("life_time", S_IRUSR, root, card,
				 &mmc_dbg_life_time_stats_fops))
			goto err;
	/* FUJITSU LIMITED:2015-05-14 H1520351 add end */
