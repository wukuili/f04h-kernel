...
/* FUJITSU LIMITED:2015-01-23 H1510110-1 add start */
	if (mmc_host_get_type(card->host) == HOST_TYPE_MMC)
		kfree(card->work_buff);
/* FUJITSU LIMITED:2015-01-23 H1510110-1 add end */
	kfree(card);
}
...
/* FUJITSU LIMITED:2015-01-23 H1510110-1 add start */
	if (mmc_host_get_type(host) == HOST_TYPE_MMC) {
		card->work_buff = kmalloc(MMC_WORKBUFF_SIZE, GFP_KERNEL);
		if (!card->work_buff) {
			kfree(card);
			return ERR_PTR(-ENOMEM);
		}
...
int mmc_add_card(struct mmc_card *card)
{
	...
	if(mmc_host_get_type(card->host) != HOST_TYPE_SDIO) { /* FUJITSU LIMITED:2015-01-23 H1510175 add */
