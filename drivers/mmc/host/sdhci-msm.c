...
/* FUJITSU LIMITED:2015-02-20 H1510118-1 add start */
#include <linux/debugfs.h>
#include <linux/list.h>
#include <linux/pinctrl/pinconf-generic.h>
#include "../../pinctrl/core.h"
/* FUJITSU LIMITED:2015-02-20 H1510118-1 add end */
...
#define MSM_MMC_STATUS_DELAY		1000 /* msecs */
#define MSM_MMC_STATUS_INTERVAL		5    /* msecs */
#define MSM_MMC_STATUS_RETRY		200
#define MSM_MMC_STATUS_MACH_COUNT	5
...
/* FUJITSU LIMITED:2014-11-26 H1510096 add start */
#define MSM_MMC_VDD_DATA_OFF_TIME	1000 /* msecs */
/* FUJITSU LIMITED:2014-11-26 H1510096 add end */
/* FUJITSU LIMITED:2015-12-03 H1610016 add start */
#define MSM_MMC_DEFAULT_IDLE_TIMEOUT	5000 /* msecs */
/* FUJITSU LIMITED:2015-12-03 H1610016 add end */
...
#define MSM_PWR_IRQ_TIMEOUT_MS 2000 /* FUJITSU LIMITED:2015-12-03 H1610016 mod */
...
struct sdhci_msm_reg_data {
	...
	u32 low_vol_level;
	u32 high_vol_level;
	u32 middle_vol_level; /* FUJITSU LIMITED:2014-11-26 H1510092 add */
...
struct sdhci_msm_pltfm_data {
	...
	int is_status_gpio_active_low; /* FUJITSU LIMITED:2016-01-19 H16100337 add */
	int host_type;                 /* FUJITSU LIMITED:2015-01-23 H1510175 add */
	void * phost;                  /* FUJITSU LIMITED:2014-11-26 H1510096 add */
...
struct sdhci_msm_host {
	...
	struct device_attribute idle_timeout;	/* FUJITSU LIMITED:2014-11-26 H1510116 add */
...
/* FUJITSU LIMITED:2016-01-19 H16100337 add start */
	...
	int status_working_flg;
	struct wake_lock status_wlock;
/* FUJITSU LIMITED:2016-01-19 H16100337 add end */
/* FUJITSU LIMITED:2014-11-26 H1510096 add start */
	struct timespec vdd_off_start_ts;
/* FUJITSU LIMITED:2014-11-26 H1510096 add end */
/* FUJITSU LIMITED:2015-12-03 H1610016 add start */
...
static int sdhci_msm_runtime_resume(struct device *dev);	    /* FUJITSU LIMITED:2015-12-03 H1610016 add */
static int sdhci_msm_runtime_suspend(struct device *dev);       /* FUJITSU LIMITED:2015-12-03 H1610016 add */
static void sdhci_msm_add_debugfs(struct sdhci_msm_host *host);	/* FUJITSU LIMITED:2014-11-26 H1510118 add */

/* FUJITSU LIMITED:2015-01-23 H1510175 add start */
static int sdhci_msm_get_host_type(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;

	return msm_host->pdata->host_type;
}
/* FUJITSU LIMITED:2015-01-23 H1510175 add end */

/* FUJITSU LIMITED:2016-01-19 H16100337 add start */
static unsigned int sdhci_msm_get_cd(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;

	return !msm_host->eject;
}

static unsigned int sdhci_msm_cd_gpio_status(struct sdhci_msm_host *msm_host)
{
	int status;

	status = gpio_get_value_cansleep(msm_host->pdata->status_gpio);
	if (msm_host->pdata->is_status_gpio_active_low)
		status = !status;

	return status;
}

static void sdhci_check_status(struct sdhci_msm_host *msm_host)
{
	unsigned int status;

	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
		status = sdhci_msm_cd_gpio_status(msm_host);

		msm_host->eject = !status;

		if (status ^ msm_host->oldstat) {
			if (msm_host->pdata->is_status_gpio_active_low)
				pr_info("%s: Slot status change detected "
					"(%d -> %d) and the card detect GPIO"
					" is ACTIVE_LOW\n",
					mmc_hostname(msm_host->mmc),
					msm_host->oldstat, status);
			else
				pr_info("%s: Slot status change detected "
					"(%d -> %d) and the card detect GPIO"
					" is ACTIVE_HIGH\n",
...
void sdhci_msm_status_work_handler(struct work_struct *work)
{
	struct sdhci_msm_host *msm_host = container_of(work, struct sdhci_msm_host, status_work.work);
	int i;
	int mach_count = 0;
	unsigned int status;
	unsigned int oldstatus = EINVAL;

	pr_info("%s: %s: enter\n", mmc_hostname(msm_host->mmc), __func__);
	for (i = 0; i < MSM_MMC_STATUS_RETRY; i++) {
		if (i != 0) {
			msleep(MSM_MMC_STATUS_INTERVAL);
		}

		status = sdhci_msm_cd_gpio_status(msm_host);
		if (status == oldstatus) {
			mach_count++;
			if (mach_count >= (MSM_MMC_STATUS_MACH_COUNT - 1)) {
				pr_info("%s: %d consecutive read. read_count=%d slot_status=%d\n",
					mmc_hostname(msm_host->mmc), mach_count, i + 1, status);
				break;
			}
		} else {
			mach_count = 0;
	...
	if (i >= MSM_MMC_STATUS_RETRY) {
		pr_info("%s: non consecutive read. read_count=%d slot_status=%d\n",
			mmc_hostname(msm_host->mmc), i, status);
		status = 1;
	}
	...
	if (status ^ msm_host->oldstat) {
		if (msm_host->pdata->is_status_gpio_active_low)
			...
			pr_info("%s: Slot status change detected "
				...
				msm_host->oldstat, status);
		mmc_detect_change(msm_host->mmc, 0);
		mmc_detect_work_sync(msm_host->mmc);
	}
	msm_host->oldstat = status;

	msm_host->status_working_flg = 0;
	wake_unlock(&msm_host->status_wlock);
	pr_info("%s: %s: leave\n", mmc_hostname(msm_host->mmc), __func__);
}

irqreturn_t sdhci_msm_status_irq(int irq, void *dev_id)
{
	...
	pr_debug("%s: %d\n", __func__, irq);
	if (msm_host->status_working_flg == 0) {
		msm_host->status_working_flg = 1;
		wake_lock(&msm_host->status_wlock);
		queue_delayed_work(msm_host->status_workqueue, &msm_host->status_work, msecs_to_jiffies(MSM_MMC_STATUS_DELAY));
	} else {
		pr_debug("%s: interrupt skip. irq=%d\n", __func__, irq);
	}
	return IRQ_HANDLED;
}
/* FUJITSU LIMITED:2016-01-19 H16100337 add end */
...
int sdhci_msm_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	u32 tuning_status = 0; /* FUJITSU LIMITED:2014-11-26 H1510114 add */
...
retry:
	...
	do {
		...
		if (!cmd.error && !data.error &&
			...
			pr_debug("%s: %s: found *** good *** phase = %d\n",
				mmc_hostname(mmc), __func__, phase);
			tuning_status |= (1 << phase ); /* FUJITSU LIMITED:2014-11-26 H1510113 add */
		} else {
	...
	if (tuned_phase_cnt) {
		...
		/* FUJITSU LIMITED:2014-11-26 H1510113 mod start */
		pr_info("%s: %s: finally setting the tuning phase to %d  tuning_status:0x%x\n",
				mmc_hostname(mmc), __func__, phase, tuning_status);
		/* FUJITSU LIMITED:2014-11-26 H1510113 mod end */
	} else {
		if (--tuning_seq_cnt)
			goto retry;
		/* tuning failed */
...
/* FUJITSU LIMITED:2014-11-26 H1510092 mod start */
	if (prop && (len == (2 * sizeof(__be32)))) {
		vreg->low_vol_level = be32_to_cpup(&prop[0]);
		vreg->high_vol_level = be32_to_cpup(&prop[1]);
		vreg->middle_vol_level = 0;
	}else if (prop && (len == (3 * sizeof(__be32)))) {
		vreg->low_vol_level = be32_to_cpup(&prop[0]);
		vreg->high_vol_level = be32_to_cpup(&prop[1]);
		vreg->middle_vol_level = be32_to_cpup(&prop[2]);
	} else {
		dev_warn(dev, "%s %s property\n",
			prop ? "invalid format" : "no", prop_name);
	}
/* FUJITSU LIMITED:2014-11-26 H1510092 mod end */
...
/* FUJITSU LIMITED:2016-01-19 H16100337 add end */
/* FUJITSU LIMITED:2015-01-23 H1510175 add start */
	if (of_get_property(np, "fj,is-sdio", NULL))
		pdata->host_type = HOST_TYPE_SDIO;
	else if (of_get_property(np, "fj,is-sd", NULL))
		pdata->host_type = HOST_TYPE_SD;
	else
		pdata->host_type = HOST_TYPE_MMC;
/* FUJITSU LIMITED:2015-01-23 H1510175 add end */
...
static int sdhci_msm_vreg_enable(struct sdhci_msm_reg_data *vreg)
{
	...
	if (!vreg->is_enabled) {
/* FUJITSU LIMITED:2014-11-26 H1510092 add start */
		if(vreg->middle_vol_level) {
			/* Set middle voltage level */
			ret = sdhci_msm_vreg_set_voltage(vreg, vreg->middle_vol_level,
							vreg->middle_vol_level);
			if (ret)
				return ret;
			
			ret = regulator_enable(vreg->reg);
			if (ret) {
				pr_err("%s: regulator_enable(%s) middle failed. ret=%d\n",
						__func__, vreg->name, ret);
				return ret;
			}
		}
/* FUJITSU LIMITED:2014-11-26 H1510092 add end */
		/* Set voltage level */
...
/* FUJITSU LIMITED:2014-11-26 H1510092 add start */
		if(vreg->middle_vol_level) {
			...
			if (ret) {
				pr_err("%s: regulator_disable(%s) middle failed. ret=%d\n",
					__func__, vreg->name, ret);
				goto out;
			}
		}
...
/* FUJITSU LIMITED:2014-11-26 H1510096 add start */
	...
	int sleep_ms;
	struct sdhci_msm_host *msm_host = pdata->phost;
/* FUJITSU LIMITED:2014-11-26 H1510096 add end */
...
/* FUJITSU LIMITED:2014-11-26 H1510096 add start */
	if (msm_host->pdata->host_type == HOST_TYPE_SD) { /* FUJITSU LIMITED:2015-01-23 H1510175 mod */
		if ((enable) && (!curr_slot->vdd_data->is_enabled)) {
			getnstimeofday(&now_ts);
			end_ts = msm_host->vdd_off_start_ts;
			timespec_add_ns(&end_ts, MSM_MMC_VDD_DATA_OFF_TIME * NSEC_PER_MSEC);
			pr_debug("%s: vdd_data off->on. start_ts=%ld.%ld now_ts=%ld.%ld end_ts=%ld.%ld\n",
				mmc_hostname(msm_host->mmc),
				msm_host->vdd_off_start_ts.tv_sec, msm_host->vdd_off_start_ts.tv_nsec,
				now_ts.tv_sec, now_ts.tv_nsec,
				end_ts.tv_sec, end_ts.tv_nsec);

			if ((msm_host->vdd_off_start_ts.tv_sec == 0) && (msm_host->vdd_off_start_ts.tv_nsec == 0)) { /* init on? */
				pr_debug("%s: init skip\n", mmc_hostname(msm_host->mmc));
			} else if (timespec_compare(&msm_host->vdd_off_start_ts, &now_ts) > 0) { /* future time? */
				pr_err("%s: future time\n", mmc_hostname(msm_host->mmc));
				msleep(MSM_MMC_VDD_DATA_OFF_TIME);
			} else if (timespec_compare(&end_ts, &now_ts) > 0) { /* need to wait? */
				sleep_ts = timespec_sub(end_ts, now_ts);
				sleep_ms = sleep_ts.tv_sec * MSEC_PER_SEC + DIV_ROUND_UP(sleep_ts.tv_nsec, NSEC_PER_MSEC);
				pr_debug("%s: wait start. sleep_ts=%ld.%ld sleep_ms=%d\n",
					mmc_hostname(msm_host->mmc), sleep_ts.tv_sec, sleep_ts.tv_nsec, sleep_ms);
				msleep(sleep_ms);
				pr_debug("%s: wait end\n", mmc_hostname(msm_host->mmc));
			} else { /* time over? */
				pr_debug("%s: non wait\n", mmc_hostname(msm_host->mmc));
			}
		} else if ((!enable) && (curr_slot->vdd_data->is_enabled)) {
			getnstimeofday(&msm_host->vdd_off_start_ts);
			pr_debug("%s: vdd_data on->off. start_ts=%ld.%ld\n",
				mmc_hostname(msm_host->mmc), msm_host->vdd_off_start_ts.tv_sec, msm_host->vdd_off_start_ts.tv_nsec);
		}
	}
/* FUJITSU LIMITED:2014-11-26 H1510096 add end */
...
/* FUJITSU LIMITED:2014-11-26 H1510116 add start */
static ssize_t
		...
		const char *buf, size_t count)
{
	...
	if (!kstrtou32(buf, 0, &timeout)) {
		timeout *= 1000;
		spin_lock_irqsave(&host->lock, flags);
		if (timeout <= MSM_MMC_DEFAULT_IDLE_TIMEOUT)
			msm_host->idle_tout = MSM_MMC_DEFAULT_IDLE_TIMEOUT;
		else
			msm_host->idle_tout = timeout;
		spin_unlock_irqrestore(&host->lock, flags);
	}
	return count;
}
/* FUJITSU LIMITED:2014-11-26 H1510116 add end */
...
/* FUJITSU LIMITED:2015-12-03 H1610016 mod start */
	else if (!wait_for_completion_timeout(&msm_host->pwr_irq_completion,
				msecs_to_jiffies(MSM_PWR_IRQ_TIMEOUT_MS)))
			panic("%s: %s: pwr_irq completion timeout\n", mmc_hostname(host->mmc), __func__);
/* FUJITSU LIMITED:2015-12-03 H1610016 mod end */
...
/* FUJITSU LIMITED:2015-12-03 H1610016 add start */
static void sdhci_msm_platform_bus_voting(struct sdhci_host *host, u32 enable)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	struct mmc_host *mmc = host->mmc;
	...
	/* FUJITSU LIMITED:2015-01-23 H1510175 add start */
	if (msm_host->pdata->host_type == HOST_TYPE_SDIO)
		return;
	/* FUJITSU LIMITED:2015-01-23 H1510175 add end */

	if (enable) {
		if (msm_host->sdcc_suspended && msm_host->pending_resume) {
			msm_host->pending_resume = false;
			pm_runtime_get_noresume(dev);
			rc = sdhci_msm_runtime_resume(dev);
			goto out;
		}
...
static struct sdhci_ops sdhci_msm_ops = {
	...
	.get_cd = sdhci_msm_get_cd,								/* FUJITSU LIMITED:2016-01-19 H16100337 add */
	.get_host_type = sdhci_msm_get_host_type,				/* FUJITSU LIMITED:2015-01-23 H1510175 add */
	.platform_bus_voting = sdhci_msm_platform_bus_voting,	/* FUJITSU LIMITED:2015-12-03 H1610016 add */
};
...
/* FUJITSU LIMITED:2014-11-26 H1510096 add start */
	msm_host->vdd_off_start_ts.tv_sec = 0;
	msm_host->vdd_off_start_ts.tv_nsec = 0;
/* FUJITSU LIMITED:2014-11-26 H1510096 add end */
	/* Setup regulators */
	...
	msm_host->mmc->caps2 |= MMC_CAP2_INIT_BKOPS;		/* FUJITSU LIMITED:2016-01-21 H16100720 add */
	msm_host->mmc->pm_caps |= MMC_PM_KEEP_POWER | MMC_PM_WAKE_SDIO_IRQ;
	msm_host->mmc->caps2 |= MMC_CAP2_SANITIZE;

	if (msm_host->pdata->host_type == HOST_TYPE_SDIO) { /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		msm_host->mmc->caps2 |= MMC_CAP2_CORE_RUNTIME_PM; /* FUJITSU LIMITED:2015-12-03 H1610016 mod */
		msm_host->mmc->caps2 |= MMC_CAP2_CORE_PM;         /* FUJITSU LIMITED:2015-12-03 H1610016 mod */
		msm_host->mmc->caps2 |= MMC_CAP2_CLK_SCALE;       /* FUJITSU LIMITED:2014-11-26 H1510107 mod */
		msm_host->mmc->caps2 |= MMC_CAP2_STOP_REQUEST;    /* FUJITSU LIMITED:2014-11-26 H1510107 mod */
	}	/* FUJITSU LIMITED:2015-01-23 H1510175 add */
	...
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
/* FUJITSU LIMITED:2016-01-19 H16100337 add start */
		msm_host->oldstat = sdhci_msm_cd_gpio_status(msm_host);
...
/* FUJITSU LIMITED:2016-01-19 H16100337 add end */
		 ...
		 */
/*		usleep_range(10000, 10500); */ /* FUJITSU LIMITED:2015-05-14 H1520350 del */
...
/* FUJITSU LIMITED:2016-01-19 H16100337 add start */
		msm_host->status_workqueue = create_singlethread_workqueue("sdhci-msm_status");
		if (!msm_host->status_workqueue) {
			pr_err("%s: create status workqueue failed\n", __func__);
			goto free_cd_gpio;
		}
		INIT_DELAYED_WORK(&msm_host->status_work, sdhci_msm_status_work_handler);
		msm_host->status_working_flg = 0;
		pr_info("%s: create status workqueue done\n", __func__);
		wake_lock_init(&msm_host->status_wlock, WAKE_LOCK_SUSPEND, mmc_hostname(host->mmc));
/* FUJITSU LIMITED:2016-01-19 H16100337 add end */
	}
...
/* FUJITSU LIMITED:2015-12-03 H1610016 add end */
	...
	if (ret) {
		...
		/* FUJITSU LIMITED:2015-01-23 H1510175 mod start */
		if (msm_host->pdata->host_type == HOST_TYPE_SDIO)
			goto free_cd_gpio;
		else
		/* FUJITSU LIMITED:2015-01-23 H1510175 mod end */
			goto disable_pm_runtime; /* FUJITSU LIMITED:2015-12-03 H1610016 add */
	}

	if (msm_host->pdata->host_type != HOST_TYPE_SDIO) { /* FUJITSU LIMITED:2015-01-23 H1510175 add */
/* FUJITSU LIMITED:2014-11-26 H1510114 add start */
		ret = pre_mmc_alloc_sg(msm_host->mmc);
		if (ret)
			goto disable_pm_runtime;
/* FUJITSU LIMITED:2014-11-26 H1510114 add end */
	} /* FUJITSU LIMITED:2015-01-23 H1510175 add */
...
/* FUJITSU LIMITED:2014-11-26 H1510116 add start */
	if (msm_host->pdata->host_type != HOST_TYPE_SDIO) { /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		msm_host->idle_timeout.show = show_idle_timeout;
		msm_host->idle_timeout.store = store_idle_timeout;
		sysfs_attr_init(&msm_host->idle_timeout.attr);
		msm_host->idle_timeout.attr.name = "idle_timeout";
		msm_host->idle_timeout.attr.mode = S_IRUGO | S_IWUSR;
		ret = device_create_file(&pdev->dev, &msm_host->idle_timeout);
		if (ret)
...
/* FUJITSU LIMITED:2014-11-26 H1510116 add end */
	...
	sdhci_msm_add_debugfs(msm_host); /* FUJITSU LIMITED:2014-11-26 H1510118 add */
...
static int sdhci_msm_remove(struct platform_device *pdev)
{
	...
	pr_debug("%s: %s\n", dev_name(&pdev->dev), __func__);
	if (msm_host->pdata->host_type != HOST_TYPE_SDIO) /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		device_remove_file(&pdev->dev, &msm_host->idle_timeout); /* FUJITSU LIMITED:2014-11-26 H1510116 add */
	if (!gpio_is_valid(msm_host->pdata->status_gpio))
		device_remove_file(&pdev->dev, &msm_host->polling);
	device_remove_file(&pdev->dev, &msm_host->msm_bus_vote.max_bus_bw);
	sdhci_remove_host(host, dead);
...
/* FUJITSU LIMITED:2016-01-19 H16100337 mod start */
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
		mmc_gpio_free_cd(msm_host->mmc);
		wake_lock_destroy(&msm_host->status_wlock);
		cancel_delayed_work_sync(&msm_host->status_work);
	}
/* FUJITSU LIMITED:2016-01-19 H16100337 mod end */
	...
	if (msm_host->pdata->host_type != HOST_TYPE_SDIO) { /* FUJITSU LIMITED:2015-01-23 H1510175 add */
/* FUJITSU LIMITED:2014-11-26 H1510114 add start */
		kfree(msm_host->mmc->cur_sg_pool);
		kfree(msm_host->mmc->pre_sg_pool);
/* FUJITSU LIMITED:2014-11-26 H1510114 add end */
	} /* FUJITSU LIMITED:2015-01-23 H1510175 add */
...
/* FUJITSU LIMITED:2015-12-03 H1610016 add end */	
	...
	} else { /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		/* FUJITSU LIMITED:2015-12-03 H1610016 add start */
		if (mmc) {
			...
			if (!ret) {
				spin_lock_irqsave(&msm_host->lock, flags);
				msm_host->sdcc_suspended = true;
				spin_unlock_irqrestore(&msm_host->lock, flags);
			}
...
skip_disable_host_irq:
	...
	/* FUJITSU LIMITED:2015-01-23 H1510175 add start */
	if ((msm_host->pdata->host_type == HOST_TYPE_SDIO) ||
	    ((msm_host->pdata->host_type != HOST_TYPE_SDIO) && (!ret))) {
	/* FUJITSU LIMITED:2015-01-23 H1510175 add end */
		/*
	...
	if (msm_host->pdata->host_type == HOST_TYPE_SDIO) /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		ret = 0;                                      /* FUJITSU LIMITED:2015-01-23 H1510175 add */
...
/* FUJITSU LIMITED:2015-12-03 H1610016 add start */
static int sdhci_msm_runtime_idle(struct device *dev)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	...
	pm_schedule_suspend(dev, msm_host->idle_tout);
...
static int sdhci_msm_suspend(struct device *dev)
{
	...
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	unsigned long flags;	/* FUJITSU LIMITED:2014-11-26 H1510116 add */
	int ret = 0;
...
/* FUJITSU LIMITED:2015-12-03 H1610016 add end */
	...
	if (msm_host->pdata->host_type != HOST_TYPE_SDIO) /* FUJITSU LIMITED:2015-01-23 H1510175 add */
		msm_host->pending_resume = true; /* FUJITSU LIMITED:2015-12-03 H1610016 add */
	else /* FUJITSU LIMITED:2015-01-23 H1510175 add */
...
/*			   NULL) */					/* FUJITSU LIMITED:2015-12-03 H1610016 del */
			   sdhci_msm_runtime_idle)	/* FUJITSU LIMITED:2015-12-03 H1610016 add */
...
/* FUJITSU LIMITED:2014-11-26 H1510118 add start */
#define MAX_DRV_RW 32 
...
static int sdhci_msm_drv_open(struct inode *inode, struct file *filp)
{
	filp->private_data = inode->i_private;
	return 0;
}

static ssize_t sdhci_msm_drv_read(struct file *filp, char __user *ubuf,
		size_t cnt, loff_t *ppos)
{
	char kbuf[MAX_DRV_RW] = {0};
	int i;
	int index = 0;
	struct sdhci_msm_host *host = filp->private_data;
	int clk_val = -1, cmd_val = -1, data_val = -1;
	int *data_p;
	struct pinctrl_setting *setting;

	if (!host || !host->pdata || !host->pdata->pctrl_data)
		return -ENODEV;

	list_for_each_entry(setting, &host->pdata->pctrl_data->pins_active->settings, node) {
		pr_debug("num_configs %d type %d name %s\n", setting->data.configs.num_configs, setting->type, setting->dev_name);

		for (i = 0; i < setting->data.configs.num_configs; i++) {
			pr_debug("%d configs0x%x\n",i, (unsigned int)setting->data.configs.configs[i]);
			if (pinconf_to_config_param(setting->data.configs.configs[i]) == PIN_CONFIG_DRIVE_STRENGTH) {
				switch (index) {
				case SDC_CLK:
					data_p = &clk_val;
					break;
				...
				*data_p = pinconf_to_config_argument(setting->data.configs.configs[i]);
			}
		}
		index++;
	}
	sprintf(kbuf, "CLK:%dmA CMD:%dmA DATA:%dmA\n", clk_val, cmd_val, data_val);
	return simple_read_from_buffer(ubuf, cnt, ppos, kbuf, strlen(kbuf));
}

static ssize_t sdhci_msm_drv_write(struct file *filp,
		...
		loff_t *ppos)
{
	...
	simple_write_to_buffer(kbuf, sizeof(kbuf), ppos, ubuf, cnt);
    if (sscanf(kbuf, "%d %d %d", &clk, &cmd, &data) != 3)
		return -EINVAL;
	...
	list_for_each_entry(setting, &host->pdata->pctrl_data->pins_active->settings, node) {
		...
		for (i = 0; i < setting->data.configs.num_configs; i++) {
			...
			setting->data.configs.configs[i] = PIN_CONF_PACKED(setting->data.configs.configs[i], conf_data);
			pr_debug("after %d configs0x%x\n",i, (unsigned int)setting->data.configs.configs[i]);
		}
		index++;
	}

	sdhci_msm_setup_pins(host->pdata, false);
	sdhci_msm_setup_pins(host->pdata, true);

	return cnt;
}

static const struct file_operations sdhci_msm_dbg_drv_fops = {
	.open = sdhci_msm_drv_open,
	.read = sdhci_msm_drv_read,
	.write = sdhci_msm_drv_write,
};

static void sdhci_msm_add_debugfs(struct sdhci_msm_host *host)
{
	struct mmc_host *mmc = host->mmc;
	if (!mmc || ! mmc->debugfs_root)
		return;

	debugfs_create_file("drv", S_IRUGO | S_IWUSR, mmc->debugfs_root, host, &sdhci_msm_dbg_drv_fops);
}
/* FUJITSU LIMITED:2014-11-26 H1510118 add end */
