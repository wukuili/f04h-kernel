		   ...
		   spinlock_t *lock, const char *subname)
{
	...
	if (!mqrq_cur->bounce_buf && !mqrq_prev->bounce_buf) {
/*		unsigned int max_segs = host->max_segs;  */ /* FUJITSU LIMITED:2014-11-26 H1510114 del */

		blk_queue_bounce_limit(mq->queue, limit);
		blk_queue_max_hw_sectors(mq->queue,
			min(host->max_blk_count, host->max_req_size / 512));
		blk_queue_max_segment_size(mq->queue, host->max_seg_size);
/* retry: */ /* FUJITSU LIMITED:2014-11-26 H1510114 del */
		blk_queue_max_segments(mq->queue, host->max_segs);

/* FUJITSU LIMITED:2014-11-26 H1510114 add start */
		mqrq_cur->sg = 	host->cur_sg_pool;
		mqrq_prev->sg = host->pre_sg_pool;
		goto success;
/* FUJITSU LIMITED:2014-11-26 H1510114 add end */
/* FUJITSU LIMITED:2014-11-26 H1510114 del start */
#if 0
...
#endif
/* FUJITSU LIMITED:2014-11-26 H1510114 del end */
	}
...
#ifdef CONFIG_MMC_BLOCK_BOUNCE /* FUJITSU LIMITED:2014-11-26 H1510114 add */
 cleanup_queue:
#endif                         /* FUJITSU LIMITED:2014-11-26 H1510114 add */
/*	kfree(mqrq_cur->sg);  */   /* FUJITSU LIMITED:2014-11-26 H1510114 del */
	mqrq_cur->sg = NULL;
	kfree(mqrq_cur->bounce_buf);
	mqrq_cur->bounce_buf = NULL;

/*	kfree(mqrq_prev->sg); */   /* FUJITSU LIMITED:2014-11-26 H1510114 del */
	mqrq_prev->sg = NULL;
...
/* FUJITSU LIMITED:2014-11-26 H1510114 add start */
int pre_mmc_alloc_sg(struct mmc_host *host)
{
	int ret = 0;
	
	host->cur_sg_pool = mmc_alloc_sg(host->max_segs, &ret);
	if (!ret) {
		host->pre_sg_pool= mmc_alloc_sg(host->max_segs, &ret);
		if (ret)
			kfree(host->cur_sg_pool);
	}
	return ret;
}
/* FUJITSU LIMITED:2014-11-26 H1510114 add end */
