...
/* FUJITSU LIMITED:2014-11-26 H1510093 add start */
#define MMC_BLK_CHECK_WRITE_ALIGN(arg, blocks) 		((blocks <= 0x10) && (arg & 0xf) && (((arg & 0xf) + blocks) > 0x10))
/* FUJITSU LIMITED:2014-11-26 H1510093 add end */
/* FUJITSU LIMITED:2014-11-26 H1510106 add start */
/* FUJITSU LIMITED:2015-12-24 H16100342 mod start */
#define MMC_FJ_ERASE_ARG	MMC_SECURE_ERASE_ARG
#define MMC_FJ_TRIM_ARG		MMC_SECURE_TRIM1_ARG
/* FUJITSU LIMITED:2015-12-24 H16100342 mod end */
/* FUJITSU LIMITED:2014-11-26 H1510106 add end */
...
/* FUJITSU LIMITED:2014-11-26 H1510101 mod start */
	/*
	...
	if (bdev != bdev->bd_contains)
		return -EPERM;
/* FUJITSU LIMITED:2014-11-26 H1510101 mod end */

	idata = mmc_blk_ioctl_copy_from_user(ic_ptr);
	if (IS_ERR_OR_NULL(idata))
		/* return PTR_ERR(idata); */                       /* FUJITSU LIMITED:2016-01-19 H16100337 del */
		return !PTR_ERR(idata) ? -EINVAL : PTR_ERR(idata); /* FUJITSU LIMITED:2016-01-19 H16100337 add */
	...
	if (IS_ERR_OR_NULL(card)) {
		/* err = PTR_ERR(card); */                      /* FUJITSU LIMITED:2016-01-19 H16100337 del */
		err = !PTR_ERR(card) ? -ENODEV : PTR_ERR(card); /* FUJITSU LIMITED:2016-01-19 H16100337 add */
		goto cmd_done;
	}
	...
	struct mmc_ioc_rpmb __user *ic_ptr)
{
	...
	md = mmc_blk_get(bdev->bd_disk);
/* FUJITSU LIMITED:2014-11-26 H1510117 mod start */
...
/* FUJITSU LIMITED:2014-11-26 H1510117 mod end */
...
cmd_done:
	mmc_blk_put(md);
cmd_err: /* FUJITSU LIMITED:2014-11-26 H1510117 add */
	return err;
}
...
/* FUJITSU LIMITED:2014-11-26 H1510104 add start */
static int mmc_blk_issue_secdiscard_rq_fj(struct mmc_queue *mq,
				       struct request *req, unsigned int from, unsigned int nr)
{
	struct mmc_blk_data *md = mq->data;
	struct mmc_card *card = md->queue.card;
	...
	unsigned int from_rem, nr_rem, count = 0, i = 0;

	memset(&param, 0, sizeof(param));

	do {
		from_rem = from % card->erase_size;

		if (from_rem > 0) {
			param[count].from = from;
			param[count].nr   = card->erase_size - from_rem;
			if (param[count].nr > nr) {
				param[count].nr = nr;
			}
			param[count].arg  = MMC_FJ_TRIM_ARG;	/* FUJITSU LIMITED:2014-11-26 H1510106 mod */
			count++;

			if (param[count-1].nr == nr) {
				break;
			}
			param[count].from = from + param[count-1].nr;
			param[count].nr   = nr - param[count-1].nr;

			if (param[count].nr < card->erase_size) {
				param[count].arg = MMC_FJ_TRIM_ARG;	/* FUJITSU LIMITED:2014-11-26 H1510106 mod */
				count++;
				break;
			}
			nr_rem = param[count].nr % card->erase_size;
			param[count].nr = param[count].nr - nr_rem;
			param[count].arg = MMC_FJ_ERASE_ARG;	/* FUJITSU LIMITED:2014-11-26 H1510106 mod */
			count++;
			if (nr_rem == 0) {
				break;
			}
			param[count].from = param[count-1].from + param[count-1].nr;
			param[count].nr = nr_rem;
			param[count].arg = MMC_FJ_TRIM_ARG;		/* FUJITSU LIMITED:2014-11-26 H1510106 mod */
			count++;

		} else {
			if (nr < card->erase_size) {
				param[count].from = from;
				param[count].nr   = nr;
				param[count].arg  = MMC_FJ_TRIM_ARG; 
				count++;
				break;
			}
			nr_rem = nr % card->erase_size;

			param[count].from = from;
			param[count].nr   = nr - nr_rem;
			param[count].arg  = MMC_FJ_ERASE_ARG;	/* FUJITSU LIMITED:2014-11-26 H1510106 mod */
			count++;
...
retry:
	for (; i < count; i++) {
		if (card->quirks & MMC_QUIRK_INAND_CMD38) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
					 INAND_CMD38_ARG_EXT_CSD,
					 param[i].arg == MMC_SECURE_TRIM1_ARG ?
		...
		err = mmc_erase(card, param[i].from, param[i].nr, param[i].arg);
		if (err == -EIO)
			goto out_retry;
		if (err)
			goto out;

		if (param[i].arg == MMC_SECURE_TRIM1_ARG) {
			if (card->quirks & MMC_QUIRK_INAND_CMD38) {
				err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
						 INAND_CMD38_ARG_EXT_CSD,
			...
			err = mmc_erase(card, param[i].from, param[i].nr, MMC_SECURE_TRIM2_ARG);
			if (err == -EIO)
				goto out_retry;
			if (err)
				goto out;
...
/* FUJITSU LIMITED:2014-11-26 H1510104 add end */
...
/*		arg = MMC_SECURE_TRIM1_ARG; */                            /* FUJITSU LIMITED:2014-11-26 H1510104 del */
		return mmc_blk_issue_secdiscard_rq_fj(mq, req, from, nr); /* FUJITSU LIMITED:2014-11-26 H1510104 add */
	else
		arg = MMC_FJ_ERASE_ARG;  /* FUJITSU LIMITED:2014-11-26 H1510106 mod */
...
/* FUJITSU LIMITED:2014-11-26 H1510093 add start */
	...
	if ((mmc_card_mmc(card)) && (rq_data_dir(req) == WRITE)) {
		if (MMC_BLK_CHECK_WRITE_ALIGN(brq->cmd.arg, brq->data.blocks)) {
			brq->data.blocks = 0x10 - (brq->cmd.arg & 0xf);
		}
	}
/* FUJITSU LIMITED:2014-11-26 H1510093 add end */
...
/* FUJITSU LIMITED:2014-11-26 H1510093 add start */
		blocks = blk_rq_sectors(cur);
		arg = blk_rq_pos(cur);
		if (MMC_BLK_CHECK_WRITE_ALIGN(arg, blocks)) {
			goto no_packed;
		}
/* FUJITSU LIMITED:2014-11-26 H1510093 add end */
	}
...
/* FUJITSU LIMITED:2014-11-26 H1510093 add start */
			blocks = blk_rq_sectors(next);
			arg = blk_rq_pos(next);
			if (MMC_BLK_CHECK_WRITE_ALIGN(arg, blocks)) {
				MMC_BLK_UPDATE_STOP_REASON(stats, WRITE_ALIGN);
				put_back = 1;
				break;
			}
/* FUJITSU LIMITED:2014-11-26 H1510093 add end */
...
/*		md->flags |= MMC_BLK_REL_WR; */ /* FUJITSU LIMITED:2014-11-26 H1510094 del */
