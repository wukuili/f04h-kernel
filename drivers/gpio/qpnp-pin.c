...
/* FUJITSU LIMITED:2015-11-17 H1610003 add start */
	{
		...
		if (check_and_get_table_from_NV() & BeforeInitDriver_LogType) {
			for (i = 0; i < spmi->num_dev_node; i++) {
				q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
				if (WARN_ON(!q_spec)) {
					rc = -ENODEV;
...
/* FUJITSU LIMITED:2015-11-17 H1610003 add start */
void qpnp_pin_gpio_mpp_dbg_out(int detailflag)
{
	int i;
	int state;
	struct qpnp_pin_chip *q_chip;
	...
	list_for_each_entry(q_chip, &qpnp_pin_chips, chip_list) {
		...
		/* check parameter. */
		if (gpio_chip == NULL) {
			printk("qpnp_pin_gpio_mpp_dbg_out error continue\n");
			continue;
		}
		dev = gpio_chip->dev;
		...
		printk("GPIOs %d-%d, %s/%s, %s, %s:\n",
			...
			dev ? dev_name(dev) : "",
			gpio_chip->label ? gpio_chip->label : "",
			gpio_chip->can_sleep ? ", can sleep" : "");
		...
		for (i = 0; i < gpio_chip->ngpio; i++) {
		    ...
		    state = qpnp_pin_get(gpio_chip, i);
			if (q_spec->type == Q_GPIO_TYPE) {
				printk("GPIO no=%4d. state=%s \n", q_spec->pmic_pin, state ? "hi" : "lo");
			} else {
				printk("MPP no=%4d. state=%s \n", q_spec->pmic_pin, state ? "hi" : "lo");
			}
			...
			param.cs_out    = q_reg_get(&q_spec->regs[Q_REG_I_SINK_CTL],
						       Q_REG_CS_OUT_SHIFT,
						       Q_REG_CS_OUT_MASK);
			printk("mode %d, output_type %d, invert %d, pull %d,\n", param.mode, param.output_type, param.invert, param.pull);
			printk("vin_sel %d, out_strength %d, src_sel %d, master_en %d,\n", param.vin_sel, param.out_strength, param.src_sel, param.master_en);
			printk("aout_ref %d, ain_route %d, cs_out %d\n", param.aout_ref, param.ain_route, param.cs_out);
			...
			if (q_spec->type == Q_GPIO_TYPE) {
				...
				case 0:
					...
					break;
				case 1:
					printk("OUT_BUF_OPEN_DRAIN_NMOS,");
					break;
				case 2:
					printk("OUT_BUF_OPEN_DRAIN_PMOS,");
					break;
				default:
					printk("OUTPUT_TYPE:?,");
					break;
				...
				switch (param.src_sel) {
				case 0:
					printk("SEL_FUNC_CONSTANT,");
					break;
				case 1:
					printk("SEL_FUNC_PAIRED,");
					break;
				case 2:
					printk("SEL_FUNC_1,");
					break;
				...
				switch (param.pull) {
				case 0:
					printk("GPIO_PULL_UP_30,");
					break;
				case 1:
					printk("GPIO_PULL_UP_1P5,");
					break;
				case 2:
					printk("GPIO_PULL_UP_31P5,");
					break;
				case 3:
					printk("GPIO_PULL_UP_1P5_30,");
					break;
				case 4:
					printk("GPIO_PULL_DN,");
					break;
				...
				case 1:
					printk("OUT_STRENGTH_LOW,");
					break;
				case 2:
					printk("OUT_STRENGTH_MED,");
					break;
				case 3:
					printk("OUT_STRENGTH_HIGH,");
					break;
				default:
					printk("OUTPUT_DRV_SELL:?,");
					break;
				}
			}
			else if (q_spec->type == Q_MPP_TYPE) {
				...
				switch (param.pull) {
				case 0:
					printk("MPP_PULL_UP_0P6KOHM,");
					break;
				case 1:
					printk("MPP_PULL_UP_OPEN,");
					break;
				case 2:
					printk("MPP_PULL_UP_10KOHM,");
					break;
				case 3:
					printk("MPP_PULL_UP_30KOHM,");
					break;
				default:
					printk("PULLUP_SEL:?,");
					break;
				...
				if ((param.mode == 0) || (param.mode == 1) || (param.mode == 2)) {
					...
					case 3:
						...
						break;
					case 4:
						printk("AIN_AMUX_ABUS1,");
						break;
					case 5:
						printk("AIN_AMUX_ABUS2,");
						break;
					case 6:
						printk("AIN_AMUX_ABUS3,");
						break;
					case 7:
						printk("AIN_AMUX_ABUS4,");
						break;
					default:
						printk("AIN_ROUTE_SEL:?,");
						break;
					}
				}
...
EXPORT_SYMBOL(qpnp_pin_gpio_mpp_dbg_out);
/* FUJITSU LIMITED:2015-11-17 H1610003 add end */
