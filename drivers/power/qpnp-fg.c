...
/* FCNT LIMITED:2016-02-17 CHARGER mod start */
#if 0
    ...
    SETTING(DELTA_SOC,       0x450,   3,      1),
#endif
/* FCNT LIMITED:2016-02-17 CHARGER mod end */
...
/* FCNT LIMITED:2016-02-17 CHARGER add start */
/* FCNT LIMITED:2017-01-18 CHARGER mod start */
#if 0
static int capacity_convert[256] =
{
/*   00  01  02  03  04  05  06  07  08  09  0A  0B  0C  0D  0E  0F                   */
      0,  0,  0,  1,  1,  1,  2,  2,  2,  3,  3,  3,  4,  4,  4,  5,    /* 0x00Â`0x0F */
      5,  6,  6,  6,  7,  7,  8,  8,  9,  9, 10, 10, 10, 11, 11, 12,    /* 0x10Â`0x1F */
     12, 13, 13, 14, 14, 15, 15, 16, 16, 17, 17, 18, 18, 19, 19, 20,    /* 0x20Â`0x2F */
     20, 21, 21, 22, 22, 23, 23, 23, 24, 24, 25, 25, 26, 26, 27, 27,    /* 0x30Â`0x3F */
     28, 28, 29, 29, 30, 30, 31, 31, 31, 32, 32, 33, 33, 34, 34, 35,    /* 0x40Â`0x4F */
     35, 35, 36, 36, 37, 37, 38, 38, 38, 39, 39, 40, 40, 40, 41, 41,    /* 0x50Â`0x5F */
     42, 42, 43, 43, 43, 44, 44, 45, 45, 45, 46, 46, 46, 47, 47, 48,    /* 0x60Â`0x6F */
     48, 48, 49, 49, 50, 50, 50, 51, 51, 52, 52, 52, 53, 53, 53, 54,    /* 0x70Â`0x7F */
     54, 55, 55, 55, 56, 56, 56, 57, 57, 58, 58, 58, 59, 59, 59, 60,    /* 0x80Â`0x8F */
     60, 61, 61, 61, 62, 62, 63, 63, 63, 64, 64, 64, 65, 65, 66, 66,    /* 0x90Â`0x9F */
     66, 67, 67, 67, 68, 68, 69, 69, 69, 70, 70, 70, 71, 71, 72, 72,    /* 0xA0Â`0xAF */
     72, 73, 73, 73, 74, 74, 75, 75, 75, 76, 76, 76, 77, 77, 77, 78,    /* 0xB0Â`0xBF */
     78, 79, 79, 79, 80, 80, 80, 81, 81, 81, 82, 82, 83, 83, 83, 84,    /* 0xC0Â`0xCF */
     84, 84, 85, 85, 85, 86, 86, 86, 87, 87, 87, 88, 88, 88, 89, 89,    /* 0xD0Â`0xDF */
     89, 90, 90, 90, 91, 91, 91, 92, 92, 92, 93, 93, 93, 94, 94, 94,    /* 0xE0Â`0xEF */
     95, 95, 95, 96, 96, 96, 97, 97, 97, 97, 98, 98, 98, 99, 99,100,    /* 0xF0Â`0xFF */
};
#else
static int capacity_convert[256] =
{
/*   00  01  02  03  04  05  06  07  08  09  0A  0B  0C  0D  0E  0F                   */
      0,  0,  0,  1,  1,  1,  2,  2,  2,  3,  3,  3,  4,  4,  4,  5,    /* 0x00`0x0F */
      5,  6,  6,  6,  7,  7,  8,  8,  9,  9, 10, 10, 10, 11, 11, 12,    /* 0x10`0x1F */
     12, 13, 13, 14, 14, 15, 15, 16, 16, 17, 17, 18, 18, 19, 19, 20,    /* 0x20`0x2F */
     20, 21, 21, 22, 22, 23, 23, 23, 24, 24, 25, 25, 26, 26, 27, 27,    /* 0x30`0x3F */
     28, 28, 29, 29, 30, 30, 31, 31, 31, 32, 32, 33, 33, 34, 34, 35,    /* 0x40`0x4F */
     35, 35, 36, 36, 37, 37, 38, 38, 38, 39, 39, 40, 40, 40, 41, 41,    /* 0x50`0x5F */
     42, 42, 43, 43, 43, 44, 44, 45, 45, 45, 46, 46, 46, 47, 47, 48,    /* 0x60`0x6F */
     48, 48, 49, 49, 50, 50, 50, 51, 51, 52, 52, 52, 53, 53, 53, 54,    /* 0x70`0x7F */
     54, 55, 55, 55, 56, 56, 56, 57, 57, 58, 58, 58, 59, 59, 59, 60,    /* 0x80`0x8F */
     60, 61, 61, 61, 62, 62, 63, 63, 63, 64, 64, 64, 65, 65, 66, 66,    /* 0x90`0x9F */
     66, 67, 67, 67, 68, 68, 69, 69, 69, 70, 70, 70, 71, 71, 72, 72,    /* 0xA0`0xAF */
     72, 73, 73, 73, 74, 74, 75, 75, 75, 76, 76, 76, 77, 77, 77, 78,    /* 0xB0`0xBF */
     78, 79, 79, 79, 80, 80, 80, 81, 81, 81, 82, 82, 83, 83, 84, 84,    /* 0xC0`0xCF */
     84, 85, 85, 86, 86, 86, 87, 87, 88, 88, 88, 89, 89, 90, 90, 90,    /* 0xD0`0xDF */
     91, 91, 92, 92, 92, 93, 93, 94, 94, 94, 95, 95, 96, 96, 96, 97,    /* 0xE0`0xEF */
     97, 98, 98, 98, 99, 99,100,100,100,100,100,100,100,100,100,100,    /* 0xF0`0xFF */
};
#endif
/* FCNT LIMITED:2017-01-18 CHARGER mod end */
/* FCNT LIMITED:2016-02-17 CHARGER add end */
...
/* FUJITSU LIMITED:2016-01-08 CHARGER mod start */
#if 0
	if (msoc == 0)
		return EMPTY_CAPACITY;
	...
	if ((msoc >= 0) && (msoc <= FULL_SOC_RAW)) {
		pr_info("[%s]_capacity:%d msoc:0x%02x\n", __func__, capacity_convert[msoc], msoc);
		return capacity_convert[msoc];
	} else {
		pr_info("[%s]_msoc:0x%02x\n", __func__, msoc);
		return DIV_ROUND_CLOSEST((msoc - 1) * (FULL_CAPACITY - 2), FULL_SOC_RAW - 2) + 1;
	}
#endif
/* FUJITSU LIMITED:2016-01-08 CHARGER mod end */
}
...
/* FCNT LIMITED:2016-06-14 CHARGER mod start */
#if 0
	if (chip->learning_data.learned_cc_uah == 0) {
#else
	if (chip->learning_data.learned_cc_uah != chip->nom_cap_uah) {
#endif
/* FCNT LIMITED:2016-06-14 CHARGER mod end */
...
static int fg_do_restart(struct fg_chip *chip, bool write_profile)
{
	...
	u8 reg = 0;
	u8 buf[2];
	int tries = 0;	/* FCNT LIMITED:2016-06-02 CHARGER add */
...
/* FCNT LIMITED:2016-06-02 CHARGER mod start */
#if 0
...
#endif
/* FCNT LIMITED:2016-06-02 CHARGER mod end */
...
static char fg_reg_buffer[] = "0000";
static int qpnp_set_fg_reg(const char *buffer, struct kernel_param *kp)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	u8 val1 = 0, val2 = 0;
	int reg1, reg2, reg_base;
	char input[3] = { 0 };

	if (chip == NULL) {
		pr_err("cannot find \"bms\"\n");
		return 0;
	}

	if (strlen(buffer) >= 6) {
		strncpy(input, buffer, 2);
		sscanf(input, "%x", &reg_base);
		strncpy(input, buffer+2, 2);
		sscanf(input, "%x", &reg1);
		strncpy(input, buffer+4, 2);
		sscanf(input, "%x", &reg2);

		fg_read(chip, &val1, (reg_base << 8) + reg1, 1);
		fg_read(chip, &val2, (reg_base << 8) + reg2, 1);
	}

	sprintf(fg_reg_buffer, "%02x%02x", val1, val2);
	return 0;
}

static int qpnp_get_fg_reg(char *buffer, struct kernel_param *kp)
{
	return sprintf(buffer, fg_reg_buffer);
}
module_param_call(fg_reg, qpnp_set_fg_reg, qpnp_get_fg_reg, NULL, 0644);

static int qpnp_fg_soc(char *buffer, struct kernel_param *kp)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	u8 cap[2];
	int result = 0, rc = 0, capacity = 0, tries = 0;
	...
	while (tries < MAX_TRIES_SOC) {
		rc = fg_read(chip, cap, chip->soc_base + SOC_MONOTONIC_SOC, 2);
		if (rc) {
			pr_err("spmi read failed: addr=%03x, rc=%d\n",
				chip->soc_base + SOC_MONOTONIC_SOC, rc);
			return rc;
	...
	result = sprintf(buffer, "%02x%02x", capacity, 0);

	return result;
}
module_param_call(fg_soc, NULL, qpnp_fg_soc, NULL, 0444);

static int qpnp_fg_batt_volt(char *buffer, struct kernel_param *kp)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	int result = 0;
	...
	rc = fg_mem_read(chip, reg, 0x5CC, 2, 1, 1);
	result = sprintf(buffer, "%02x%02x", reg[1], reg[0]);

	return result;
}
module_param_call(fg_batt_volt, NULL, qpnp_fg_batt_volt, NULL, 0444);

static int qpnp_fg_batt_temp(char *buffer, struct kernel_param *kp)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	int result = 0;
	...
	rc = fg_mem_read(chip, reg, 0x550, 2, 2, 1);
	result = sprintf(buffer, "%02x%02x", reg[1], reg[0]);

	return result;
}
module_param_call(fg_batt_temp, NULL, qpnp_fg_batt_temp, NULL, 0444);

static int qpnp_fg_bat_id(char *buffer, struct kernel_param *kp)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	int result = 0;
	...
	rc = fg_mem_read(chip, reg, 0x594, 2, 1, 1);
	result = sprintf(buffer, "%02x%02x", reg[1], reg[0]);

	return result;
}
module_param_call(fg_bat_id, NULL, qpnp_fg_bat_id, NULL, 0444);

static int qpnp_fg_usb_id(char *buffer, struct kernel_param *kp)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	int result = 0;
	...
	rc = fg_mem_read(chip, reg, 0x598, 2, 0, 1);
	result = sprintf(buffer, "%02x%02x", reg[1], reg[0]);

	return result;
}
module_param_call(fg_usb_id, NULL, qpnp_fg_usb_id, NULL, 0444);
