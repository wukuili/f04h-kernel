...
/* FCNT LIMITED:2017-01-27 H16200969 add start */
#include <linux/gpio_keys.h>
/* FCNT LIMITED:2017-01-27 H16200969 add end */
...
/* FCNT LIMITED:2017-01-27 H16200969 add start */
	...
	is_talk = get_gpio_keys_call_status();
#endif
/* FCNT LIMITED:2017-01-27 H16200969 add end */
...
/* FCNT LIMITED:2017-01-27 H16200969 add start */
		if (unlikely(is_talk)) {
			...
			if (irq_save == irq) {
				irq_cnt++;
				if (irq_cnt > 60) {
					panic("irq triggered not correct.");
				}
			}
		}
/* FCNT LIMITED:2017-01-27 H16200969 add end */
...
/* FCNT LIMITED:2017-01-27 H16200969 add start */
		if (unlikely(irq_save != -1)) {
			if (irq_save == irq) {
				irq_save = -1;
				if (irq_cnt >= 15) {
					pr_warn("%d irq_safe_clear cnt:%d [0x%01x, 0x%02x,0x%01x]\n",
							irq, irq_cnt, spec->slave, spec->per, spec->irq);
				}
				irq_cnt = 0;
			}
		}
