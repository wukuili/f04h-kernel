...
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add start
#define WLAN_VREG_ANTENNA_NAME	"vdd-wlan-antenna"
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add end
...
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add start
#define WLAN_VREG_ANTENNA_MAX	2800000
#define WLAN_VREG_ANTENNA_MIN	2800000
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add end
...
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add start
	struct regulator *wlan_reg_antenna;
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add end
	bool state;
};
...
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add start
	if (of_get_property(pdev->dev.of_node,
		WLAN_VREG_ANTENNA_NAME"-supply", NULL)) {
		vreg_info->wlan_reg_antenna =
			regulator_get(&pdev->dev, WLAN_VREG_ANTENNA_NAME);
		if (!IS_ERR(vreg_info->wlan_reg_antenna)) {
			ret = regulator_set_voltage(vreg_info->wlan_reg_antenna,
				WLAN_VREG_ANTENNA_MIN, WLAN_VREG_ANTENNA_MAX);
			if (ret) {
				pr_err("%s: Set wlan_reg_antenna failed!\n",
					__func__);
				goto err_reg_antenna_set;
			}
#ifdef WLAN_2ND_ANTENNA_CTRL
// Not Enable
#else
			ret = regulator_enable(vreg_info->wlan_reg_antenna);
			if (ret) {
				pr_err("%s: Enable wlan_reg_antenna failed!\n",
					__func__);
				goto err_reg_antenna_enable;
			}
#endif //WLAN_2ND_ANTENNA_CTRL
		}
		pr_err("%s: Set wlan_reg_antenna complete\n",__func__);
	}
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add end
...
err_reg_get2:
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] mod start
//	if (vreg_info->wlan_reg_sp2t)
//		regulator_disable(vreg_info->wlan_reg_sp2t);
	if (vreg_info->wlan_reg_antenna)
		regulator_disable(vreg_info->wlan_reg_antenna);

#ifdef WLAN_2ND_ANTENNA_CTRL
// Not Enable
#else //WLAN_2ND_ANTENNA_CTRL
err_reg_antenna_enable:
#endif //WLAN_2ND_ANTENNA_CTRL
	if (vreg_info->wlan_reg_antenna)
		regulator_put(vreg_info->wlan_reg_antenna);

err_reg_antenna_set:
	if (vreg_info->wlan_reg_sp2t)
		regulator_disable(vreg_info->wlan_reg_sp2t);
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] mod end
...
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add start
#ifdef WLAN_2ND_ANTENNA_CTRL
int cnss_wlan_2nd_antenna_on( void )
{
	struct cnss_wlan_vreg_info *vreg_info;

	if (!penv){
		pr_err("cnss_wlan_2nd_antenna_on --------> ENODEV ERR1");
		return -ENODEV;
	}
	vreg_info = &penv->vreg_info;

	if (!IS_ERR_OR_NULL(vreg_info->wlan_reg_antenna))
	{
		pr_debug("cnss_wlan_2nd_antenna_on --------> OK");
		return regulator_enable(vreg_info->wlan_reg_antenna);
	}
	pr_err("cnss_wlan_2nd_antenna_on --------> ENODEV ERR2");
	return -ENODEV;
}
EXPORT_SYMBOL(cnss_wlan_2nd_antenna_on);

int cnss_wlan_2nd_antenna_off( void )
{
	struct cnss_wlan_vreg_info *vreg_info;

	if (!penv) {
		pr_err("cnss_wlan_2nd_antenna_off --------> ENODEV ERR1");
		return -ENODEV;
	}
	vreg_info = &penv->vreg_info;
	...
	if (!IS_ERR_OR_NULL(vreg_info->wlan_reg_antenna))
	{
		pr_debug("cnss_wlan_2nd_antenna_off --------> OK");
		return regulator_disable(vreg_info->wlan_reg_antenna);
	}
	pr_err("cnss_wlan_2nd_antenna_off --------> ENODEV ERR2");
	return -ENODEV;
}
EXPORT_SYMBOL(cnss_wlan_2nd_antenna_off);
#endif	//WLAN_2ND_ANTENNA_CTRL
//FUJITSU:2015-05-22 CONNECTIVITY(WLAN) [V002] add end
