...
/* FCNT LIMITED:2016-02-23 H16100736 add start */
static bool in_call_suspend = false;
module_param(in_call_suspend, bool ,0640);
/* FCNT LIMITED:2016-02-23 H16100736 add end */
...
void qpnp_system_pwr_off_shipmode(void)
{
	struct qpnp_pon *pon;
	struct qpnp_pon *tmp;
	u8 reg;
	...
	list_for_each_entry_safe(pon, tmp, &spon_dev_list, list) {
		if (pon->spmi->sid == 2) {
			reg = 0xA5;
			spmi_ext_register_writel(pon->spmi->ctrl, pon->spmi->sid, 0x12D0, &reg, 1);
			reg = 0x00;
			spmi_ext_register_writel(pon->spmi->ctrl, pon->spmi->sid, 0x1240, &reg, 1);
		}
	}
...
EXPORT_SYMBOL(qpnp_system_pwr_off_shipmode);
...
/* FUJITSU LIMITED:2014-11-25 H1510020 add start */
	{
		extern void hung_up_timer_start(void);
/* FCNT LIMITED:2016-02-23 H16100736 mod start */
		if (key_status) {
			if (cfg->key_code == KEY_POWER) {
				hung_up_timer_start();
			}
		}
/* FCNT LIMITED:2016-02-23 H16100736 mod end */
	}
/* FUJITSU LIMITED:2014-11-25 H1510020 add end */
	...
	input_sync(pon->pon_input);
/* FUJITSU LIMITED:2014-10-27 H1510026 add start */
	pr_info("pon_input_dispatch: code = %x, value = %d\n", cfg->key_code, (pon_rt_sts & pon_rt_bit));
/* FUJITSU LIMITED:2014-10-27 H1510026 add end */
...
/* FUJITSU LIMITED:2014-11-25 H1510020 add start */
	{
		extern void hung_up_remove(void);
		hung_up_remove();
	}
/* FUJITSU LIMITED:2014-11-25 H1510020 add end */
	return 0;
...
static int qpnp_pon_suspend(struct device *dev)
{
	...
	dev_dbg(dev, "[qpnp_debug]pon->num_pon_config:%d\n", pon->num_pon_config);
	...
	dev_dbg(dev, "[qpnp_debug]suspend_start key_code:%d\n", cfg->key_code);
	...
	if (cfg->key_code == KEY_VOLUMEUP) {
		...
		dev_dbg(dev, "[qpnp_debug]suspend_call_check status:%d, in_call_suspend:%d\n", status, in_call_suspend);
		...
		if (status) {
			enable_irq_wake(cfg->state_irq);
			in_call_suspend = true;
		}
	}

	dev_dbg(dev, "[qpnp_debug]suspend_end\n");
...
static int qpnp_pon_resume(struct device *dev)
{
	...
	dev_dbg(dev, "[qpnp_debug]resume_start key_code:%d\n", cfg->key_code);

	if (cfg->key_code == KEY_VOLUMEUP) {

		dev_dbg(dev, "[qpnp_debug]resume_call_check in_call_suspend:%d\n", in_call_suspend);

		if (in_call_suspend) {
			disable_irq_wake(cfg->state_irq);
			in_call_suspend = false;
		}
	}

	dev_dbg(dev, "[qpnp_debug]resume_end\n");
...
static SIMPLE_DEV_PM_OPS(qpnp_pon_pm_ops, qpnp_pon_suspend, qpnp_pon_resume);/* FCNT LIMITED:2016-02-23 H16100736 add */

static struct spmi_driver qpnp_pon_driver = {
	.driver		= {
		.name	= "qcom,qpnp-power-on",
		.of_match_table = spmi_match_table,
		.pm	= &qpnp_pon_pm_ops,/* FCNT LIMITED:2016-02-23 H16100736 add */
	},
