...
/* FUJITSU add-s */
static struct pm_qos_request pm_qos_thermal_lit_max_freq;
static struct pm_qos_request pm_qos_thermal_big_max_freq;
/* FUJITSU add-e */
...
static uint32_t __add_mask_cluster_core_limit(uint32_t mask); /* FUJITSU:2014-10-24 POWERSAVE Add */
static __ref int do_hotplug(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		mask = __add_mask_cluster_core_limit(mask); /* FUJITSU:2014-10-24 POWERSAVE Add */
...
/* FUJITSU add-s */
	if ( cluster == 0)
		pm_qos_update_request(&pm_qos_thermal_lit_max_freq, freq);
	else
		pm_qos_update_request(&pm_qos_thermal_big_max_freq, freq);
/* FUJITSU add-e */
...
/* FUJITSU add-s */
	pm_qos_add_request(&pm_qos_thermal_lit_max_freq, PM_QOS_LIT_MAXF, INT_MAX);
	pm_qos_add_request(&pm_qos_thermal_big_max_freq, PM_QOS_BIG_MAXF, INT_MAX);
/* FUJITSU add-e */
...
enum {
	CORE_LIMIT_ID_LITTLE,
	CORE_LIMIT_ID_BIG,
	CORE_LIMIT_ID_MAX
};
#define CORE_LIMIT_DEFAULT	INT_MAX
static int cluster_core_limit[CORE_LIMIT_ID_MAX] = {
	[0 ... CORE_LIMIT_ID_MAX-1] = CORE_LIMIT_DEFAULT
};

int msm_thermal_set_core_limit(int num_core_limit_little,
			       int num_core_limit_big,
			       unsigned int flag)
{
	pr_debug("%s(little:%d, big:%d, flag:%u)\n", __FUNCTION__, num_core_limit_little, num_core_limit_big, flag);

	mutex_lock(&core_control_mutex);

	if (flag & (1 << 0)) {
		pr_debug("  little: %d -> %d\n", cluster_core_limit[CORE_LIMIT_ID_LITTLE], num_core_limit_little);
		cluster_core_limit[CORE_LIMIT_ID_LITTLE] = num_core_limit_little;
	}
	if (flag & (1 << 1)) {
		pr_debug("  big: %d -> %d\n", cluster_core_limit[CORE_LIMIT_ID_BIG], num_core_limit_big);
		cluster_core_limit[CORE_LIMIT_ID_BIG] = num_core_limit_big;
	}
...
void msm_thermal_reset_core_limit(void)
{
	pr_debug("%s()\n", __FUNCTION__);
	mutex_lock(&core_control_mutex);

	cluster_core_limit[CORE_LIMIT_ID_LITTLE] = CORE_LIMIT_DEFAULT;
	cluster_core_limit[CORE_LIMIT_ID_BIG] = CORE_LIMIT_DEFAULT;
...
#ifdef CONFIG_SMP
struct core_limit_sort {
	uint32_t cpu;
	long temp;
};
/* Call with core_control_mutex locked */
static uint32_t __add_mask_cluster_core_limit(uint32_t mask)
{
	struct core_limit_sort cputemp[CORE_LIMIT_ID_MAX][NR_CPUS];
	int count[CORE_LIMIT_ID_MAX] = {0};
	int active[CORE_LIMIT_ID_MAX] = {0};
	uint32_t cpu;
	int idx;
	long temp;
	int i, j;

	if (cluster_core_limit[CORE_LIMIT_ID_LITTLE] == CORE_LIMIT_DEFAULT &&
	    cluster_core_limit[CORE_LIMIT_ID_BIG] == CORE_LIMIT_DEFAULT)
		return mask;
	...
	for_each_possible_cpu(cpu) {
		if (((msm_thermal_info.core_control_mask & BIT(cpu)) &&
		    ...
		    !cpu_online(cpu)))
			continue;
		/* regarded as the cluster0 LITTLE cluster currently */
		idx = ((cpus[cpu].parent_ptr->cluster_id ==
			core_ptr->child_entity_ptr[0].cluster_id) ?
			CORE_LIMIT_ID_LITTLE :
			CORE_LIMIT_ID_BIG);
		active[idx]++;
		if (!(msm_thermal_info.core_control_mask & BIT(cpu)))
			continue;
		cputemp[idx][count[idx]].cpu = cpu;
		count[idx]++;
	}

	for (idx = 0; idx < CORE_LIMIT_ID_MAX; idx++) {
		if (active[idx] <= cluster_core_limit[idx])
			continue;
		pr_debug("  cluster%d active:%d > limit:%d\n", idx, active[idx], cluster_core_limit[idx]);
		active[idx] -= cluster_core_limit[idx];

		/* sort by temperature */
		for (i = 0; i < count[idx]; i++) {
			cpu = cputemp[idx][i].cpu;
			temp = 0;
			therm_get_temp(cpus[cpu].sensor_id, THERM_ZONE_ID, &temp);
			for (j = i; j > 0; j--) {
				if (cputemp[idx][j - 1].temp <= temp)
					break;
				cputemp[idx][j] = cputemp[idx][j - 1];
			}
			cputemp[idx][j].cpu = cpu;
			cputemp[idx][j].temp = temp;
			pr_debug("  ...cluster%d cpu%d[%ld]\n", idx, cpu, temp);
		}
		/* add mask a higher temperature cpu */
		for (i = count[idx] - 1; i >= 0 && active[idx] > 0; i--) {
			mask |= BIT(cputemp[idx][i].cpu);
			active[idx]--;
			pr_debug("  mask cpu%d\n", cputemp[idx][i].cpu);
		}
	}
