...
#define	PWM_LUT_MAX_SIZE		23
/* FUJITSU LIMITED:2016-01-20 MOD-E */
#define	PWM_GPLED_LUT_MAX_SIZE		31
#define RGB_LED_DISABLE			0x00

/* FUJITSU LIMITED:2016-01-20 ADD-S */
#define RGB_LED_PWM_PERIOD_US		1000
#define RGB_LED_COLOR_DEFAULT	0x00000000
#define RGB_LED_PAUSE_DEFAULT	1000
#define RGB_LED_DUTY_DEFAULT		1 /* FCNT LIMITED:2016-06-01 ADRV mod */
#define RGB_LED_STORE_COLOR_MAX		0xFFFFFFFF
#define RGB_LED_SET_COLOR_MAX		0x00FFFFFF
#define RGB_LED_PAUSE_MAX			60000
#define RGB_LED_NUM_DUTY_PCTS_MIN	2
#define RGB_LED_NUM_DUTY_PCTS_MAX	PWM_LUT_MAX_SIZE
#define RGB_LED_SET_OFF_TIME	0x0100
#define RGB_LED_PM_PWM_LUT_MASK	0x00FF

#define RGB_LED_COLOR_RED		0x00FF0000
#define RGB_LED_COLOR_GREEN		0x0000FF00
#define RGB_LED_COLOR_BLUE		0x000000FF

#define RGB_LED_PAUSE_BUF_SIZE  8
#define RGB_LED_COLOR_BUF_SIZE  6
#define RGB_LED_NUM_DCTS_BUF_SIZE   2

/* Led status */
#define LED_STATUS_NORMAL      0
#define LED_STATUS_SUSPEND     1
#define LCD_DEVICE_NAME        "/sys/class/graphics/fb0/name"
#define LCD_DEVICE_NAME_SIZE   16
/* FUJITSU LIMITED:2016-01-20 ADD-E */
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
struct rgb_leds_data {
	unsigned int lut_color_lo;
	unsigned int lut_color_hi;
	int lut_pause_lo;
	int lut_pause_hi;
	int num_duty_pcts;
	int ramp_step_ms;
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
	struct workqueue_struct *rgb_wq;
...
int led_status = LED_STATUS_NORMAL;
struct led_classdev *led_rgb_cdev;
char *fb_id = NULL;

/* OffCharge Mode Flg*/
extern int charging_mode;
bool led_init_flag = false;
extern int makercmd_mode;

struct rgb_leds_data leds_data = {
	.lut_color_lo = RGB_LED_COLOR_DEFAULT,
	.lut_color_hi = RGB_LED_COLOR_DEFAULT,
	.lut_pause_lo = RGB_LED_PAUSE_DEFAULT,
	.lut_pause_hi = RGB_LED_PAUSE_DEFAULT,
	.num_duty_pcts = RGB_LED_NUM_DUTY_PCTS_MIN,
	.ramp_step_ms = QPNP_LUT_RAMP_STEP_DEFAULT,
	.flags = QPNP_LED_PWM_FLAGS,
	.duty = RGB_LED_DUTY_DEFAULT,
};
	
struct rgb_leds_data leds_pd_data = {
	.lut_color_lo = RGB_LED_COLOR_DEFAULT,
	.lut_color_hi = RGB_LED_COLOR_DEFAULT,
	.lut_pause_lo = RGB_LED_PAUSE_DEFAULT,
	.lut_pause_hi = RGB_LED_PAUSE_DEFAULT,
...
struct rgb_leds_data current_leds_data = {
	.lut_color_lo = RGB_LED_COLOR_DEFAULT,
	.lut_color_hi = RGB_LED_COLOR_DEFAULT,
	.lut_pause_lo = RGB_LED_PAUSE_DEFAULT,
	.lut_pause_hi = RGB_LED_PAUSE_DEFAULT,
...
#else
	DBG_LOG_LED_DEBUG("===== %s LED register dump start =====", led->cdev.name);
#endif
/* FUJITSU LIMITED:2016-01-20 MOD-E */
...
/* FUJITSU LIMITED:2016-01-20 MOD-S */
		...
		pr_debug("%s: 0x%x = 0x%x\n", led->cdev.name,
					led->base + regs[i], val);
#else
		DBG_LOG_LED_DEBUG("%s: 0x%x = 0x%x", led->cdev.name,led->base + regs[i], val);
#endif
/* FUJITSU LIMITED:2016-01-20 MOD-E */
	}
/* FUJITSU LIMITED:2016-01-20 ADD-S */
...
#else
	DBG_LOG_LED_DEBUG("===== %s LED register dump end =====", led->cdev.name);
#endif
/* FUJITSU LIMITED:2016-01-20 ADD-E */
}
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
static void leds_rgb_work(struct work_struct *work)
{
	int cnt;
	struct qpnp_led_data *info = container_of(work,
					struct qpnp_led_data, rgb_work);
	
	DBG_LOG_FUNCENTER("work=%p", work);
	...
	for(cnt = 0; cnt < RGB_MAX; cnt++){
		...
		case QPNP_ID_RGB_GREEN:
		case QPNP_ID_RGB_BLUE:
			if (info[cnt].cdev.brightness == LED_OFF) {
				DBG_LOG_LED_SET("ID[%d]:blightness = %d",info[cnt].id, info[cnt].cdev.brightness);
				__qpnp_led_work(&info[cnt], info[cnt].cdev.brightness);
			}
		}
	}
	for(cnt = 0; cnt < RGB_MAX; cnt++){
...
static void leds_rgb_set_enable(struct qpnp_led_data *info, enum led_brightness brightness)
{
	DBG_LOG_FUNCENTER("info=%p, brightness=%d", info, brightness);
	
	if (brightness == LED_OFF) {
		info[RGB_RED].cdev.brightness = brightness;
		info[RGB_GREEN].cdev.brightness = brightness;
		info[RGB_BLUE].cdev.brightness = brightness;
	}
	...
	DBG_LOG_LED_SET("queue_work start");
	queue_work(info[RGB_RED].rgb_wq, &info[RGB_RED].rgb_work);
	
	DBG_LOG_FUNCLEAVE("");
}

static int leds_make_data(void)
{
	int ret = 0;
	struct rgb_leds_data new_leds_data;
	
	DBG_LOG_FUNCENTER("");

	new_leds_data = leds_data;
	
	if (led_status == LED_STATUS_SUSPEND) {
		new_leds_data.lut_color_lo = leds_pd_data.lut_color_lo;
		new_leds_data.lut_color_hi = leds_pd_data.lut_color_hi;
		new_leds_data.lut_pause_lo = leds_pd_data.lut_pause_lo;
		new_leds_data.lut_pause_hi = leds_pd_data.lut_pause_hi;
		new_leds_data.num_duty_pcts = leds_pd_data.num_duty_pcts;
	}
	
	if (new_leds_data.lut_pause_lo == 0 
		&& new_leds_data.lut_pause_hi == 0) {
		new_leds_data.lut_color_lo = 0;
		new_leds_data.lut_color_hi = 0;
	} else if (new_leds_data.lut_pause_lo == 0) {
		new_leds_data.lut_color_lo = new_leds_data.lut_color_hi;
		new_leds_data.num_duty_pcts = RGB_LED_NUM_DUTY_PCTS_MIN;
	} else if (new_leds_data.lut_pause_hi == 0) {
		new_leds_data.lut_color_hi = new_leds_data.lut_color_lo;
		new_leds_data.num_duty_pcts = RGB_LED_NUM_DUTY_PCTS_MIN;
	}
	
	if (current_leds_data.num_duty_pcts == new_leds_data.num_duty_pcts
		&& current_leds_data.ramp_step_ms == new_leds_data.ramp_step_ms
		&& current_leds_data.flags == new_leds_data.flags
		&& current_leds_data.duty == new_leds_data.duty
		&& current_leds_data.lut_color_lo == new_leds_data.lut_color_lo
		&& current_leds_data.lut_color_hi == new_leds_data.lut_color_hi
		&& current_leds_data.lut_pause_lo == new_leds_data.lut_pause_lo
		&& current_leds_data.lut_pause_hi == new_leds_data.lut_pause_hi) {
		/* same data */
		DBG_LOG_LED_DEBUG("same data.");
		ret = -1;
	} else {
		current_leds_data.num_duty_pcts = new_leds_data.num_duty_pcts;
		current_leds_data.ramp_step_ms = new_leds_data.ramp_step_ms;
		current_leds_data.flags = new_leds_data.flags;
		current_leds_data.duty = new_leds_data.duty;
		current_leds_data.lut_color_lo = new_leds_data.lut_color_lo;
	...
	DBG_LOG_LED_DEBUG("num_duty_pcts=%d ramp_step_ms=%d flags=0x%02x duty=%d lut_color_lo=0x%08x lut_color_hi=0x%08x lut_pause_lo=%d lut_pause_hi=%d",
			current_leds_data.num_duty_pcts,
			current_leds_data.ramp_step_ms,
			current_leds_data.flags,
			current_leds_data.duty,
			current_leds_data.lut_color_lo,
			current_leds_data.lut_color_hi,
			current_leds_data.lut_pause_lo,
			current_leds_data.lut_pause_hi);
	
	DBG_LOG_FUNCLEAVE("ret=%d",ret);
	return ret;
}

static void leds_update_lut_config(struct qpnp_led_data *info, enum led_brightness brightness)
{
	int lo_color[RGB_MAX];
	int hi_color[RGB_MAX];
	int led_brightness[RGB_MAX];
	int cnt,i,rate;
	int ret;
	int start_idx = 0;
	DBG_LOG_FUNCENTER("info=%p, brightness=%d", info, brightness);
	...
	ret = leds_make_data();
	lo_color[RGB_RED] = (current_leds_data.lut_color_lo & RGB_LED_COLOR_RED) >> 16;
	lo_color[RGB_GREEN] = (current_leds_data.lut_color_lo & RGB_LED_COLOR_GREEN) >> 8;
	lo_color[RGB_BLUE] = (current_leds_data.lut_color_lo & RGB_LED_COLOR_BLUE);
	hi_color[RGB_RED] = (current_leds_data.lut_color_hi & RGB_LED_COLOR_RED) >> 16;
	hi_color[RGB_GREEN] = (current_leds_data.lut_color_hi & RGB_LED_COLOR_GREEN) >> 8;
	hi_color[RGB_BLUE] = (current_leds_data.lut_color_hi & RGB_LED_COLOR_BLUE);
	
	for(cnt = 0; cnt < RGB_MAX; cnt++){
		if (lo_color[cnt] == 0 && hi_color[cnt] == 0) {
			led_brightness[cnt] = LED_OFF;
		} else {
			led_brightness[cnt] = brightness;
		}
	}
	
	if (ret < 0 
		&& info[RGB_RED].cdev.brightness == led_brightness[RGB_RED]
		&& info[RGB_GREEN].cdev.brightness == led_brightness[RGB_GREEN]
		&& info[RGB_BLUE].cdev.brightness == led_brightness[RGB_BLUE]) {
		DBG_LOG_LED_SET("same data. no update lut.");
		return ;
	}
	...
	for(cnt = 0; cnt < RGB_MAX; cnt++){
		DBG_LOG_LED_SET("RGB = %d",cnt);
		if(info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts){
			devm_kfree(&(info[cnt].spmi_dev->dev),info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts);
			info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts = devm_kzalloc(&(info[cnt].spmi_dev->dev),
																			sizeof(int) * current_leds_data.num_duty_pcts,GFP_KERNEL);
			for(i = 0; i < current_leds_data.num_duty_pcts; i++){
				if (i == 0) {
					info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts[i] = (int)((lo_color[cnt] * current_leds_data.duty)/100);
				} else if (i >= current_leds_data.num_duty_pcts-1) {
					info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts[i] = (int)((hi_color[cnt] * current_leds_data.duty)/100);
				} else {
					if (!(current_leds_data.flags & RGB_LED_SET_OFF_TIME)) {
						rate = ((hi_color[cnt] - lo_color[cnt]) * i) / (current_leds_data.num_duty_pcts-1);
						info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts[i] = (int)(((lo_color[cnt] + rate) * current_leds_data.duty)/100);
					} else {
						info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts[i] = 0;
					}
				}
			}
		}

		if (led_debug_mask & DBG_OPTN_LED_SET) {
			printk(KERN_INFO THIS_FILE "L%d: %s() pcts = [ ", __LINE__, __FUNCTION__);
			for(i = 0; i < current_leds_data.num_duty_pcts; i++){
				printk("%d ", info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts[i]);
			}
			printk("]\n");
		}
		
		info[cnt].rgb_cfg->pwm_cfg->pwm_period_us = RGB_LED_PWM_PERIOD_US;
		info[cnt].rgb_cfg->pwm_cfg->lut_params.lut_pause_hi = current_leds_data.lut_pause_hi;
		info[cnt].rgb_cfg->pwm_cfg->lut_params.lut_pause_lo = current_leds_data.lut_pause_lo;
		
		if(cnt == RGB_RED) {
			info[cnt].rgb_cfg->pwm_cfg->lut_params.start_idx = start_idx;
		} else {
			for(i = 0; i < cnt; i++) {
				if (lo_color[cnt] == lo_color[i]
					&& hi_color[cnt] == hi_color[i]) {
					info[cnt].rgb_cfg->pwm_cfg->lut_params.start_idx = info[i].rgb_cfg->pwm_cfg->lut_params.start_idx;
					break;
				}
			}
			if (i == cnt) {
				if(info[cnt-1].cdev.brightness == LED_OFF) {
					info[cnt].rgb_cfg->pwm_cfg->lut_params.start_idx = start_idx;
				} else {
					start_idx += current_leds_data.num_duty_pcts;
					info[cnt].rgb_cfg->pwm_cfg->lut_params.start_idx = start_idx;
				}
			}
		}
		if (info[cnt].cdev.brightness) {
			if (info[cnt].rgb_cfg->pwm_cfg->lut_params.start_idx + current_leds_data.num_duty_pcts > PWM_LUT_MAX_SIZE) {
				dev_err(&info[cnt].spmi_dev->dev,	"Exceed LUT limit\n");
				info[RGB_RED].cdev.brightness = LED_OFF;
				info[RGB_GREEN].cdev.brightness = LED_OFF;
				info[RGB_BLUE].cdev.brightness = LED_OFF;
				break;
			}
		}

		info[cnt].rgb_cfg->pwm_cfg->lut_params.idx_len = current_leds_data.num_duty_pcts;
		info[cnt].rgb_cfg->pwm_cfg->lut_params.ramp_step_ms = current_leds_data.ramp_step_ms;
		info[cnt].rgb_cfg->pwm_cfg->lut_params.flags = current_leds_data.flags & RGB_LED_PM_PWM_LUT_MASK;

		DBG_LOG_LED_SET("pwm_period_us = %d", info[cnt].rgb_cfg->pwm_cfg->pwm_period_us);
		DBG_LOG_LED_SET("lut_pause_hi = %d", info[cnt].rgb_cfg->pwm_cfg->lut_params.lut_pause_hi);
		DBG_LOG_LED_SET("lut_pause_lo = %d", info[cnt].rgb_cfg->pwm_cfg->lut_params.lut_pause_lo);
		DBG_LOG_LED_SET("start_idx = %d", info[cnt].rgb_cfg->pwm_cfg->lut_params.start_idx);
		DBG_LOG_LED_SET("idx_len = %d", info[cnt].rgb_cfg->pwm_cfg->lut_params.idx_len);
		DBG_LOG_LED_SET("ramp_step_ms = %d", info[cnt].rgb_cfg->pwm_cfg->lut_params.ramp_step_ms);
		DBG_LOG_LED_SET("flags = 0x%02x", info[cnt].rgb_cfg->pwm_cfg->lut_params.flags);
	}
	
	for(cnt = 0; cnt < RGB_MAX; cnt++){
		if (info[cnt].cdev.brightness) {
			ret = pwm_lut_config(info[cnt].rgb_cfg->pwm_cfg->pwm_dev,
				info[cnt].rgb_cfg->pwm_cfg->pwm_period_us,
				info[cnt].rgb_cfg->pwm_cfg->duty_cycles->duty_pcts,
				info[cnt].rgb_cfg->pwm_cfg->lut_params);
			if (ret < 0) {
				dev_err(&(info[cnt].spmi_dev->dev), "Failed to configure pwm LUT\n");
				return ;
			}
		}
	}
	
	leds_rgb_set_enable(info, brightness);
	
	DBG_LOG_FUNCLEAVE("");
}

static ssize_t leds_lo_color_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long lo_color = RGB_LED_COLOR_DEFAULT;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &lo_color);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set lo_color");
		return -EINVAL;
	}
	if(lo_color < 0 || lo_color > RGB_LED_STORE_COLOR_MAX){
		DBG_LOG_LED("lo_color is out of range");
		return -EINVAL;
	}
	
	leds_data.lut_color_lo = (unsigned int)lo_color & RGB_LED_SET_COLOR_MAX;
	DBG_LOG_LED_LO_COLOR("store lo_color = 0x%08x", leds_data.lut_color_lo);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_lo_color_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_LO_COLOR("show lo_color = 0x%08x", leds_data.lut_color_lo);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.lut_color_lo);
}

static ssize_t leds_hi_color_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long hi_color = RGB_LED_COLOR_DEFAULT;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &hi_color);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set hi_color");
		return -EINVAL;
	}
	if(hi_color < 0 || hi_color > RGB_LED_STORE_COLOR_MAX){
		DBG_LOG_LED("hi_color is out of range");
		return -EINVAL;
	}
	
	leds_data.lut_color_hi = (unsigned int)hi_color & RGB_LED_SET_COLOR_MAX;
	DBG_LOG_LED_HI_COLOR("store hi_color = 0x%08x", leds_data.lut_color_hi);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_hi_color_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_HI_COLOR("show hi_color = 0x%08x", leds_data.lut_color_hi);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.lut_color_hi);
}

static ssize_t leds_lo_pause_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long lo_pause = RGB_LED_PAUSE_DEFAULT;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &lo_pause);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set lo_pause");
		return -EINVAL;
	}
	if(lo_pause < 0 || lo_pause > RGB_LED_PAUSE_MAX){
		DBG_LOG_LED("lo_pause is out of range");
		return -EINVAL;
	}
	
	leds_data.lut_pause_lo = (int)lo_pause;
	DBG_LOG_LED_LO_PAUSE("store lo_pause = %d", leds_data.lut_pause_lo);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_lo_pause_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_LO_PAUSE("show lo_pause = %d", leds_data.lut_pause_lo);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.lut_pause_lo);
}

static ssize_t leds_hi_pause_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long hi_pause = RGB_LED_PAUSE_DEFAULT;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &hi_pause);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set hi_pause");
		return -EINVAL;
	}
	if(hi_pause < 0 || hi_pause > RGB_LED_PAUSE_MAX){
		DBG_LOG_LED("hi_pause is out of range");
		return -EINVAL;
	}
	
	leds_data.lut_pause_hi = (int)hi_pause;
	DBG_LOG_LED_HI_PAUSE("store hi_pause = %d", leds_data.lut_pause_hi);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_hi_pause_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_HI_PAUSE("show hi_pause = %d", leds_data.lut_pause_hi);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.lut_pause_hi);
}

static ssize_t leds_num_duty_pcts_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long num_duty_pcts = RGB_LED_NUM_DUTY_PCTS_MIN;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &num_duty_pcts);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set num_duty_pcts");
		return -EINVAL;
	}
	if(num_duty_pcts < RGB_LED_NUM_DUTY_PCTS_MIN || num_duty_pcts > RGB_LED_NUM_DUTY_PCTS_MAX){
		DBG_LOG_LED("num_duty_pcts is out of range");
		return -EINVAL;
	}
	
	leds_data.num_duty_pcts = (int)num_duty_pcts;
	DBG_LOG_LED_NUM_DUTY_PCTS("store num_duty_pcts = %d", leds_data.num_duty_pcts);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_num_duty_pcts_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_NUM_DUTY_PCTS("show num_duty_pcts = %d", leds_data.num_duty_pcts);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.num_duty_pcts);
}

static ssize_t leds_ramp_step_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long ramp_step_ms = QPNP_LUT_RAMP_STEP_DEFAULT;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &ramp_step_ms);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set ramp_step_ms");
		return -EINVAL;
	}
	if(ramp_step_ms < 0 || ramp_step_ms > PM_PWM_LUT_RAMP_STEP_TIME_MAX){
		DBG_LOG_LED("ramp_step_ms is out of range");
		return -EINVAL;
	}
	
	leds_data.ramp_step_ms = (int)ramp_step_ms;
	DBG_LOG_LED_RAMP_STEP("store ramp_step_ms = %d", leds_data.ramp_step_ms);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_ramp_step_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_RAMP_STEP("show ramp_step_ms = %d", leds_data.ramp_step_ms);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.ramp_step_ms);
}

static ssize_t leds_flags_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long flags = QPNP_LED_PWM_FLAGS;
	int ret;
	
	DBG_LOG_FUNCENTER("");
	ret = kstrtol(buf, 10, &flags);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set flags");
		return -EINVAL;
	}
	if(flags < 0x0000 || flags > 0xFFFF){
		DBG_LOG_LED("flags is out of range");
		return -EINVAL;
	}
	
	leds_data.flags = (int)flags;
	DBG_LOG_LED_FLAGS("store flags = 0x%08x", leds_data.flags);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_flags_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_FLAGS("show flags = 0x%08x", leds_data.flags);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.flags);
}

static ssize_t leds_duty_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	long duty = RGB_LED_DUTY_DEFAULT;
	int ret;
	...
	if(ret || ret < 0){
		DBG_LOG_LED("cant set duty");
		return -EINVAL;
	}
	if(duty < 0 || duty > 100){
		DBG_LOG_LED("duty is out of range");
		return -EINVAL;
	}
	
	leds_data.duty = (int)duty;
	DBG_LOG_LED_DUTY("store duty = 0x%08x", leds_data.duty);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_duty_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_HI_COLOR("show duty = 0x%08x", leds_data.duty);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "%d\n", leds_data.duty);
}

static ssize_t leds_display_off_color_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	char lo_pause[RGB_LED_PAUSE_BUF_SIZE+1];
	char hi_pause[RGB_LED_PAUSE_BUF_SIZE+1];
	char lo_color[RGB_LED_COLOR_BUF_SIZE+1];
	char hi_color[RGB_LED_COLOR_BUF_SIZE+1];
	char num_dcts[RGB_LED_NUM_DCTS_BUF_SIZE+1];
	
	unsigned long lo_color_state;
	unsigned long hi_color_state;
	unsigned long lo_pause_count;
	unsigned long hi_pause_count;
	unsigned long num_dcts_state;
	
	char *after;
	
	DBG_LOG_FUNCENTER("");
	
	memset(lo_pause, 0x00, sizeof(lo_pause));
	memset(hi_pause, 0x00, sizeof(hi_pause));
	memset(lo_color, 0x00, sizeof(lo_color));
	memset(hi_color, 0x00, sizeof(hi_color));
	memset(num_dcts, 0x00, sizeof(num_dcts));
	
	strncpy(lo_pause, buf, RGB_LED_PAUSE_BUF_SIZE);
	strncpy(hi_pause, buf+RGB_LED_PAUSE_BUF_SIZE, RGB_LED_PAUSE_BUF_SIZE);
	strncpy(lo_color, buf+RGB_LED_PAUSE_BUF_SIZE*2, RGB_LED_COLOR_BUF_SIZE);
	strncpy(hi_color, buf+RGB_LED_PAUSE_BUF_SIZE*2+RGB_LED_COLOR_BUF_SIZE, RGB_LED_COLOR_BUF_SIZE);
	strncpy(num_dcts, buf+RGB_LED_PAUSE_BUF_SIZE*2+RGB_LED_COLOR_BUF_SIZE*2, RGB_LED_NUM_DCTS_BUF_SIZE);

	lo_pause_count = simple_strtoul (lo_pause, &after, 16);
	hi_pause_count = simple_strtoul (hi_pause, &after, 16);
	lo_color_state = simple_strtoul (lo_color, &after, 16);
	hi_color_state = simple_strtoul (hi_color, &after, 16);
	num_dcts_state = simple_strtoul (num_dcts, &after, 16);
	
	if((unsigned int)lo_pause_count > RGB_LED_PAUSE_MAX){
		DBG_LOG_LED("lo_pause is out of range");
		return -EINVAL;
	}
	
	if((unsigned int)hi_pause_count > RGB_LED_PAUSE_MAX){
		DBG_LOG_LED("hi_pause is out of range");
		return -EINVAL;
	}
	if((unsigned int)num_dcts_state < RGB_LED_NUM_DUTY_PCTS_MIN){
		num_dcts_state = RGB_LED_NUM_DUTY_PCTS_MIN;
	} else if((unsigned int)num_dcts_state > RGB_LED_NUM_DUTY_PCTS_MAX){
		DBG_LOG_LED("num_duty_pcts is out of range");
		return -EINVAL;
	}
	
	leds_pd_data.lut_color_lo = (unsigned int)lo_color_state;
	leds_pd_data.lut_color_hi = (unsigned int)hi_color_state;
	leds_pd_data.lut_pause_lo = (unsigned int)lo_pause_count;
	leds_pd_data.lut_pause_hi = (unsigned int)hi_pause_count;
	leds_pd_data.num_duty_pcts = (unsigned int)num_dcts_state;
	DBG_LOG_LED_DISP_OFF("store display off parameter: lo_color=0x%06x hi_color=0x%06x lo_pause=%d hi_pause=%d num_duty_pcts=%d",
				leds_pd_data.lut_color_lo,
				leds_pd_data.lut_color_hi,
				leds_pd_data.lut_pause_lo,
				leds_pd_data.lut_pause_hi,
				leds_pd_data.num_duty_pcts);
	DBG_LOG_FUNCLEAVE("");
	return count;
}

static ssize_t leds_display_off_color_show(struct device * dev, 
	struct device_attribute * attr, 
	char * buf)
{
	DBG_LOG_FUNCENTER("");
	DBG_LOG_LED_DISP_OFF("show display off parameter: lo_color=0x%06x hi_color=0x%06x lo_pause=%d hi_pause=%d num_duty_pcts=%d",
				leds_pd_data.lut_color_lo,
				leds_pd_data.lut_color_hi,
				leds_pd_data.lut_pause_lo,
				leds_pd_data.lut_pause_hi,
				leds_pd_data.num_duty_pcts);
	DBG_LOG_FUNCLEAVE("");
	return snprintf(buf, PAGE_SIZE, "lo_color=0x%06x hi_color=0x%06x lo_pause=%d hi_pause=%d num_duty_pcts=%d\n",
				leds_pd_data.lut_color_lo,
				leds_pd_data.lut_color_hi,
				leds_pd_data.lut_pause_lo,
				leds_pd_data.lut_pause_hi,
...
static ssize_t leds_set_led_ptn(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	...
	ret = kstrtol(buf, 10, &start_led);
	
	if(ret || ret < 0){
		DBG_LOG_LED("cant set led_ptn");
		return -EINVAL;
	}
	...
	DBG_LOG_LED_SET("set led_ptn = %ld", start_led);
	mutex_lock(&leds_mutex_lock);

	if (!led_init_flag){
		led_init_flag = true;
		DBG_LOG_LED("led_init_flag is false. initialize register.");
		for(cnt = 0; cnt < RGB_MAX; cnt++){
			rc = qpnp_rgb_init(&info[cnt]);
			if (rc)
				dev_err(&info[cnt].spmi_dev->dev,
					"RGB initialize failed(%d)\n", rc);
		}
	}
	
	if (start_led > 0) {
		leds_update_lut_config(info, LED_FULL);
	} else {
		leds_rgb_set_enable(info, LED_OFF);
	}
	mutex_unlock(&leds_mutex_lock);
...
static void leds_unblank_set(struct qpnp_led_data *info)
{
	DBG_LOG_FUNCENTER("");
	if (charging_mode || makercmd_mode) {
		DBG_LOG_FUNCLEAVE("unblank skip. charging_mode=%d, makercmd_mode=%d.", charging_mode, makercmd_mode);
		return;
	}
	
	if (led_status == LED_STATUS_NORMAL) {
		DBG_LOG_FUNCLEAVE("aleady led_status_normal");
		return;
	}
	led_status = LED_STATUS_NORMAL;
	DBG_LOG_LED_RESUME("led_status=%d", led_status);	
...
static void leds_powerdown_set(struct qpnp_led_data *info)
{
	DBG_LOG_FUNCENTER("");
	if (charging_mode || makercmd_mode) {
		DBG_LOG_FUNCLEAVE("powerdown skip. charging_mode=%d, makercmd_mode=%d.", charging_mode, makercmd_mode);
		return;
	}
	
	if (led_status == LED_STATUS_SUSPEND) {
		DBG_LOG_FUNCLEAVE("aleady led_status_suspend");
		return;
	}
	led_status = LED_STATUS_SUSPEND;
	DBG_LOG_LED_SUSPEND("led_status=%d", led_status);	
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	int *blank;
	struct qpnp_led_data *info = container_of(self, struct qpnp_led_data, fb_notif);
	struct file *file;
	
	if(!fb_id){
		fb_id = kzalloc(LCD_DEVICE_NAME_SIZE, GFP_KERNEL);
		if(!fb_id){
			DBG_LOG_LED("failure kzalloc");
			return 0;
		}
		file = filp_open(LCD_DEVICE_NAME, O_RDONLY, 0);
		if (IS_ERR(file)) {
			DBG_LOG_LED("filp_open error");
			kfree(fb_id);
			fb_id = NULL;
			return 0;
		}
		if (kernel_read(file, 0, fb_id, LCD_DEVICE_NAME_SIZE) < 0) {
			DBG_LOG_LED("kernel_read error");
			kfree(fb_id);
			fb_id = NULL;
		...
		filp_close(file, NULL);
		*(fb_id + (strnlen(fb_id, LCD_DEVICE_NAME_SIZE)-1))='\0';
	}
	DBG_LOG_LED_DEBUG("fb_id=%s", fb_id);
	if(evdata){
		DBG_LOG_LED_DEBUG("evdata->info->fix.id=%s", evdata->info->fix.id);
		if(strncmp(fb_id, evdata->info->fix.id, LCD_DEVICE_NAME_SIZE) == 0){
			if (evdata->data && event == FB_EVENT_BLANK ) {
				blank = evdata->data;
				if (*blank == FB_BLANK_UNBLANK) {
					DBG_LOG_LED_RESUME("FB_BLANK_UNBLANK");
					leds_unblank_set(info);
				} else if (*blank == FB_BLANK_POWERDOWN) {
					DBG_LOG_LED_SUSPEND("FB_BLANK_POWERDOWN");
					leds_powerdown_set(info);
				}
			}
		}
	}
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
static DEVICE_ATTR(lo_color, S_IRUGO | S_IWUSR | S_IWGRP, leds_lo_color_show, leds_lo_color_store);
static DEVICE_ATTR(hi_color, S_IRUGO | S_IWUSR | S_IWGRP, leds_hi_color_show, leds_hi_color_store);
static DEVICE_ATTR(lo_pause, S_IRUGO | S_IWUSR | S_IWGRP, leds_lo_pause_show, leds_lo_pause_store);
static DEVICE_ATTR(hi_pause, S_IRUGO | S_IWUSR | S_IWGRP, leds_hi_pause_show, leds_hi_pause_store);
static DEVICE_ATTR(num_duty_pcts, S_IRUGO | S_IWUSR | S_IWGRP, leds_num_duty_pcts_show, leds_num_duty_pcts_store);
static DEVICE_ATTR(ramp_step, S_IRUGO | S_IWUSR | S_IWGRP, leds_ramp_step_show, leds_ramp_step_store);
static DEVICE_ATTR(flags, S_IRUGO | S_IWUSR | S_IWGRP, leds_flags_show, leds_flags_store);
static DEVICE_ATTR(duty, S_IRUGO | S_IWUSR | S_IWGRP, leds_duty_show, leds_duty_store);
static DEVICE_ATTR(display_off_color, S_IRUGO | S_IWUSR | S_IWGRP, leds_display_off_color_show, leds_display_off_color_store);
static DEVICE_ATTR(start_led_ptn, S_IRUGO | S_IWUSR | S_IWGRP, NULL, leds_set_led_ptn);
/* FUJITSU LIMITED:2016-01-20 ADD-E */
...
static struct attribute *sec_led_attributes[] = {
	&dev_attr_lo_color.attr,
	&dev_attr_hi_color.attr,
	&dev_attr_lo_pause.attr,
	&dev_attr_hi_pause.attr,
	&dev_attr_num_duty_pcts.attr,
	&dev_attr_ramp_step.attr,
	&dev_attr_flags.attr,
	&dev_attr_duty.attr,
	&dev_attr_display_off_color.attr,
	&dev_attr_start_led_ptn.attr,
	NULL
};
...
#else
		if (charging_mode || makercmd_mode) {
			printk(KERN_INFO THIS_FILE "L%d: %s() skip initialize. charging_mode=%d, makercmd_mode=%d.\n", 
				__LINE__, __FUNCTION__, charging_mode, makercmd_mode);
		} else {
			led_init_flag = true;
			rc = qpnp_rgb_init(led);
			if (rc)
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
	int ret;
	unsigned char leddbg_getNvData[APNV_SIZE_LED_DBG_MODE];
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
	if(led_array[RGB_RED].id == QPNP_ID_RGB_RED &&
		...
		led_array[RGB_RED].fb_notif.notifier_call = fb_notifier_callback;
		fb_register_client(&led_array[RGB_RED].fb_notif);
		
		memset(leddbg_getNvData, 0x00, sizeof(leddbg_getNvData));
		ret = get_nonvolatile(leddbg_getNvData, APNV_LED_DBG_MODE, APNV_SIZE_LED_DBG_MODE);
		if (unlikely(ret < 0)) {
			printk(KERN_INFO THIS_FILE "L%d %s(): get_nonvolatile() [ret=%d]", __LINE__, __FUNCTION__, ret);
			leddbg_getNvData[APNV_ADR_LED_DBG_MODE_1] = 0x00;
			leddbg_getNvData[APNV_ADR_LED_DBG_MODE_2] = 0x01;
		}
		led_debug_mask = (leddbg_getNvData[APNV_ADR_LED_DBG_MODE_1] << 8 | leddbg_getNvData[APNV_ADR_LED_DBG_MODE_2]);

		led_array[RGB_RED].rgb_wq = create_singlethread_workqueue("leds_rgb_workq");
		if (!led_array[RGB_RED].rgb_wq) {
			dev_err(&spmi->dev, "L%d: %s() create_singlethread_workqueue() error.\n",__LINE__, __FUNCTION__);
			fb_unregister_client(&led_array[RGB_RED].fb_notif);
			rc = -1;
			goto fail_id_check;
		}

		INIT_WORK(&led_array[RGB_RED].rgb_work, leds_rgb_work);
		
		DBG_LOG_LED_DEBUG("*** LED driver create sysfs ***");
		led_rgb_cdev = kzalloc(sizeof(*led_rgb_cdev), GFP_KERNEL);
		if (unlikely(!led_rgb_cdev)) {
			fb_unregister_client(&led_array[RGB_RED].fb_notif);
			dev_err(&spmi->dev, "L%d: %s() failure kzalloc \n",__LINE__, __FUNCTION__);
			rc = -ENOMEM;
			goto fail_id_check;
		}
		led_rgb_cdev->name = "rgb_leds";
		led_rgb_cdev->brightness = LED_OFF;
		rc = led_classdev_register(&spmi->dev, led_rgb_cdev);
		if (rc) {
			kfree(led_rgb_cdev);
			fb_unregister_client(&led_array[RGB_RED].fb_notif);
			dev_err(&spmi->dev, "L%d: %s() unable to register rgb_leds rc=%d\n",__LINE__, __FUNCTION__, rc);
			goto fail_id_check;
		}
		dev_set_drvdata(led_rgb_cdev->dev, led_array);
		rc = sysfs_create_group(&led_rgb_cdev->dev->kobj, &sec_led_attr_group);
		if(rc) {
			led_classdev_unregister(led_rgb_cdev);
			kfree(led_rgb_cdev);
			fb_unregister_client(&led_array[RGB_RED].fb_notif);
			dev_err(&spmi->dev, "L%d: %s() unable to create sysfs\n",__LINE__, __FUNCTION__);
			goto fail_id_check;
		}
		...
		if (((led_getNvData[0] >= 0x01) && (led_getNvData[0] <= 0x07)) ||
		     ((led_getNvData[0] >= 0x11) && (led_getNvData[0] <= 0x17))) {
			tcxo_flg = 1;
		} else {
			tcxo_flg = 0;
		}
		DBG_LOG_LED_DEBUG("tcxo_flg=%d", tcxo_flg);
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
		if (i == RGB_RED) {
			fb_unregister_client(&led_array[i].fb_notif);
			cancel_work_sync(&led_array[i].rgb_work);
		}
/* FUJITSU LIMITED:2016-01-20 ADD-E */
...
/* FUJITSU LIMITED:2016-01-20 ADD-S */
	if (led_rgb_cdev) {
		sysfs_remove_group(&led_rgb_cdev->dev->kobj, &sec_led_attr_group);
		led_classdev_unregister(led_rgb_cdev);
		kfree(led_rgb_cdev);
	}
	if(fb_id) {
