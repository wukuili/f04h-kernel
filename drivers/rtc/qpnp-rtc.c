...
/* FUJITSU:2013/06/26 add start */
#include <linux/nonvolatile_common.h>
#include <linux/delay.h>
#include <linux/workqueue.h>

#define APNV_SECURITY_SC_FLAG_I 48002
#define SC_FLAG_RETRY_COUNT 5
#define NV_FIRST_DELAY_MS 2500
#define NV_RETRY_DELAY_MS 500
#define RTC_INIT_THRESHOLD_SEC 10UL
/* FUJITSU:2013/06/26 add end */
...
/* FUJITSU:2013/06/26 add start */
static void qpnp_work_queue(struct work_struct *work) {
	struct qpnp_rtc *rtc_dd = container_of(work, struct qpnp_rtc, work.work);
	u8 value = 0;
	int ret;

	ret = get_nonvolatile(&value, APNV_SECURITY_SC_FLAG_I, sizeof(value));
	if (ret >= 0 && value == 1)
		ret = set_nonvolatile(&rtc_dd->sc_flag, APNV_SECURITY_SC_FLAG_I, sizeof(rtc_dd->sc_flag));
	if (ret < 0) {
		if (rtc_dd->retry_count++ >= SC_FLAG_RETRY_COUNT) {
			dev_err(rtc_dd->rtc_dev, "SC flag write failed, retry out\n");
			return;
		}
		dev_warn(rtc_dd->rtc_dev, "SC flag write failed, retry\n");
		INIT_DELAYED_WORK(&rtc_dd->work, qpnp_work_queue);
		schedule_delayed_work(&rtc_dd->work, msecs_to_jiffies(NV_RETRY_DELAY_MS));
	}
}

static ssize_t qpnp_rtc_sysfs_show_sc_flag(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct qpnp_rtc *rtc_dd = dev_get_drvdata(dev);
	buf[0] = rtc_dd->sc_flag;
	return 1;
}
static struct device_attribute dev_attr_sc_flag = __ATTR(sc_flag, 0444, qpnp_rtc_sysfs_show_sc_flag, NULL);
/* FUJITSU:2013/06/26 add end */
...
static int qpnp_rtc_probe(struct spmi_device *spmi)
{
	...
	rc = qpnp_read_wrapper(rtc_dd, value,
				...
				NUM_8_BIT_RTC_REGS);
	if (rc) {
		dev_err(&spmi->dev, "Read from RTC reg failed\n");
		goto fail_init_detection;
	}
	...
	dev_info(&spmi->dev, "Probe rtc secs=%lu\n", secs);
	if (secs < RTC_INIT_THRESHOLD_SEC) {
		dev_info(&spmi->dev, "rtc init detected\n");
		rtc_dd->sc_flag = 0;
		INIT_DELAYED_WORK(&rtc_dd->work, qpnp_work_queue);
		schedule_delayed_work(&rtc_dd->work, msecs_to_jiffies(NV_FIRST_DELAY_MS));
	} else {
		rtc_dd->sc_flag = 1;
	}
	rc = device_create_file(&spmi->dev, &dev_attr_sc_flag);
	if (rc) {
		dev_err(&spmi->dev, "failed to create sc_flag attribute, %d\n", rc);
		goto fail_init_detection;
	}
	/* FUJITSU:2013/06/26 add end */
...
static int qpnp_rtc_remove(struct spmi_device *spmi)
{
	...
	free_irq(rtc_dd->rtc_alarm_irq, rtc_dd);
	/* FUJITSU:2013/06/26 add start */
	device_remove_file(&spmi->dev, &dev_attr_sc_flag);
	/* FUJITSU:2013/06/26 add end */
	rtc_device_unregister(rtc_dd->rtc);
	dev_set_drvdata(&spmi->dev, NULL);
