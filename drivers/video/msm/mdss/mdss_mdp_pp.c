...
static atomic_t fj_gamma_index = ATOMIC_INIT(0);

extern u32 *gammaC0C1_TablePtr[];
extern u32 *gammaC2_TablePtr[];


int fj_gamma_set_index(struct msm_fb_data_type *mfd,int newvalue)
{
	atomic_set(&fj_gamma_index, newvalue);
	return 0;
}

int fj_gamma_get_index(struct msm_fb_data_type *mfd)
{
	return atomic_read(&fj_gamma_index);
}

bool mdss_lut_push = false;
bool fj_mdss_mdp_lut_push(void)
{
	return mdss_lut_push;
}

void fj_mdss_mdp_set_lut(bool value)
{
	mdss_lut_push = value;
	return;
}
int fj_mdss_mdp_lut_igc_config(struct msm_fb_data_type *mfd)
{
	int ret = 0;
	u32 copyback = 0;
	u32 copy_from_kernel = 1;
	...
	ctl = mfd_to_ctl(mfd);
	if (!ctl){
		pr_err("%s(%d) error ctl=%p\n",__func__,__LINE__,ctl);
		return -EINVAL;
	}

	index = fj_gamma_get_index(mfd);
	config.len = IGC_LUT_ENTRIES;
	config.ops = MDP_PP_OPS_WRITE | MDP_PP_OPS_ENABLE;
	config.block = (ctl->mfd->index) + MDP_LOGICAL_BLOCK_DISP_0;
	config.c0_c1_data = gammaC0C1_TablePtr[index];
	config.c2_data    = gammaC2_TablePtr[index];

	ret = mdss_mdp_igc_lut_config(&config, &copyback,copy_from_kernel);
...
/*                                                   */
/*  fj implimentation of MSMFB_SET_LUT.              */
/*  HSIC or CABL must be disabled for this function. */
/*                                                   */
int fj_lut_update(struct msm_fb_data_type *mfd, struct fb_cmap *cmap)
{
	int ret = 0;
	int need_update = 0;
	int i;
	...
	pr_debug("[GAMMA]%s(%d) start\n",__func__,__LINE__);

	if(!mfd || !cmap ){
		pr_err("%s(%d) error mfd=%p cmap=%p\n",__func__,__LINE__, mfd, cmap);
		return -EINVAL;
	}

	start = cmap->start;
	last = cmap->start + cmap->len;
	if(last > IGC_LUT_ENTRIES){
		pr_err("%s(%d) error offset out of range start=%d len=%d\n",__func__,__LINE__,cmap->start,cmap->len);
		return -EINVAL;
	}

	if(cmap->len <= 0 || start < 0){
		pr_err("%s(%d) start=%d len=%d returns no error\n",__func__,__LINE__,cmap->start,cmap->len);
		return 0;
	}
	...
	mutex_lock(&mdss_pp_mutex);
	{
		for (i = start; i < last; i++) {
			if (copy_from_user(&r, cmap->red++, sizeof(r)) ||
				...
				pr_err("%s(%d) error data copy \n",__func__,__LINE__);
				ret = -EINVAL;
				break;
			}
			gammaC0C1_TablePtr[0][i] = ((g << 4) | (b << 20));
			gammaC2_TablePtr[0][i] = (r << 4);
		}

		if(ret == 0 && fj_gamma_get_index(mfd) == 0){
			need_update = 1;
		}
	}
	mutex_unlock(&mdss_pp_mutex);
