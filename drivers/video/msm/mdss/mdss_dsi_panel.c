...
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_01_0001 mod start */
	if (pinfo->pdest == DISPLAY_1) {
		mdss_dsi_cmdlist_put(ctrl, &cmdreq);
	}
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_01_0001 mod end */
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_02_0002 mod start */
static unsigned char gamma_setting[256]={
	0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B,  0x1B, 0x1B, 0x1B, 0x1A, 0x1A, 0x1A, 0x1A, 0x1A, 
	0x1A, 0x1A, 0x1A, 0x19, 0x19, 0x19, 0x19, 0x19,  0x19, 0x19, 0x19, 0x18, 0x18, 0x18, 0x18, 0x18, 
	0x18, 0x18, 0x18, 0x17, 0x17, 0x17, 0x17, 0x17,  0x17, 0x17, 0x17, 0x16, 0x16, 0x16, 0x16, 0x16, 
	0x16, 0x16, 0x16, 0x15, 0x15, 0x15, 0x15, 0x15,  0x15, 0x15, 0x15, 0x14, 0x14, 0x14, 0x14, 0x14, 
	...
	0x0A, 0x0A, 0x0A, 0x09, 0x09, 0x09, 0x09, 0x09,  0x09, 0x09, 0x09, 0x08, 0x08, 0x08, 0x08, 0x08, 
	0x08, 0x08, 0x08, 0x07, 0x07, 0x07, 0x07, 0x07,  0x07, 0x07, 0x07, 0x06, 0x06, 0x06, 0x06, 0x06, 
	0x06, 0x06, 0x06, 0x05, 0x05, 0x05, 0x05, 0x05,  0x05, 0x05, 0x05, 0x04, 0x04, 0x04, 0x04, 0x04, 
...
static unsigned char aor_setting[28][2]={
	{0x06, 0x00}, {0x06, 0x00}, {0x06, 0x00}, {0x23, 0x00}, {0x58, 0x00}, {0x87, 0x00}, {0xAD, 0x00}, {0xAD, 0x00},
	{0xAD, 0x00}, {0xAD, 0x00}, {0xAD, 0x00}, {0xAD, 0x00}, {0xAD, 0x00}, {0xB3, 0x00}, {0xD2, 0x00}, {0xE8, 0x00},
	{0xFE, 0x00}, {0x11, 0x01}, {0x23, 0x01}, {0x32, 0x01}, {0x3C, 0x01}, {0x47, 0x01}, {0x52, 0x01}, {0x59, 0x01},
	{0x63, 0x01}, {0x69, 0x01}, {0x70, 0x01}, {0x77, 0x01}
};
static unsigned char elvss_setting_acloff[28]={
	0x19, 0x1A, 0x1C, 0x1D, 0x1D, 0x1D, 0x1D, 0x1F,
	0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F,
	0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F,
	0x1F, 0x1F, 0x1F, 0x1F
};
static char led_data1[2] = {0x82,0x00};
static char led_data2[3] = {0x85,0x06,0x00};
static char led_data3[2] = {0xB0,0x34};
static char led_data4[2] = {0xBB,0x19};
static char led_data5[2] = {0xB0,0x2E};
static char led_data6[2] = {0xBB,0x01};
static struct dsi_cmd_desc oled_backlight_cmd[] = {
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 0, sizeof(led_data1)}, led_data1},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(led_data2)}, led_data2},
	{{DTYPE_GEN_WRITE2, 0, 0, 0, 0, sizeof(led_data3)}, led_data3},
	{{DTYPE_GEN_WRITE2, 0, 0, 0, 0, sizeof(led_data4)}, led_data4},
	{{DTYPE_GEN_WRITE2, 0, 0, 0, 0, sizeof(led_data5)}, led_data5},
	{{DTYPE_GEN_WRITE2, 1, 0, 0, 0, sizeof(led_data6)}, led_data6},
};

static void mdss_dsi_panel_bklt_oled(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
	unsigned char index = gamma_setting[level];
	...
	led_data1[1] = index;
	led_data2[1] = aor_setting[index][0];
	led_data2[2] = aor_setting[index][1];
	led_data4[1] = elvss_setting_acloff[index];

	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = oled_backlight_cmd;
	cmdreq.cmds_cnt = 6;
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_02_0002 mod end */
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add start */
/* FCNT LIMITED:2016-03-30 H161_5_DISP_02_0002 mod start */
#define BKL_LEVEL_ABOOT_SET 118
/* FCNT LIMITED:2016-03-30 H161_5_DISP_02_0002 mod end */
struct dentry *fjbkl_debugfs_root;
struct dentry *fjbkl_debugfs_brightness_inc_time_msec = NULL;
static	int brightness_inc_time_msec = 128;
struct dentry *fjbkl_debugfs_brightness_dec_time_msec = NULL;
static	int brightness_dec_time_msec = 128;
struct dentry *fjbkl_debugfs_brightness_duration_msec = NULL;
static	int brightness_duration_msec = 16;

static void _mdss_dsi_panel_bklt_set(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	pr_debug("%s: level=%d ndx=%d\n", __func__, level, ctrl->ndx);
	if (ctrl->ndx == DSI_CTRL_LEFT) {
		if (level < 0)
			level = 0;
		else if (level > ctrl->bklt_max)
			level = ctrl->bklt_max;

		mutex_lock(&ctrl->bklt_info.lock);
		ctrl->bklt_info.target_brightness_new = level;
		if ((ctrl->bklt_info.current_brightness == 0) && (level != 0)) {
			/* ON */
			pr_info("[BKL]%s: enter Backlight turn on.\n", __func__);
			ctrl->bklt_info.slope_count = 0;
			ctrl->bklt_info.current_brightness = level;
			mdss_dsi_panel_bklt_oled(ctrl, level);
			pr_info("[BKL]%s: leave Backlight turn on.(%d)\n", __func__, ctrl->bklt_info.current_brightness);
		} else if ((ctrl->bklt_info.current_brightness != 0) && (level == 0)) {
			/* OFF */
			pr_info("[BKL]%s: enter Backlight turn off.(%d)\n", __func__, ctrl->bklt_info.current_brightness);
			cancel_delayed_work_sync(&ctrl->bklt_info.work);
			ctrl->bklt_info.slope_count = 0;
			ctrl->bklt_info.current_brightness = level;
			mdss_dsi_panel_bklt_oled(ctrl, level);
			pr_info("[BKL]%s: leave Backlight turn off.\n", __func__);
		} else {
			if (ctrl->bklt_info.current_brightness == level) {
				/* stop slope */
				pr_info("[BKL]%s: brightness No change end(%d)\n", __func__, ctrl->bklt_info.current_brightness);
				cancel_delayed_work_sync(&ctrl->bklt_info.work);
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add start */
				ctrl->bklt_info.slope_count = 0;
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add end */
			} else {
				if (ctrl->bklt_info.slope_count == 0) {
					/* start slope */
					pr_debug("%s: start slope\n", __func__);
					queue_delayed_work(ctrl->bklt_info.wq, &ctrl->bklt_info.work, 0);
				} else {
					pr_debug("%s: sloping now. changed only a target brightness.\n", __func__);
				}
			}
		}
		mutex_unlock(&ctrl->bklt_info.lock);
	}
}

static void _mdss_dsi_panel_bklt_work(struct work_struct *work)
{
	int target = 0;
	int brightness_delta = 0;
	struct delayed_work *delayed_work;
	struct backlight_info *bklt_info;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata;
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add start */
	unsigned long delay_time = 0, elapsed_time = 0;
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add end */

	delayed_work = container_of(work, struct delayed_work, work);
	bklt_info = container_of(delayed_work, struct backlight_info, work);
	ctrl_pdata = container_of(bklt_info, struct mdss_dsi_ctrl_pdata, bklt_info);

	pr_debug("%s: start\n", __func__);
	mutex_lock(&bklt_info->lock);
	target = bklt_info->target_brightness_new;
	if (bklt_info->slope_count == 0) {
		/* First time */
		if (bklt_info->current_brightness < target) {
			/* UP Slope */
			if (brightness_inc_time_msec <= brightness_duration_msec) {
				bklt_info->slope_count = 1;
			} else {
				bklt_info->slope_count = brightness_inc_time_msec/brightness_duration_msec;
			}
		} else {
			/* DOWN Slope */
			if (brightness_dec_time_msec <= brightness_duration_msec) {
				bklt_info->slope_count = 1;
			} else {
				bklt_info->slope_count = brightness_dec_time_msec/brightness_duration_msec;
			}
		}
		pr_info("[BKL]%s: brightness change start(%d)->(%d)\n", __func__, bklt_info->current_brightness, target);
		bklt_info->target_brightness = target;
		pr_debug("%s: set slope_count = %d\n", __func__, bklt_info->slope_count);
		queue_delayed_work(bklt_info->wq, &bklt_info->work, msecs_to_jiffies(brightness_duration_msec));
	} else {
		if (bklt_info->slope_count == 1) {
			/* Last */
			bklt_info->current_brightness = bklt_info->target_brightness;
		} else {
			pr_debug("%s: current_brightness = %d\n", __func__, bklt_info->current_brightness);
			if (bklt_info->current_brightness < bklt_info->target_brightness) {
				/* UP Slope */
				/* Rounding off */
				brightness_delta = ((((bklt_info->target_brightness - bklt_info->current_brightness) * 100) / bklt_info->slope_count) + 50) / 100;
				if (brightness_delta <= 0) {
					brightness_delta = 0;
				}
				bklt_info->current_brightness += brightness_delta;
			} else {
				/* DOWN Slope */
				/* Rounding off */
				brightness_delta = ((((bklt_info->current_brightness - bklt_info->target_brightness) * 100) / bklt_info->slope_count) + 50) / 100;
				if (brightness_delta <= 0) {
					brightness_delta = 0;
				}
				bklt_info->current_brightness -= brightness_delta;
			}
			pr_debug("%s: current_brightness = %d delta = %d\n", __func__, bklt_info->current_brightness, brightness_delta);
		}
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add start */
		delay_time = msecs_to_jiffies(brightness_duration_msec);
		elapsed_time = jiffies;
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add end */
		/* update */
		mdss_dsi_panel_bklt_oled( ctrl_pdata, bklt_info->current_brightness);
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add start */
		elapsed_time = jiffies - elapsed_time;
		if (elapsed_time == 0) {
			elapsed_time = 1;
		}
...
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 add end */
		if (target != bklt_info->target_brightness) {
			/* Changed */
			pr_debug("[BKL]Brightness Changed, Slope Stop\n");
			bklt_info->slope_count = 0;
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 mod start */
//			queue_delayed_work(bklt_info->wq, &bklt_info->work, msecs_to_jiffies(brightness_duration_msec));
			queue_delayed_work(bklt_info->wq, &bklt_info->work, delay_time);
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 mod end */
		} else {
			bklt_info->slope_count--;
			pr_debug("%s: dec slope_count = %d\n", __func__, bklt_info->slope_count);
			if (bklt_info->slope_count == 0) {
				/* end */
				pr_info("[BKL]%s: brightness change end(%d)\n", __func__, bklt_info->current_brightness);
			} else {
				/* continue */
/* FCNT LIMITED:2016-02-26 H161_5_DISP_02_0003 mod start */
//				queue_delayed_work(bklt_info->wq, &bklt_info->work, msecs_to_jiffies(brightness_duration_msec));
...
/* FUJITSU LIMITED:2015-05-07 H1520349 add start */
	{
		static unsigned int bl_suspend_bak;
		unsigned int bl_suspend = (bl_level == 0 ? 1 : 0);
		if (bl_suspend != bl_suspend_bak) {
			extern void filog_display_suspend(unsigned int state);
			filog_display_suspend(bl_suspend);
			bl_suspend_bak = bl_suspend;
		}
	}
/* FUJITSU LIMITED:2015-05-07 H1520349 add end */
...
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_07_0001 add start */
	pr_debug("[LCD]%s: pid=%d, brightness(%d)\n",__func__, current->pid, bl_level);
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_07_0001 add end */
...
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add start */
	case BL_OLED:
		_mdss_dsi_panel_bklt_set(ctrl_pdata, bl_level);
		break;
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add end */
	default:
...
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_01_0003 add start */
void mdss_dsi_panel_switch_mode_nop(struct mdss_panel_data *pdata,
							int mode)
{
    pr_info("[LCD]%s enter\n",__func__);
    pr_info("[LCD]%s leave\n",__func__);
...
void mdss_dsi_panel_bl_ctrl_nop(struct mdss_panel_data *pdata,
							u32 bl_level)
{
    pr_info("[LCD]%s enter\n",__func__);
    pr_info("[LCD]%s leave\n",__func__);
...
int mdss_dsi_panel_on_nop(struct mdss_panel_data *pdata)
{
    pr_info("[LCD]%s enter\n",__func__);
    pr_info("[LCD]%s leave\n",__func__);
	return 0;
}
int mdss_dsi_panel_off_nop(struct mdss_panel_data *pdata)
{
    pr_info("[LCD]%s enter\n",__func__);
    pr_info("[LCD]%s leave\n",__func__);
	return 0;
...
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_01_0003 add end */
	...
	if (data) {
		if (!strncmp(data, "bl_ctrl_wled", 12)) {
			...
			pr_debug("%s: Configured DCS_CMD bklt ctrl\n",
								__func__);
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add start */
		} else if (!strncmp(data, "bl_ctrl_oled", 12)) {
			ctrl_pdata->bklt_ctrl = BL_OLED;
			pr_debug("%s: Configured OLED bklt ctrl\n", __func__);
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add end */
		}
	}
...
/* FUJITSU LIMITED:2015-05-20 H152_2_DISP_01_0003 mod start */
	if ((!strcmp(pinfo->panel_name, "Sim dual 0 video mode dsi panel")) ||
		(!strcmp(pinfo->panel_name, "Sim dual 1 video mode dsi panel"))) {
		pr_info("[LCD]%s panel state change\n",__func__);
		pinfo->panel_state = false;
		ctrl_pdata->on = mdss_dsi_panel_on_nop;
		ctrl_pdata->off = mdss_dsi_panel_off_nop;
		ctrl_pdata->panel_data.set_backlight = mdss_dsi_panel_bl_ctrl_nop;
		ctrl_pdata->switch_mode = mdss_dsi_panel_switch_mode_nop;
	} else {
...
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add start */
		if (ctrl_pdata->ndx == DSI_CTRL_LEFT) {
			if (fjbkl_debugfs_root == NULL) {
				fjbkl_debugfs_root = debugfs_create_dir("fjbkl", NULL);
			}
			if (fjbkl_debugfs_brightness_inc_time_msec == NULL && fjbkl_debugfs_root) {
				fjbkl_debugfs_brightness_inc_time_msec = debugfs_create_u32("fjbkl_brightness_inc_time_msec", 0644, fjbkl_debugfs_root,(u32 *)&brightness_inc_time_msec);
			}
			if (fjbkl_debugfs_brightness_dec_time_msec == NULL && fjbkl_debugfs_root) {
				fjbkl_debugfs_brightness_dec_time_msec = debugfs_create_u32("fjbkl_brightness_dec_time_msec", 0644, fjbkl_debugfs_root,(u32 *)&brightness_dec_time_msec);
			}
			if (fjbkl_debugfs_brightness_duration_msec == NULL && fjbkl_debugfs_root) {
				fjbkl_debugfs_brightness_duration_msec = debugfs_create_u32("fjbkl_brightness_duration_msec", 0644, fjbkl_debugfs_root,(u32 *)&brightness_duration_msec);
			}
			ctrl_pdata->bklt_info.wq = create_singlethread_workqueue("fj_bklt_slope");
			mutex_init(&ctrl_pdata->bklt_info.lock);
			INIT_DELAYED_WORK(&ctrl_pdata->bklt_info.work, _mdss_dsi_panel_bklt_work);
			ctrl_pdata->bklt_info.target_brightness_new = 0;
			ctrl_pdata->bklt_info.target_brightness = 0;
			ctrl_pdata->bklt_info.slope_count = 0;
			if (cmd_cfg_cont_splash) {
				ctrl_pdata->bklt_info.current_brightness = BKL_LEVEL_ABOOT_SET;
			} else {
				ctrl_pdata->bklt_info.current_brightness = 0;
			}
		}
/* FUJITSU LIMITED:2016-01-22 H161_5_DISP_02_0003 add end */
	}
/* FUJITSU LIMITED:2015-05-20 H152_2_DISP_01_0003 mod end */
