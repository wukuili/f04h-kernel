...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0010 mod start */
#if 0
#define DEFAULT_VIDEO_RESOLUTION HDMI_VFRMT_640x480p60_4_3
#else
#define DEFAULT_VIDEO_RESOLUTION HDMI_VFRMT_1920x1080p30_16_9
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0010 mod end */
#define DEFAULT_HDMI_PRIMARY_RESOLUTION HDMI_VFRMT_1280x720p60_16_9
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0007 add start */
#define DEFAULT_VIDEO_RESOLUTION_MC		HDMI_VFRMT_720x480p60_4_3
extern int makercmd_mode;
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0007 add end */
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod start */
	...
	if (!hdmi_ctrl->feature_data[HDMI_TX_FEAT_CEC])
		DEV_INFO("%s: cec is not supported\n", __func__);
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod end */
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add start */
		init_completion(&hdmi_ctrl->hpd_off_done);
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add end */
	} else {
		hdmi_tx_set_audio_switch_node(hdmi_ctrl, 0);
		hdmi_tx_wait_for_audio_engine(hdmi_ctrl);
...
static int hdmi_tx_set_hev_vic(struct platform_device *pdev, uint8_t write_burst_vic)
{
	...
	hdmi_ctrl->mhl_write_burst_vic = write_burst_vic;
        DEV_ERR("%s: set hev vic:0x%2x\n", __func__, write_burst_vic);
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add start */
	if (hdmi_ctrl->mhl_hpd_on == on) {
		DEV_INFO("%s:mhl_hpd_on:%d, on:%d\n", __func__, hdmi_ctrl->mhl_hpd_on, on);
		return 0;
	}
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add end */
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add start */
	io = &hdmi_ctrl->pdata.io[HDMI_TX_CORE_IO];
	if (!io->base) {
		DEV_ERR("%s: invalid input 2\n", __func__);
		return -EINVAL;
	}
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add end */

/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add start */
	if( !on && !hdmi_ctrl->hpd_initialized ) {
		/* already HDMI power off and hpd off */
		DEV_INFO("%s:HDMI power off. hpd is already disabled.\n", __func__);
	} else
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add end */
	if (!on && hdmi_ctrl->hpd_feature_on) {
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add start */
		timeout = wait_for_completion_interruptible_timeout(&hdmi_ctrl->hpd_off_done, (HZ * timeout_sec));
		if (!timeout) {
			DEV_INFO("%s:hpd off wait:timeout\n", __func__);
			hdmi_tx_set_audio_switch_node(hdmi_ctrl, 0);
			hdmi_tx_wait_for_audio_engine(hdmi_ctrl);
			hdmi_tx_send_cable_notification(hdmi_ctrl, 0);
		}
		...
		/* Turn off HPD interrupts */
/* FCNT LIMITED:2016-03-30 H161_5_MHL_01_0018 mod start */
#if 0
		DSS_REG_W(io, HDMI_HPD_INT_CTRL, 0);
#else
		if (hdmi_ctrl->pdata.pin_states & BIT(HDMI_TX_HPD_PM)) {
			DSS_REG_W(io, HDMI_HPD_INT_CTRL, 0);
		}
#endif
/* FCNT LIMITED:2016-03-30 H161_5_MHL_01_0018 mod end */
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0005 add end */
		rc = hdmi_tx_sysfs_enable_hpd(hdmi_ctrl, false);
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0007 mod start */
#if 0
	if (!hdmi_ctrl->pdata.primary)
		hdmi_ctrl->video_resolution = DEFAULT_VIDEO_RESOLUTION;
#else
	if (makercmd_mode) {
		hdmi_ctrl->video_resolution = DEFAULT_VIDEO_RESOLUTION_MC;
	} else if (!hdmi_ctrl->pdata.primary) {
		hdmi_ctrl->video_resolution = DEFAULT_VIDEO_RESOLUTION;
	}
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0007 mod end */
