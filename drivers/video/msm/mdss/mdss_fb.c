/*---------------------------------------------------------------*/
/* COPYRIGHT(C) FUJITSU CONNECTED TECHNOLOGIES LIMITED 2015-2016 */
/*---------------------------------------------------------------*/
/*
...
/* FUJITSU LIMITED:2015-05-13 H152_2_DISP_05_0001 add start */
#include "gammatable.h"
/* FUJITSU LIMITED:2015-05-13 H152_2_DISP_05_0001 add end */
/* FUJITSU:2015-03-10 GEN-141-SCRU-ALT ADD S */
#ifdef CONFIG_SECURITY_FJSEC
extern void fjsec_make_page_acl(char *id, unsigned long addr, unsigned long size);
#endif
/* FUJITSU:2015-03-10 GEN-141-SCRU-ALT ADD E */
...
/* FUJITSU LIMITED:2016-01-20 H161_5_DISP_05_0001 add start */
...
extern int fj_gamma_set_index(struct msm_fb_data_type *mfd,int newvalue);
extern int fj_mdss_mdp_lut_igc_config(struct msm_fb_data_type *mfd);

/* read lut update data from buffer */
/*    int color  -  r:0  g:1  b:2   */
int fj_gamma_colortable_write(int tablenum,int color,const char *buf,size_t count,int *offset){
	char tmp[4] = {' ', ' ', ' ',' '};
	int i;
	int j = 0;
	int k = 0;
		...
		if (status == 0 ) {
			...
			if (buf[i] == ' ') {
				...
				if (j==2) {
					...
					for(k=0;k<2;k++){
						if (tmp[k] >= '0' && tmp[k] <= '9' ) {
							value = (value * 0x10) + (tmp[k] - '0');
						} else if (tmp[k] >= 'A' && tmp[k] <= 'F') {
							value = (value * 0x10) + (tmp[k] + 0xa - 'A' );
						} else if (tmp[k] >= 'a' && tmp[k] <= 'f') {
							value = (value * 0x10) + (tmp[k] + 0xa - 'a' );
						} else {
							return -1;
					...
					if(color == 0/*red*/) {
						gammaC2_TablePtr[tablenum][l]   = (gammaC2_TablePtr[tablenum][l] & 0xFFFF0000)   | (value << 4);
					} else if(color == 1/*green*/) {
						gammaC0C1_TablePtr[tablenum][l] = (gammaC0C1_TablePtr[tablenum][l] & 0xFFFF0000) | (value << 4);
					} else if(color == 2/*blue*/) {
						gammaC0C1_TablePtr[tablenum][l] = (gammaC0C1_TablePtr[tablenum][l] & 0x0000FFFF) | (value << 20);
					}
...
//#define FJ_GAMMA_DEBUG
#ifdef FJ_GAMMA_DEBUG
void fj_gamma_colortable_dump(int tablenum){
	int i;

	pr_debug("%s:%d color c0c1 table=%d\n",__func__,__LINE__,tablenum);
	for(i=0; i<GAMMA_TBL_SZ ; i+=4){
		pr_debug("%d: %08x %08x %08x %08x\n",i,gammaC0C1_TablePtr[tablenum][i],gammaC0C1_TablePtr[tablenum][i+1],gammaC0C1_TablePtr[tablenum][i+2],gammaC0C1_TablePtr[tablenum][i+3]);
	}
	pr_debug("%s:%d color c2 table=%d\n",__func__,__LINE__,tablenum);
	for(i=0; i<GAMMA_TBL_SZ ; i+=4){
		pr_debug("%d: %08x %08x %08x %08x\n",i,gammaC2_TablePtr[tablenum][i],gammaC2_TablePtr[tablenum][i+1],gammaC2_TablePtr[tablenum][i+2],gammaC2_TablePtr[tablenum][i+3]);
	}
}
#endif

/* read command data from buffer */
int fj_gamma_read_command(const char* buf,size_t count,int* offset,int *command){
	int status = 0;
	int j = 0;
	char cmd[4] = {' ', ' ', ' ',' '};
	...
	if(cmd[0] == 'o' && cmd[1] == 'n'){
		...
		*command = 1; /*off*/
	} else if(cmd[0] == 'l' && cmd[1] == 'u' && cmd[2] == 't' && cmd[3] == 'r'){
		*command = 2; /*lutr*/
	} else if(cmd[0] == 'l' && cmd[1] == 'u' && cmd[2] == 't' && cmd[3] == 'g'){
		*command = 3; /*lutg*/
	} else if(cmd[0] == 'l' && cmd[1] == 'u' && cmd[2] == 't' && cmd[3] == 'b'){
		*command = 4; /*lutb*/
	} else {
		return -1;
	}
...
/* read priority data from buffer */
int fj_gamma_read_priority(const char* buf,size_t count,int* offset,int *prio){
	int status = 0;
	int j = 0;
	char priority[4] = {' ', ' ', ' ',' '};
	...
	*prio = 0;
	...
	for(j=0 ; j<3 ; j++){
		if(priority[j] <= '9' && priority[j] >='0'){
			*prio = (*prio * 10) + (priority[j] - '0');
		} else {
			break;
		}
	}
...
int fj_gamma_get_active_lut(void){
	int i;

	for(i = NO_OF_GAMMA_TABLES-1 ;i>=0 ; i--)
	{
		if(gamma_condition[i] > 0)
			return i;
	}
...
/* reset gamma activity */
void fj_gamma_reset_active_lut(void){
	int i;
...
/* for debug */
void fj_gamma_print_usage(int table){
	if (table < NO_OF_GAMMA_TABLES) {
		pr_info("Set gammatable %d\n",table);
	} else {
		pr_err("ERROR unknown gammatable. table=%d\n",table);
	}
}

extern void fj_mdss_mdp_set_lut(bool value);
ssize_t gamma_store(struct device *device,
																...
																size_t count)
{
	...
	int ret;
	int previous_active_lut;
	int new_active_lut;
	int command;
	int priority;

	/* get command */
	ret = fj_gamma_read_command(buf,count,&i,&command);
	if(ret)
		return count;

	pr_debug("command = %d i=%d\n",command,i);

	/* get priority */
	ret = fj_gamma_read_priority(buf,count,&i,&priority);
	if(ret)
		return count;

	/* change priority to tablenum */
	if (priority == 0)
		table = 0;
	else if (priority == 1)
		table = 1;
	...
	pr_debug("priority = %d table = %d i=%d\n",priority,table,i);

	previous_active_lut = fj_gamma_get_active_lut();

	/* process command*/
	if(command == 2/*lutr*/){
		fj_gamma_colortable_write(table,0/*red*/,&buf[0],count,&i);
		return count;
	}else if(command == 3/*lutg*/){
		fj_gamma_colortable_write(table,1/*green*/,&buf[0],count,&i);
		return count;
	}else if(command == 4/*lutb*/){
		fj_gamma_colortable_write(table,2/*blue*/,&buf[0],count,&i);
		return count;
	}else if(command == 0/*on*/){
		gamma_condition[table] = 1;
		if(table == 0) {
			fj_gamma_reset_active_lut();
		}
	}else if (command == 1/*off*/){
		gamma_condition[table] = 0;
		if(table == 0) {
			fj_gamma_reset_active_lut();
		}
	} else {
	...
	new_active_lut = fj_gamma_get_active_lut();

	if(previous_active_lut == 0 && new_active_lut == 0) {
		return count;
	}

	fbi = dev_get_drvdata(device);
	if (!fbi) {
		pr_info("msm_fb.c: msm_fb_data_type is NULL!!\n");
		return count;
	}
...
#ifdef FJ_GAMMA_DEBUG
	fj_gamma_colortable_dump(new_active_lut);
#endif

	fj_gamma_print_usage(new_active_lut);
	fj_gamma_set_index(mfd,new_active_lut);

	/* FCNT LIMITED:2016-12-13 T161_DISP_03_0001 mod start */
	{
		char *envp[2] = {"GAMMA_UPDATE=1", NULL};
		kobject_uevent_env(&mfd->fbi->dev->kobj, KOBJ_CHANGE, envp);
	}
	/* FCNT LIMITED:2016-12-13 T161_DISP_03_0001 mod end */

	return count;
}

static DEVICE_ATTR(gamma, S_IRUGO|S_IWUSR,gamma_show, gamma_store);
/* FUJITSU LIMITED:2016-01-20 H161_5_DISP_05_0001 add end */
...
/* FUJITSU LIMITED:2015-05-12 H152_2_DISP_07_0002 add start */
	if (mfd->panel_info->pdest == DISPLAY_1) {
		mdss_fb_set_backlight(mfd,0);
	}
/* FUJITSU LIMITED:2015-05-12 H152_2_DISP_07_0002 add end */
...
/* FUJITSU:2015-03-10 GEN-141-SCRU-ALT ADD S */
#ifdef CONFIG_SECURITY_FJSEC
			fjsec_make_page_acl("fb_mmap", (unsigned long)page_to_phys(page), len);
			ret = io_remap_pfn_range(vma, addr, page_to_pfn(page), len,
						vma->vm_page_prot);
			if( ret == -EACCES ) {
				printk(KERN_INFO "%s:%d: error=%d retry\n", __FUNCTION__, __LINE__, ret);
				ret = io_remap_pfn_range(vma, addr, page_to_pfn(page), len,
							vma->vm_page_prot);
			}
			if(ret) {
...
#else /* CONFIG_SECURITY_FJSEC */
			io_remap_pfn_range(vma, addr, page_to_pfn(page), len,
					vma->vm_page_prot);
#endif /* FUJITSU:2015-03-10 GEN-141-SCRU-ALT ADD E */
			addr += len;
			if (addr >= vma->vm_end)
				break;
		}
...
/* FUJITSU:2015-03-10 GEN-141-SCRU-ALT ADD S */
#ifdef CONFIG_SECURITY_FJSEC
	fjsec_make_page_acl("fb_mmap", off >> PAGE_SHIFT, vma->vm_end - vma->vm_start);
...
static int __mdss_fb_display_thread(void *data)
{
	...
	while (1) {
		...
		if (perf_debug_mode_perform_commit())
			trace_printk("B|0|__mdss_fb_perform_commit\n");
