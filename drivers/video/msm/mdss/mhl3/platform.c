...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod start */
	...
	return gpio_get_value(starter_kit_control_gpios[MHL_INT_INDEX].gpio)
			? 0 : 1;
#else
	return mhl_gpio_get_value(FJ_TX_HW_INT) ? 0 : 1;
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod end */
}
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0006 add start */
	mhl_charge_control(max_current_in_milliamps);
...
#else
    mhl_set_hev_vic(write_burst_vic);
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod end */
}
...
static int mhl_tx_get_dt_data(struct device *dev)
{
	...
	pr_debug("%s: id=%d\n", __func__, dev->id);
#define QCOM_HDMI_PHANDLE_NAME "qcom,hdmi-tx-map"

	hdmi_tx_node = of_parse_phandle(of_node, QCOM_HDMI_PHANDLE_NAME, 0);

	MHL_TX_DBG_ERR("hdmi_tx_node:%X\n", hdmi_tx_node);
	if (!hdmi_tx_node) {
	...
	if (!drv_info.hdmi_pdev) {
		MHL_TX_DBG_ERR("%scan't find the device by node%s\n",
				ANSI_ESC_RED_TEXT, ANSI_ESC_RESET_TEXT);
		return -EINVAL;
	}
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod start */
#if 0
	MHL_TX_DBG_INFO("hdmi_pdev [0X%x] to pdata->qcom_hdmi_pdev\n",
			(unsigned int)drv_info.hdmi_pdev);
#else
	MHL_TX_DBG_INFO("hdmi_pdev [0X%pK] to pdata->qcom_hdmi_pdev\n",
			drv_info.hdmi_pdev);
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod end */
	...
	const struct i2c_device_id *id)
{
	...
	MHL_TX_DBG_WARN("%s(), i2c_device_id = %pK\n", __func__, id);
	...
	if (ret) {
		...
		MHL_TX_DBG_ERR("registering with msm/mdss\n")
		ret = msm_hdmi_register_mhl(drv_info.hdmi_pdev,
			  drv_info.hdmi_mhl_ops, NULL);
		if (ret) {
			MHL_TX_DBG_ERR("%sno registration from msm/mdss%s\n",
				ANSI_ESC_RED_TEXT, ANSI_ESC_RESET_TEXT)
			ret = 0; /* continue */
		} else
			MHL_TX_DBG_ERR("%sregistered from msm/mdss%s\n",
...
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 add start */
	if (!mhl_is_connected()) {
		pr_debug("%s MHL cable is not connected\n", __func__);
		up(&platform_lock);
		goto done;
	}
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 add end */
...
#else
		ret = mhl_platform_parse_dt(&spi_dev->dev);
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod end */
		if (ret)
			goto failed;
...
#else
		mhl_get_int_gpio());
		pr_debug("%s gpio:%d, irq:%d\n", __func__, mhl_get_int_gpio(), drv_info.irq);
#endif
/* FUJITSU LIMITED:2016-01-07 H161_5_MHL_01_0001 mod end */
