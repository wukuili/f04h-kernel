...
#define PANEL_3_3V_ON_POST_WAIT  (10*1000)
#define PANEL_3_3V_OFF_PRE_WAIT  (10*1000)
#define PANEL_POWER_DISABLE_TIME (64*1000)
#define TCON_RDY_WAIT_COUNT      50
#define TCON_RDY_WAIT_TIME       (8*1000)
#define TCON_RDY_AFTER_WAIT_TIME (1*1000)
...
/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_01_0001 add start */
	if (pdata->panel_info.panel_state == false) {
		pr_warn("%s:%d not panel connected\n", __func__, __LINE__);
		ret = 0;
		goto end;
	}

/* FUJITSU LIMITED:2015-04-20 H152_2_DISP_01_0001 add end */
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add start */
	if (pdata->panel_info.pdest == DISPLAY_2) {
		if (gpio_is_valid(ctrl_pdata->lcd3_3v_gpio)) {
			int ret2 = 0;

			ret2 = gpio_request(ctrl_pdata->lcd3_3v_gpio, "lcd_3_3v");
			if (ret2) {
				ret = ret2;
				pr_err("request lcd3_3v gpio failed, rc=%d\n", ret);
			} else {
				usleep(PANEL_3_3V_OFF_PRE_WAIT);
				gpio_direction_output((ctrl_pdata->lcd3_3v_gpio), 0);
				gpio_free(ctrl_pdata->lcd3_3v_gpio);
				usleep(PANEL_POWER_DISABLE_TIME);
			}
		}
	}
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add end */
...
/* FCNT LIMITED:2016-02-26 H161_5_DISP_01_0006 add start */
		if (DSI_PANEL_PM == i)
			continue;
/* FCNT LIMITED:2016-02-26 H161_5_DISP_01_0006 add end */
		/*
...
/* FCNT LIMITED:2016-02-26 H161_5_DISP_01_0006 add start */
	if (pdata->panel_info.pdest == DISPLAY_1) {
		if (gpio_is_valid(ctrl_pdata->lcd3_3v_gpio)) {
			ret = gpio_request(ctrl_pdata->lcd3_3v_gpio, "lcd_3_3v");
			if (ret) {
				pr_err("request lcd3_3v gpio failed, rc=%d\n", ret);
				return ret;
			} else {
				gpio_direction_output((ctrl_pdata->lcd3_3v_gpio), 1);
				gpio_free(ctrl_pdata->lcd3_3v_gpio);
				usleep(PANEL_3_3V_ON_POST_WAIT);
			}
		}
	}
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add start */
	if(panel_info->pdest == DISPLAY_1) {
		...
		tmp = MIPI_INP((sctrl_pdata->ctrl_base) + 0xac);
		tmp &= ~(1<<28);
		MIPI_OUTP((sctrl_pdata->ctrl_base) + 0xac, tmp);
		wmb();
	}
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add end */
/* FUJITSU LIMITED:2015-12-22 H161_5_DISP_01_0009 del start */
#if 0
	if (pdata->panel_info.type == MIPI_CMD_PANEL)
		mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 1);
#endif
/* FUJITSU LIMITED:2015-12-22 H161_5_DISP_01_0009 del end */
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add start */
	if(pdata->panel_info.pdest == DISPLAY_2) {
		if (gpio_is_valid(ctrl_pdata->tconrdy_gpio)) {
			...
			ret = gpio_request(ctrl_pdata->tconrdy_gpio, "tcon_rdy");
			if (ret) {
				pr_err("request tconrdy gpio failed, rc=%d\n", ret);
			} else {
				for(j=0;j<TCON_RDY_WAIT_COUNT;j++){
					tmp = gpio_get_value(ctrl_pdata->tconrdy_gpio);
					if(tmp)
						break;
					pr_debug("[LCD]%s wait_gpio retry(%d)\n",__func__, j);
					usleep(TCON_RDY_WAIT_TIME);
				}
				pr_info("[LCD]%s tcon_rdy count=%d gpio=%d\n",__func__, j, gpio_get_value(ctrl_pdata->tconrdy_gpio));
				gpio_free(ctrl_pdata->tconrdy_gpio);
			}
			...
			MIPI_OUTP((sctrl_pdata->ctrl_base) + 0xac, tmp);
			wmb();
			usleep(TCON_RDY_AFTER_WAIT_TIME);
		}
	}
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add end */
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add start */
	if (ctrl_pdata->panel_data.panel_info.panel_state == true) {
		if (ctrl_pdata->panel_data.panel_info.pdest == DISPLAY_1) {
			if (gpio_is_valid(ctrl_pdata->tconintr_gpio)) {
				rc = gpio_request(ctrl_pdata->tconintr_gpio, "tcon_intr");
				if (rc) {
					pr_err("request tconintr gpio failed, rc=%d\n", rc);
				}
			}
		}
	}
...
/* FUJITSU LIMITED:2015-12-16 H161_5_DISP_01_0006 add start */
	ctrl_pdata->tconrdy_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "fj,platform-tconrdy-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->tconrdy_gpio))
		pr_err("%s:%d, tconrdy gpio not specified\n", __func__, __LINE__);

	ctrl_pdata->tconintr_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "fj,platform-tconintr-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->tconintr_gpio))
		pr_err("%s:%d, tconintr gpio not specified\n", __func__, __LINE__);

	ctrl_pdata->lcd3_3v_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "fj,platform-lcd3_3v-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd3_3v_gpio))
		pr_err("%s:%d, lcd3_3v gpio not specified\n", __func__, __LINE__);
