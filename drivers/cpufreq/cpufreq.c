...
struct cpufreq_cpu_save_data {
	...
	int need_verify; /* FUJITSU:2014-10-24 POWERSAVE Add */
};
...
/* FUJITSU LIMITED:2015-05-07 H1520349 add start */
		{
			extern void filog_cpu_frequency(unsigned int state, unsigned int cpu_id);
			filog_cpu_frequency(freqs->new, freqs->cpu);
		}
...
/* FUJITSU:2014-10-24 POWERSAVE Add <S> */
	if (per_cpu(cpufreq_policy_save, cpu).need_verify) {
		per_cpu(cpufreq_policy_save, cpu).need_verify = 0;
		pr_debug("Verifying CPU%d user policy min %d and max %d\n", cpu,
...
/* FUJITSU:2014-10-24 POWERSAVE Add <S> */
int cpufreq_set_limit(cpumask_t cpu_mask, unsigned int max_freq, unsigned int min_freq)
{
	unsigned long flags;
	struct cpufreq_policy *policy;
	struct cpufreq_policy new_policy;
	...
	pr_debug("%s(msk:%lx, max:%u, min:%u)\n", __FUNCTION__, cpumask_bits(&cpu_mask)[0], max_freq, min_freq);
	...
	for_each_cpu(cpu, &cpu_mask) {
		...
		if (policy) {
			...
			policy->user_policy.min = new_policy.min;
			policy->user_policy.max = new_policy.max;
			pr_debug("  cpu%u set policy (max:%u, min:%u)\n", cpu, new_policy.max, new_policy.min);
			ret = cpufreq_set_policy(policy, &new_policy);
			if (ret) {
				pr_err("cpufreq: cpu%u set policy error %d\n", cpu, ret);
				err = ret;
			}
			...
			pr_debug("  cpu%u undefined policy\n", cpu);
		}
	}
	up_read(&cpufreq_rwsem);
unlock:
