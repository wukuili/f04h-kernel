...
/* FUJITSU:2014-10-24 POWERSAVE Add <S> */
/* FUJITSU LIMITED:2015-11-18 POWERSAVE mod start */
static struct cpufreq_interactive_tunables *get_cpu_tunables(int cpu)
{
	struct cpufreq_interactive_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_interactive_tunables *tunables;
...
int cpufreq_interactive_set_io_is_busy(bool io_is_busy)
{
	int ret = 0;
	struct cpufreq_interactive_policyinfo *ppol;
	struct cpufreq_interactive_tunables *tunables;
	struct cpufreq_policy *policy;
	cpumask_t related = { CPU_BITS_NONE };
	int cpu;

	pr_debug("%s(io_is_busy:%d)\n", __FUNCTION__, io_is_busy);

	get_online_cpus();

	for_each_possible_cpu(cpu) {
		if (cpumask_test_cpu(cpu, &related))
			continue;
		...
		if (policy) {
			...
			tunables = get_tunables(ppol);
			if (!tunables) {
				pr_debug("cpu%d tunable not initialized, newly alloc\n", cpu);
				down_write(&policy->rwsem);
				tunables = alloc_tunable(policy);
				if (IS_ERR(tunables)) {
					pr_err("cpu%d tunable not initialized, newly alloc failed\n", cpu);
					ret = -ENOMEM;
					tunables = NULL;
				}
			...
			pr_debug("  cpu%u undefined policy, get tunables direct\n", cpu);
			tunables = get_cpu_tunables(cpu);
		}
		if (tunables) {
			tunables->io_is_busy = io_is_busy;
			pr_debug("cpu%d set tunable.io_is_busy=%d\n", cpu, io_is_busy);
		}
		if (policy) {
			if (have_governor_per_policy()) {
				cpumask_or(&related, &related, policy->related_cpus);
				pr_debug("  related_cpu:%lx\n", cpumask_bits(policy->related_cpus)[0]);
			} else {
				cpumask_or(&related, &related, cpu_possible_mask);
				pr_debug("  related_cpu:%lx\n", cpumask_bits(cpu_possible_mask)[0]);
			}
			cpufreq_cpu_put(policy);
		} else {
			cpumask_or(&related, &related, topology_core_cpumask(cpu));
			pr_debug("  related_cpu:%lx\n", cpumask_bits(topology_core_cpumask(cpu))[0]);
		}
	}
	put_online_cpus();

	pr_debug("call sched_set_io_is_busy(%d)\n", io_is_busy);
...
#define FLAG_TARGET_LOADS		(1 << 0)
#define FLAG_ABOVE_HISPEED_DELAY	(1 << 1)
#define FLAG_MIN_SAMPLE_TIME		(1 << 2)
#define FLAG_HISPEED_FREQ		(1 << 3)
#define FLAG_GO_HISPEED_LOAD		(1 << 4)
#define FLAG_TIMER_SLACK_VAL		(1 << 5)
#define FLAG_MAX_FREQ_HYSTERESIS	(1 << 6)
int cpufreq_interactive_set_param(
                        cpumask_t cpu_mask,
                        unsigned int *target_loads,
                        unsigned int *above_hispeed_delay,
                        unsigned long min_sample_time,
                        ...
                        unsigned int flag)
{
	...
	pr_debug("%s(msk:%lx, target_loads:%p, above_hispeed_delay:%p, min_sample_time:%lu, hispeed_freq:%u, go_hispeed_load:%lu, timer_slack_val:%d, max_freq_hysteresis:%u, flag:%u)\n",
		__FUNCTION__, cpumask_bits(&cpu_mask)[0], target_loads, above_hispeed_delay, min_sample_time, hispeed_freq, go_hispeed_load, timer_slack_val, max_freq_hysteresis, flag);
	...
	for_each_cpu(cpu, &cpu_mask) {
		...
		if (tunables) {
			if (flag & FLAG_TARGET_LOADS) {
				/* format of target_loads parameter passed
				 * target_load[0]     = number of data
				 * target_load[1...n] = data
				 */
				if (target_loads && target_loads[0] > 0) {
					tmpmem = kmalloc(target_loads[0] * sizeof(unsigned int), GFP_KERNEL);
					need_dup = true;
				} else
					need_dup = false;
				spin_lock_irqsave(&tunables->target_loads_lock, flags);
				if (tunables->target_loads != default_target_loads) {
					pr_debug("cpu%d target_loads free prev\n", cpu);
					kfree(tunables->target_loads);
				}
				if (need_dup) {
					if (tmpmem != NULL) {
						tunables->target_loads = tmpmem;
						memcpy(tunables->target_loads, &target_loads[1], target_loads[0] * sizeof(unsigned int));
						tunables->ntarget_loads = target_loads[0];
						pr_debug("cpu%d target_loads set %p[%d]\n", cpu, tunables->target_loads, tunables->ntarget_loads);
					} else {
						pr_err("cpu%d target_loads kmalloc failed. set default\n", cpu);
						ret = -ENOMEM;
						tunables->target_loads = default_target_loads;
						tunables->ntarget_loads = ARRAY_SIZE(default_target_loads);
					}
				} else {
					pr_debug("cpu%d target_loads set to default\n", cpu);
					tunables->target_loads = default_target_loads;
					tunables->ntarget_loads = ARRAY_SIZE(default_target_loads);
				}
				spin_unlock_irqrestore(&tunables->target_loads_lock, flags);
			}
			if (flag & FLAG_ABOVE_HISPEED_DELAY) {
				/* format of above_hispeed_delay parameter passed
				 * above_hispeed_delay[0]     = number of data
				 * above_hispeed_delay[1...n] = data
				 */
				if (above_hispeed_delay && above_hispeed_delay[0] > 0) {
					tmpmem = kmalloc(above_hispeed_delay[0] * sizeof(unsigned int), GFP_KERNEL);
					need_dup = true;
				} else
					need_dup = false;
				spin_lock_irqsave(&tunables->above_hispeed_delay_lock, flags);
				if (tunables->above_hispeed_delay != default_above_hispeed_delay) {
					pr_debug("cpu%d above_hispeed_delay free prev\n", cpu);
					kfree(tunables->above_hispeed_delay);
				}
				if (need_dup) {
					if (tmpmem != NULL) {
						tunables->above_hispeed_delay = tmpmem;
						memcpy(tunables->above_hispeed_delay, &above_hispeed_delay[1], above_hispeed_delay[0] * sizeof(unsigned int));
						tunables->nabove_hispeed_delay = above_hispeed_delay[0];
						pr_debug("cpu%d above_hispeed_delay set %p[%d]\n", cpu, tunables->above_hispeed_delay, tunables->nabove_hispeed_delay);
					} else {
						pr_err("cpu%d above_hispeed_delay kmalloc failed. set default\n", cpu);
						ret = -ENOMEM;
						tunables->above_hispeed_delay = default_above_hispeed_delay;
						tunables->nabove_hispeed_delay = ARRAY_SIZE(default_above_hispeed_delay);
					}
				} else {
					pr_debug("cpu%d above_hispeed_delay set to default\n", cpu);
					tunables->above_hispeed_delay = default_above_hispeed_delay;
					tunables->nabove_hispeed_delay = ARRAY_SIZE(default_above_hispeed_delay);
				}
				spin_unlock_irqrestore(&tunables->above_hispeed_delay_lock, flags);
			}
			if (flag & FLAG_MIN_SAMPLE_TIME) {
				pr_debug("cpu%d min_sample_time set\n", cpu);
				tunables->min_sample_time = min_sample_time;
			}
			if (flag & FLAG_HISPEED_FREQ) {
				pr_debug("cpu%d hispeed_freq set\n", cpu);
				tunables->hispeed_freq = hispeed_freq;
			}
			if (flag & FLAG_GO_HISPEED_LOAD) {
				pr_debug("cpu%d go_hispeed_load set\n", cpu);
				tunables->go_hispeed_load = go_hispeed_load;
			}
			if (flag & FLAG_TIMER_SLACK_VAL) {
				pr_debug("cpu%d timer_slack_val set\n", cpu);
				tunables->timer_slack_val = timer_slack_val;
			}
			if (flag & FLAG_MAX_FREQ_HYSTERESIS) {
				pr_debug("cpu%d max_freq_hysteresis set\n", cpu);
				tunables->max_freq_hysteresis = max_freq_hysteresis;
			}
		}
		if (policy) {
			if (have_governor_per_policy()) {
...
int cpufreq_interactive_set_freq(cpumask_t cpu_mask, unsigned int freq)
{
	int cpu;
	int anyboost = 0;
	unsigned long flags;
	struct cpufreq_interactive_policyinfo *ppol;
	cpumask_t boost_mask = { CPU_BITS_NONE };

	cpumask_and(&cpu_mask, &cpu_mask, cpu_possible_mask);

	pr_debug("%s(msk:%lx, freq:%u)\n",
		__FUNCTION__, cpumask_bits(&cpu_mask)[0], freq);

	/* same as cpufreq_interactive_boost */
	get_online_cpus();
	...
	for_each_cpu(cpu, &cpu_mask) {
		...
		spin_lock_irqsave(&ppol->target_freq_lock, flags);
		if (ppol->target_freq < freq) {
			pr_debug("cpu%d freq %u -> %u\n", cpu, ppol->target_freq, freq);
			ppol->target_freq = freq;
			cpumask_set_cpu(cpu, &boost_mask);
			anyboost = 1;

			pr_debug("cpu%d floor_freq %u -> %u\n", cpu, ppol->floor_freq, freq);
			ppol->floor_freq = freq;
			ppol->floor_validate_time = ktime_to_us(ktime_get());
		}
	...
	if (anyboost) {
		spin_lock_irqsave(&speedchange_cpumask_lock, flags);
		cpumask_or(&speedchange_cpumask, &speedchange_cpumask, &boost_mask);
		spin_unlock_irqrestore(&speedchange_cpumask_lock, flags);
		wake_up_process(speedchange_task);
		pr_debug("%s() kick boost done\n", __FUNCTION__);
	} else {
		pr_debug("%s() done (no boost) \n", __FUNCTION__);
	}

	put_online_cpus();
	return 0;
}
/* FUJITSU LIMITED:2015-11-18 POWERSAVE mod end   */
/* FUJITSU:2014-10-24 POWERSAVE Add <E> */
