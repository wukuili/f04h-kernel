...
/*  FCNT:2016-05-20 LogImprovement ADD-S */
    if(memblock_reserve(0xA4100000,0x300000) != 0) {
        printk(KERN_INFO "fta memblock_reserve failed\n");
	} else {
		printk(KERN_INFO "fta memblock_reserve success\n");
	}
/*  FCNT:2016-05-20 LogImprovement ADD-E ADD */

/* FCNT LIMITED:2017-01-16 MDRV_FJ_RDUMP_001_01 ADD-S */
#ifdef CONFIG_FJ_RPM_RAMDUMP
	{
		extern void rpm_dump_memblock_init(void);
		rpm_dump_memblock_init();
	}
...
#ifdef CONFIG_STRICT_MEMORY_RWX_EX
			set_pmd(pmd, __pmd(__pa(p) | prot_sect_kernel | PMD_SECT_PXN | PMD_SECT_UXN));
#else
