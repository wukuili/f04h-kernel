...
static ssize_t mem_write(struct file *file, const char __user *buf,
			 size_t count, loff_t *ppos)
{
/* FUJITSU:2014-11-14 GEN-141-SCRU-ALT ADD S */
	ssize_t ret;

	ret = security_ptrace_request_check(PTRACE_POKEDATA);
	if (ret)
		return ret;

/* FUJITSU:2014-11-14 GEN-141-SCRU-ALT ADD E */
			     ...
			     size_t count, loff_t *ppos)
{
	...
	if (task->signal->oom_score_adj == -1000)
		printk(KERN_ERR "%s: deprecated set call '%s' (%d), oom_score_adj to %hd -> %hd\n",
			__func__, task->comm, task->pid, task->signal->oom_score_adj, oom_adj);
					...
					size_t count, loff_t *ppos)
{
	...
	if ((task->signal->oom_score_adj == -1000) && (((short)oom_score_adj) != -1000))
		printk(KERN_ERR "%s: set '%s' (%d), oom_score_adj to %hd -> %hd\n",
			__func__, task->comm, task->pid, task->signal->oom_score_adj, (short)oom_score_adj);
